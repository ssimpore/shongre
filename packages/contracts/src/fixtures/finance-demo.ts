...
const entry = (
  ...
  accountCode: string,
  accountLabel: string,
  accountClass: "asset" | "liability" | "equity" | "revenue" | "expense" | "contra_revenue",
  side: "debit" | "credit",
  amountMinor: number,
) => ({ id, accountCode, accountLabel, accountClass, side, amount: eur(amountMinor) });
...
export const DEMO_FINANCE_TRANSACTIONS: readonly FinanceTransaction[] = [
  {
    ...
    entries: [
      ...
      entry("le_1842_3", "4870", "Revenus différés", "liability", "credit", 9900),
    ],
  },
  {
    id: "fin_tx_1839",
    ...
    entries: [
      ...
      entry("le_1839_2", "7061", "Revenus promotions", "revenue", "credit", 826),
  ...
  },
  {
    ...
    entries: [
      ...
      entry("le_1821_2", "7064", "Revenus de commission", "revenue", "credit", 1200),
      entry("le_1821_3", "4457", "TVA collectée", "liability", "credit", 240),
      entry("le_1821_4", "4670", "Dettes envers vendeur", "liability", "credit", 10560),
    ],
  },
  {
    id: "fin_rf_0082",
    ...
    entries: [
      entry("le_0082_1", "7091", "Remboursements", "contra_revenue", "debit", 4166),
  ...
  },
  {
    ...
    entries: [
      ...
      entry("le_1803_2", "7063", "Revenus publicitaires", "revenue", "credit", 20000),
  ...
  },
  {
    ...
    entries: [
      entry("le_0031_1", "4670", "Dettes envers vendeur", "liability", "debit", 81240),
      entry("le_0031_2", "5120", "Banque — virements en transit", "asset", "credit", 81240),
    ],
  },
];

const metric = (amountMinor: number, definition: string, changeBps?: number) => ({
...
export const DEMO_PLATFORM_FINANCE_DASHBOARD: PlatformFinanceDashboard = {
  ...
  isPeriodClosed: true,
  metrics: {
    platformRevenue: metric(4_286_000, "Revenus Shongre reconnus hors TVA et fonds dus aux vendeurs.", 1240),
    netRevenue: metric(3_974_200, "Revenus plateforme diminués des frais fournisseurs et remboursements.", 1010),
    gmv: metric(12_640_000, "Valeur brute des transactions marketplace confirmées.", 1530),
    grossCollected: metric(17_260_000, "Sommes brutes encaissées, incluant TVA et fonds vendeurs.", 1180),
    taxCollected: metric(857_200, "TVA collectée et due aux administrations fiscales."),
    sellerPayable: metric(10_684_000, "Montants acquis aux vendeurs mais non encore versés."),
    outstanding: metric(846_000, "Factures et paiements attendus à la date d’arrêté."),
    deferredRevenue: metric(1_190_000, "Encaissements d’abonnement restant à reconnaître sur les périodes futures."),
    providerFees: metric(287_800, "Frais Stripe et autres fournisseurs de paiement."),
    refunds: metric(24_000, "Remboursements et avoirs comptabilisés sur la période."),
    mrr: metric(1_842_000, "Revenu mensuel récurrent des abonnements actifs.", 870),
    arr: metric(22_104_000, "MRR annualisé à situation constante."),
  },
  revenueSources: [
    { key: "subscriptions", label: "Abonnements Pro", amount: eur(1_840_000), shareBps: 4293 },
    { key: "promotions", label: "Promotions", amount: eur(1_320_000), shareBps: 3080 },
    { key: "advertising", label: "Publicité sponsorisée", amount: eur(690_000), shareBps: 1610 },
    { key: "commissions", label: "Commissions & frais", amount: eur(436_000), shareBps: 1017 },
  ],
  timeSeries: [
    ["2026-07-24", 98000, 88000], ["2026-07-28", 124000, 112000],
    ["2026-08-01", 151000, 136000], ["2026-08-05", 132000, 119000],
    ["2026-08-09", 184000, 165000], ["2026-08-13", 141000, 127000],
    ["2026-08-17", 166000, 149000], ["2026-08-22", 152000, 138000],
  ].map(([date, platformRevenue, netRevenue]) => ({
    date: String(date),
    platformRevenue: eur(Number(platformRevenue)),
    netRevenue: eur(Number(netRevenue)),
  })),
  subscriptionHealth: { paidAccounts: 214, newSubscriptions: 18, churnBps: 280, arppu: eur(8607) },
  exceptions: [
    { key: "failed_payments", label: "Paiements échoués", count: 12, severity: "critical", amountImpact: eur(118_400) },
    { key: "reconciliation_gaps", label: "Écarts de rapprochement", count: 3, severity: "warning", amountImpact: eur(6) },
    { key: "failed_payouts", label: "Virements en échec", count: 2, severity: "critical", amountImpact: eur(113_260) },
  ],
  markets: [
...
export function createDemoAccountFinanceDashboard(
  ...
  return {
    ...
    metrics: {
      spending: metric(accountKind === "professional" ? 14_256 : 999, "Achats et abonnements payés à Shongre."),
      sellerEarnings: metric(accountKind === "professional" ? 183_420 : hasSellerActivity ? 25_000 : 0, "Produit net des ventes après commissions et remboursements."),
      availableForPayout: metric(accountKind === "professional" ? 64_280 : hasSellerActivity ? 12_600 : 0, "Solde acquis et disponible pour virement."),
      pendingPayout: metric(accountKind === "professional" ? 81_240 : 0, "Virements initiés ou temporairement retenus."),
      refunded: metric(accountKind === "professional" ? 4_999 : 0, "Remboursements reçus ou émis sur la période."),
    },
