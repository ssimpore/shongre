...
describe("cross-platform shared logic", () => {
  ...
  it("supports zero-, two-, and three-digit currency exponents", () => {
    expect(formatMoney({ amountMinor: 2_999, currency: "JPY" }, "en-US")).toContain(
      "2,999",
    );
    expect(formatMoney({ amountMinor: 2_999, currency: "EUR" }, "en-US")).toContain(
      "29.99",
    );
    expect(formatMoney({ amountMinor: 2_999, currency: "BHD" }, "en-US")).toContain(
      "2.999",
    );
  });
