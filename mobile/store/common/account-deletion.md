...
The public, login-optional web entry is `${PRODUCTION_ACCOUNT_DELETION_URL}` (canonical configuration defaults to `https://shongre.com/account/delete`). A signed-out visitor can reach sign-in or support. The same operation is exposed through `AuthService`, not duplicated in the UI.
