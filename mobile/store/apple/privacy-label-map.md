...
| Location: coarse                                             | Optional                             | Yes when saved to a listing | No       | User-requested listing location                                 |
...
| Contacts, precise location, sensitive info, audio            | No                                   | N/A                         | No       | Permissions absent/blocked                                      |

The generated `PrivacyInfo.xcprivacy` declares email, user ID, photos/videos, other user content, and coarse location for app functionality, tracking false, and UserDefaults reason CA92.1. A manifest is not a substitute for App Privacy answers. Update both if orders/payments, analytics, crash reporting, AI, social login, ads, or new SDKs ship.
