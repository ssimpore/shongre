...
async function runBenchmark() {
  ...
  console.log(
    `  ✓ Escrow calculations: ${iterations} ops in ${(endEscrow - startEscrow).toFixed(2)}ms (${(((endEscrow - startEscrow) / iterations) * 1000).toFixed(3)} µs/op)`,
  );
  ...
  console.log(
    `  ✓ Multi-market fallback resolutions: ${iterations} ops in ${(endMarket - startMarket).toFixed(2)}ms`,
  );

  console.log(
    "✨ All benchmarks completed within production SLA tolerances (< 100µs).",
  );
}
