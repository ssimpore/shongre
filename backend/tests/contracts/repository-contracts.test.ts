...
describe('Repository Contract & Dual-Mode Compatibility Tests', () => {
  describe('User Repository Contract', () => {
    ...
    it('saves and updates user in Demo mode', async () => {
      ...
      const updated = await demoRepo.update('usr_test_123', { name: 'Updated User' });
  ...
  describe('Listing Repository Contract', () => {
    ...
    it('toggles favorites deterministically in Demo mode', async () => {
      const isFav1 = await demoRepo.toggleFavorite('user_thomas', 'list_new_fav');
      expect(isFav1).toBe(true);
      const isFav2 = await demoRepo.toggleFavorite('user_thomas', 'list_new_fav');
  ...
  describe('Monetization, Verification, Messaging, Notification & Admin Repositories', () => {
    it('returns boosts and plans through Monetization Repository', async () => {
      const demoRepo = new DemoMonetizationRepository();
      const boosts = await demoRepo.getBoosts();
      expect(boosts.length).toBeGreaterThanOrEqual(4);
      expect(boosts.some((b) => b.type === 'urgent')).toBe(true);

      const plans = await demoRepo.getPlans();
  ...
  describe('Repository Container Dynamic Resolution', () => {
    it('creates container in demo mode with all demo repository instances', () => {
      ...
      expect(container.monetization instanceof DemoMonetizationRepository).toBe(true);
      expect(container.verification instanceof DemoVerificationRepository).toBe(true);
      expect(container.messaging instanceof DemoMessagingRepository).toBe(true);
      expect(container.notifications instanceof DemoNotificationRepository).toBe(true);
    ...
    it('creates container in database mode with all postgres repository instances', () => {
      ...
      expect(container.listings instanceof PostgresListingRepository).toBe(true);
      expect(container.markets instanceof PostgresMarketRepository).toBe(true);
      expect(container.taxonomy instanceof PostgresTaxonomyRepository).toBe(true);
      expect(container.orders instanceof PostgresOrderRepository).toBe(true);
      expect(container.monetization instanceof PostgresMonetizationRepository).toBe(true);
      expect(container.verification instanceof PostgresVerificationRepository).toBe(true);
      expect(container.messaging instanceof PostgresMessagingRepository).toBe(true);
      expect(container.notifications instanceof PostgresNotificationRepository).toBe(true);
      ...
      expect(container.workspace instanceof PostgresWorkspaceRepository).toBe(true);
    });
  });
});
