...
import { calculateGrowth, normalizeSignal, selectTrendingTopics, timeDecay } from '../../src/modules/trending/trending.engine.js';
...
describe('backend trending engine', () => {
  it('calculates growth and logarithmic normalization', () => {
    ...
    expect(timeDecay('2026-08-19T00:00:00.000Z', new Date('2026-08-20T00:00:00.000Z'))).toBeGreaterThan(0.8);
  });

  it('keeps one topic per parent and filters listings by availability', () => {
    const config = { ...createDefaultTrendingConfig(), maxTopics: 4, maxTopicsPerParentCategory: 1 };
    const topics = selectTrendingTopics([candidate('electronics'), candidate('fashion')], config, new Date('2026-08-20T00:00:00.000Z'));
    expect(topics).toHaveLength(2);
    expect(topics.every((topic) => topic.listings.every((item) => item.status === 'published' || item.status === 'reserved'))).toBe(true);
  });
});
