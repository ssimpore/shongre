...
import { issueToken, verifyToken, TokenError, extractBearerToken } from '../../src/shared/auth/tokens.js';
import {
  GUEST_PRINCIPAL,
  Principal,
...
const buyer: Principal = { userId: 'user_thomas', email: 'thomas@example.fr', role: 'individual_buyer' };
const admin: Principal = { userId: 'user_admin', email: 'admin@shongre.com', role: 'admin' };
...
describe('Password hashing', () => {
  ...
  it('returns false rather than throwing for absent or malformed hashes', async () => {
    ...
    expect(await verifyPassword('anything-at-all', 'not-a-real-hash')).toBe(false);
    expect(await verifyPassword('anything-at-all', 'scrypt$64$zz$zz')).toBe(false);
    expect(await verifyPassword('anything-at-all', 'bcrypt$64$aa$bb')).toBe(false);
  });
  ...
  it('enforces a minimum length and rejects all-numeric passwords', () => {
    ...
    expect(() => assertPasswordAcceptable('1'.repeat(MIN_PASSWORD_LENGTH))).toThrow(WeakPasswordError);
    expect(() => assertPasswordAcceptable('long-enough-and-mixed')).not.toThrow();
  });
});

describe('Session tokens', () => {
  const claims = { sub: 'user_thomas', email: 'thomas@example.fr', role: 'individual_buyer' as const };
  ...
  it('rejects a token signed with a different secret', () => {
    const token = issueToken(claims, SECRET);
    expect(() => verifyToken(token, 'a-different-secret-entirely')).toThrow(TokenError);
  });

  it('rejects a token whose payload was swapped for a privileged one', () => {
    const token = issueToken(claims, SECRET);
    const [header, , signature] = token.split('.');
    const forged = Buffer.from(JSON.stringify({ ...claims, role: 'super_admin', exp: 9999999999 }))
      .toString('base64url');
    expect(() => verifyToken(`${header}.${forged}.${signature}`, SECRET)).toThrow(TokenError);
  });
  ...
  it('refuses to issue or verify without a secret', () => {
    ...
    expect(() => verifyToken(issueToken(claims, SECRET), '')).toThrow(TokenError);
  });
...
describe('Principal guards', () => {
  ...
  it('rejects a guest before checking permissions', () => {
    ...
    expect(() => requirePermission(GUEST_PRINCIPAL, 'listing.read')).toThrow(/connecté/i);
  });

  it('allows owners and refuses everyone else', () => {
    expect(() => requireOwnership(buyer, 'user_thomas')).not.toThrow();
    expect(() => requireOwnership(buyer, 'user_camille')).toThrow(/introuvable/i);
  });

  it('honours an explicit staff override', () => {
    expect(() => requireOwnership(admin, 'user_camille', 'user.manage')).not.toThrow();
    // Without naming an override, even an admin is not an owner.
    expect(() => requireOwnership(admin, 'user_camille')).toThrow(/introuvable/i);
  });
...
describe('Stripe webhook signatures', () => {
  ...
  it('accepts a correctly signed payload', () => {
    ...
    expect(verifyStripeSignature({ payload, signatureHeader: header, secret, nowSeconds: now })).toEqual({ ok: true });
  });
  ...
  it('rejects a payload modified after signing', () => {
    ...
    const result = verifyStripeSignature({ payload: tampered, signatureHeader: header, secret, nowSeconds: now });
    expect(result.ok).toBe(false);
  });

  it('rejects a signature made with a different secret', () => {
    const header = buildStripeSignatureHeader(payload, 'whsec_attacker_secret', now);
    expect(verifyStripeSignature({ payload, signatureHeader: header, secret, nowSeconds: now }).ok).toBe(false);
  });
  ...
  it('rejects a replayed event outside the tolerance window', () => {
    ...
    expect(result).toEqual({ ok: false, reason: 'signature timestamp outside tolerance window' });
  });

  it('rejects missing, empty and malformed headers', () => {
    expect(verifyStripeSignature({ payload, signatureHeader: undefined, secret, nowSeconds: now }).ok).toBe(false);
    expect(verifyStripeSignature({ payload, signatureHeader: '', secret, nowSeconds: now }).ok).toBe(false);
    expect(verifyStripeSignature({ payload, signatureHeader: 'garbage', secret, nowSeconds: now }).ok).toBe(false);
    expect(verifyStripeSignature({ payload, signatureHeader: `t=${now}`, secret, nowSeconds: now }).ok).toBe(false);
  });
  ...
  it('accepts one valid signature among several during secret rotation', () => {
    const valid = buildStripeSignatureHeader(payload, secret, now).split('v1=')[1];
    const header = `t=${now},v1=${'0'.repeat(valid.length)},v1=${valid}`;
    expect(verifyStripeSignature({ payload, signatureHeader: header, secret, nowSeconds: now }).ok).toBe(true);
  });
});
