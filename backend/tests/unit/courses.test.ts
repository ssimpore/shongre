...
describe('Shongre Cours domain service', () => {
  it('projects Cours plan prices from the active commercial version', async () => {
    class CommercialRepository extends DemoBusinessRulesRepository {
      override async getActiveCatalog(marketCode: string) {
        ...
        changed.products.find((product) => product.id === 'course.tutor.pro')!.prices[0].amount.amountMinor = 2345;
        return changed;
      }
    }
    ...
    expect(catalog.plans.find((plan) => plan.id === 'tutor_pro')?.monthlyPrice?.amountMinor).toBe(2345);
  });
  ...
  it('requires a guardian and explicit consent context for every minor request', async () => {
    ...
    await expect(
      service.submitLearnerRequest('user_learner', {
        ...
        objective: 'Préparer le brevet et retrouver de la confiance en mathématiques.',
  ...
  it('routes a valid request with normalized money and without duplicate tutor leads', async () => {
    ...
    expect(tutorLeads.filter((lead) => lead.learnerRequestId === request.id)).toHaveLength(1);
  });
  ...
  it('releases contact only after the owning tutor accepts a live lead', async () => {
    ...
    await expect(
      service.respondToOwnLead('another_user', 'tutor_sophie', lead.id, 'accept'),
    ).rejects.toThrow(/destinée/i);
  });

  it('hard-blocks booking while the market gate is disabled', async () => {
    const { service } = createService();
    await expect(service.createBooking('learner', 'FR', {} as never)).rejects.toThrow(
      /pas activés/i,
    );
  });
  ...
  it('enforces organization membership before returning team data', async () => {
    ...
    await expect(
      service.getOwnOrganizationWorkspace('unrelated_user', 'org_college_lumiere'),
  ...
  it('updates market-scoped subjects without changing their identifier', async () => {
    ...
    const reactivated = await service.updateSubject('FR', 'subject_mathematics', {
      isActive: true,
    });
