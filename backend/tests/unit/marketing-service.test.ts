...
describe("Marketing service", () => {
  it("estimates an authoritative audience and excludes unsubscribed profiles", async () => {
    ...
    const before = await service.estimateAudience(principal, {
      includeListIds: [lists.items[0].id],
      includeSegmentIds: [],
      includeProfileIds: [],
      excludeListIds: [],
      excludeSegmentIds: [],
    ...
    await service.unsubscribeProfile(
      principal,
      profiles.items.find((item) => item.email === "camille@example.fr")!.id,
    );
  ...
  it("keeps global marketing suppression from silently reactivating", async () => {
    ...
    const profile = (
      await service.listProfiles(principal, { query: "camille@example.fr" })
    ).items[0];
    ...
    await expect(
      service.confirmProfile(principal, profile.id),
    ).rejects.toMatchObject({ code: "CONFLICT" });
  });
  ...
  it("uses double opt-in and non-enumerating public receipts", async () => {
    ...
    await expect(
      service.confirmPublic({ token: "not-a-valid-token" }),
    ).rejects.toThrow();
  });
  ...
  it("lets an authenticated account manage preferences and explicitly resubscribe", async () => {
    ...
    const updated = await service.updateAccountPreferences(account, {
      marketCode: "FR",
      topics: ["new_features"],
    });
    expect(updated.topics).toEqual(["new_features"]);
    expect((await service.unsubscribeAccount(account, "FR")).status).toBe(
      "UNSUBSCRIBED",
    );

    const resubscribed = await service.subscribeAccount(account, {
      marketCode: "FR",
      locale: "fr-FR",
      topics: ["editorial"],
      consentGiven: true,
  ...
  it("rejects unapproved fields even in nested segment groups", async () => {
    const service = new MarketingService(new DemoMarketingRepository());
    await expect(
      service.createSegment(principal, {
        name: "Unsafe segment",
        definition: {
          combinator: "AND",
          conditions: [],
  ...
  it("preflights and queues a valid campaign through shared provider abstractions", async () => {
    const service = new MarketingService(new DemoMarketingRepository());
    const campaign = (await service.listCampaigns(principal)).items.find(
      (item) => item.status === "DRAFT",
    )!;
    const preflight = await service.preflight(principal, campaign.id);
    expect(preflight.canSend).toBe(true);
    expect(preflight.info.map((item) => item.code)).toContain(
      "IDEMPOTENCY_ENABLED",
    );
  ...
  it("persists a manual A/B winner and accepts attributed conversions idempotently", async () => {
    ...
    const campaign = await service.createCampaign(principal, {
      ...
      subject: "Variante A",
      content: {
        blocks: [{ id: "legal", type: "UNSUBSCRIBE", text: "Se désabonner" }],
      },
      audience: {
    ...
    expect(
      (
        await service.selectExperimentWinner(principal, campaign.id, {
          variantId: "b",
        })
      ).winningVariantId,
    ).toBe("b");

    const operationsService = new MarketingOperationsService(
      repository,
      operations,
    );
    await expect(
      operationsService.recordConversion(principal, {
        idempotencyKey: "order:demo-1",
