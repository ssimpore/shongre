...
describe('BusinessRulesService quotes', () => {
  ...
  it('snapshots the selected annual price and grants subscription entitlements after payment', async () => {
    ...
    const plan = catalog.products.find((product) => product.id === 'plan.pro.starter')!;
    const annualPrice = plan.prices.find((price) => price.billingPeriod === 'year')!;
    ...
    const subscriptions = await service.getSubscriptions('professional_subscription_test');
    const entitlements = await service.getActiveEntitlements('professional_subscription_test');
    expect(subscriptions).toContainEqual(
      expect.objectContaining({ productId: plan.id, status: 'active' }),
    );
    expect(entitlements.some((entry) => entry.productId === plan.id)).toBe(true);
  ...
  it('returns a stable reason when a promotion is unpublished', async () => {
    const service = new BusinessRulesService(new DemoBusinessRulesRepository());
    const validation = await service.validatePromotion('individual_promotion_test', {
      code: 'WELCOME10',
      productIds: ['premium.urgent'],
      marketCode: 'FR',
    });
  ...
  it('publishes atomically only after a distinct approval', async () => {
    ...
    expect(overview.versions.find((version) => version.id === 'commercial-fr-v1')?.status).toBe('archived');
    ...
    expect((await service.getAdminOverview('FR')).versions.find((entry) => entry.id === 'commercial-fr-v1')?.status).toBe('archived');
  });
