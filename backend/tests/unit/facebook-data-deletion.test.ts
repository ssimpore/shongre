...
function signedRequest(
  subject = "facebook-app-scoped-user-42",
...
describe("Facebook provider-data deletion callback", () => {
  it("validates the signed request, queues a minimal record, and exposes opaque status", async () => {
    ...
    await expect(service.status(result.confirmation_code)).resolves.toEqual({
      status: "completed",
    });
  ...
  it("rejects a forged or stale request without queuing it", async () => {
    ...
    await expect(
      service.request(`forged.${signedRequest().split(".")[1]}`),
    ).rejects.toThrow("invalide");
    await expect(
      service.request(
        signedRequest("old-user", Math.floor(Date.now() / 1000) - 90_000),
      ),
    ).rejects.toThrow("invalide");
