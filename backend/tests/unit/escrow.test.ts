...
import { calculateOrderTotal, DEFAULT_MARKET_RULES } from '../../src/shared/money/escrow.js';

describe('Escrow & Order Calculation Engine', () => {
  it('calculates standard France protection fee correctly (4% + 0.70€)', () => {
    const res = calculateOrderTotal({ itemAmount: 100, shippingFee: 5, marketCode: 'FR' });
    // 100 * 0.04 + 0.70 = 4.70
  ...
  it('calculates Belgian market fee correctly (4.5% + 0.80€)', () => {
    const res = calculateOrderTotal({ itemAmount: 200, shippingFee: 10, marketCode: 'BE' });
    // 200 * 0.045 + 0.80 = 9.80
  ...
  it('handles 0 euro shipping fee for hand delivery', () => {
    const res = calculateOrderTotal({ itemAmount: 50, shippingFee: 0, marketCode: 'FR' });
    // 50 * 0.04 + 0.70 = 2.70
