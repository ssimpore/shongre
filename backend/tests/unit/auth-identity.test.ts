...
describe('Email normalization', () => {
  it('lowercases and trims', () => {
    expect(normalizeEmail('  Chloe.Martin@Example.FR ')).toBe('chloe.martin@example.fr');
  });

  it('rejects values that are not addresses', () => {
    for (const bad of ['', '   ', 'not-an-email', '@example.fr', 'user@', null, undefined, 42 as never]) {
...
describe('resolveIdentity — rule 1: exact identity match', () => {
  it('signs in the linked account', () => {
    const linked: LinkedIdentity = { userId: 'user_chloe', provider: 'google', subject: 'google-sub-12345' };
    const result = resolveIdentity(googleProfile, linked, null);
    expect(result).toEqual({ outcome: 'authenticate', userId: 'user_chloe', identityMatched: true });
  });
  ...
  it('wins even when the provider email now belongs to a different account', () => {
    ...
    const linked: LinkedIdentity = { userId: 'user_chloe', provider: 'google', subject: 'google-sub-12345' };
    const someoneElse = account({ userId: 'user_intruder', email: 'chloe.martin@example.fr' });
  ...
  it('rejects a mismatched identity row rather than trusting it', () => {
    const wrong: LinkedIdentity = { userId: 'user_chloe', provider: 'facebook', subject: 'other' };
...
describe('resolveIdentity — new accounts', () => {
  ...
  it('creates an account with no email when Facebook withholds one', () => {
    const facebook: ProviderProfile = { provider: 'facebook', subject: 'fb-app-scoped-9', email: null };
    const result = resolveIdentity(facebook, null, null);
    expect(result).toEqual({ outcome: 'create_account', email: null, emailVerified: false });
  });
...
describe('resolveIdentity — rules 2-4: never merge on email alone', () => {
  it('requires linking when a verified email matches an existing account', () => {
    ...
    if (result.outcome !== 'require_account_linking') throw new Error('unreachable');
  ...
  it('does NOT merge when the provider did not verify the email', () => {
    ...
    const unverified: ProviderProfile = { ...googleProfile, emailVerified: false };
    const result = resolveIdentity(unverified, null, account());
    expect(result).toEqual({ outcome: 'create_account', email: null, emailVerified: false });
  });

  it('treats a missing emailVerified claim as unverified', () => {
    const noClaim: ProviderProfile = { ...googleProfile, emailVerified: undefined };
...
describe('resolveIdentity — blocked accounts cannot be reached via social login', () => {
  for (const status of ['suspended', 'banned', 'deleted', 'archived'] as const) {
    it(`blocks a ${status} account`, () => {
      const result = resolveIdentity(googleProfile, null, account({ status }));
      expect(result).toEqual({ outcome: 'blocked', userId: 'user_chloe', status });
    });
  }
});
...
describe('resolveIdentity — malformed provider responses', () => {
  it('rejects a missing subject', () => {
    expect(() => resolveIdentity({ ...googleProfile, subject: '' }, null, null)).toThrow(AppError);
    expect(() => resolveIdentity({ ...googleProfile, subject: '   ' }, null, null)).toThrow(AppError);
  });

  it('rejects an unsupported provider', () => {
    const bogus = { ...googleProfile, provider: 'twitter' } as unknown as ProviderProfile;
...
describe('evaluateLinkRequest', () => {
  ...
  it('requires recent authentication', () => {
    expect(evaluateLinkRequest({ ...base, hasRecentAuthentication: false })).toEqual({
      decision: 'rejected',
  ...
  it('rule 6: refuses to move an identity between accounts', () => {
    const owned: LinkedIdentity = { userId: 'user_someone_else', provider: 'google', subject: 'google-sub-12345' };
  ...
  it('is idempotent when already linked to the caller', () => {
    const mine: LinkedIdentity = { userId: 'user_chloe', provider: 'google', subject: 'google-sub-12345' };
  ...
  it('refuses a second identity for a provider already linked', () => {
    expect(
      evaluateLinkRequest({ ...base, actingAccountProviders: ['password', 'google'] })
...
describe('evaluateUnlinkRequest — nobody may lock themselves out', () => {
  ...
  it('refuses to remove the only sign-in method', () => {
    ...
    ).toEqual({ decision: 'rejected', reason: 'would_remove_last_login_method' });
  });
  ...
  it('requires recent authentication', () => {
    ...
    ).toEqual({ decision: 'rejected', reason: 'recent_authentication_required' });
  });
...
describe('reconcileProfileFields — the provider never overwrites user choices', () => {
  ...
  it('fills a missing name', () => {
    const update = reconcileProfileFields({ ...current, name: null }, googleProfile);
  ...
  it('does not clear a name when Apple omits it on a return visit', () => {
    ...
    const appleReturning: ProviderProfile = { provider: 'apple', subject: 'apple-sub-77', displayName: null };
  ...
  it('promotes unverified email to verified, but never the reverse', () => {
    expect(reconcileProfileFields({ ...current, isEmailVerified: false }, googleProfile).isEmailVerified).toBe(true);
    const unverified: ProviderProfile = { ...googleProfile, emailVerified: false };
    expect(reconcileProfileFields(current, unverified)).not.toHaveProperty('isEmailVerified');
  });

  it('does not adopt a provider avatar once the user has edited their profile', () => {
    const update = reconcileProfileFields(
      { name: 'Chloé M.', avatarUrl: null, isEmailVerified: true, hasUserEditedProfile: true },
      googleProfile
    );
...
describe('resolveSafeRedirect — open redirect protection', () => {
  it('accepts ordinary internal paths', () => {
    expect(resolveSafeRedirect('/annonces/velo-de-course')).toBe('/annonces/velo-de-course');
    expect(resolveSafeRedirect('/recherche?q=velo&page=2')).toBe('/recherche?q=velo&page=2');
  });
  ...
  it('rejects a duplicated parameter rather than picking one', () => {
    expect(resolveSafeRedirect(['/safe', 'https://evil.com'])).toBe(DEFAULT_RETURN_PATH);
  });
  ...
  it('refuses to bounce back into the auth flow', () => {
    for (const value of ['/login', '/connexion?next=/x', '/auth/callback', '/inscription']) {
      expect(resolveSafeRedirect(value)).toBe(DEFAULT_RETURN_PATH);
    }
  });

  it('honours a custom fallback', () => {
    expect(resolveSafeRedirect('https://evil.com', { fallback: '/vendeur' })).toBe('/vendeur');
  });

  it('preserves the locale the user started in', () => {
    expect(resolveSafeRedirect('/annonces', { locale: 'fr' })).toBe('/fr/annonces');
    expect(resolveSafeRedirect('/fr/annonces', { locale: 'fr' })).toBe('/fr/annonces');
    expect(resolveSafeRedirect('/', { locale: 'fr' })).toBe('/fr');
  });

  it('ignores a malformed locale rather than building a broken path', () => {
    expect(resolveSafeRedirect('/annonces', { locale: '../evil' })).toBe('/annonces');
  });
});

describe('isAllowedCallbackOrigin', () => {
  const allowed = ['https://shongre.fr', 'https://staging.shongre.fr', 'http://localhost:3000'];

  it('accepts an exact configured origin', () => {
    expect(isAllowedCallbackOrigin('https://shongre.fr', allowed)).toBe(true);
    expect(isAllowedCallbackOrigin('http://localhost:3000', allowed)).toBe(true);
  });
...
describe('buildStatePayload', () => {
  it('sanitizes the return path before it enters signed state', () => {
    const state = buildStatePayload({ nonce: 'n1', returnTo: 'https://evil.com', locale: 'fr' });
  ...
  it('carries a link intent through', () => {
    const state = buildStatePayload({ nonce: 'n2', returnTo: '/compte/securite', intent: 'link' });
