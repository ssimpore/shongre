...
export class MarketingWebhookWorker {
  ...
  async run(): Promise<{ processed: number; delivered: number }> {
    ...
    const { data, error } = await this.client.rpc("claim_marketing_webhook_delivery");
    ...
    try {
      ...
      const retrySeconds = Math.min(21_600, 30 * 2 ** Math.max(0, delivery.attempt_count - 1));
      await Promise.all([
        this.client.from("marketing_webhook_deliveries").update({
          status: terminal ? "DEAD_LETTER" : "FAILED",
          available_at: new Date(Date.now() + retrySeconds * 1_000).toISOString(),
          last_error_code: "OUTGOING_WEBHOOK_FAILED",
        }).eq("id", delivery.id),
        this.client.from("marketing_webhook_subscriptions").update({
          last_failure_at: new Date().toISOString(),
        }).eq("tenant_id", delivery.tenant_id).eq("id", delivery.subscription_id),
      ]);
  ...
  private async deliver(delivery: WebhookDeliveryRow) {
    const { data: subscription, error } = await this.client
      ...
      .select("url,status,signing_secret_ciphertext,signing_secret_iv,signing_secret_tag,key_version")
      .eq("tenant_id", delivery.tenant_id)
      .eq("id", delivery.subscription_id)
      .maybeSingle();
    if (error) throw error;
    if (!subscription || subscription.status !== "ACTIVE") {
      await this.client.from("marketing_webhook_deliveries").update({ status: "CANCELLED" }).eq("id", delivery.id);
      return false;
    }
    const { data: workspace, error: workspaceError } = await this.client.from("marketing_workspaces").select("market_code").eq("tenant_id", delivery.tenant_id).order("created_at").limit(1).single();
    if (workspaceError) throw workspaceError;
    const usage = await this.operations.usage(delivery.tenant_id, workspace.market_code);
    if (!usage.entitlements.enabled || !usage.entitlements.webhooks) {
      await this.client.from("marketing_webhook_deliveries").update({ status: "CANCELLED", last_error_code: "MARKETING_WEBHOOK_NOT_ENTITLED" }).eq("id", delivery.id);
      return false;
    }
    ...
    if (Buffer.byteLength(body, "utf8") > 256_000) throw new Error("OUTGOING_WEBHOOK_PAYLOAD_TOO_LARGE");
    ...
    if (response.status < 200 || response.status >= 300) throw new Error(`OUTGOING_WEBHOOK_HTTP_${response.status}`);
