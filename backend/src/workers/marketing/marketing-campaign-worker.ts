  ...
  const blocks = (content?.blocks ?? [])
    .map((block: any) => {
      if (block.type === "HEADING")
        return `<h2>${escapeHtml(personalize(String(block.text || ""), profile, links))}</h2>`;
      if (block.type === "PARAGRAPH")
        return `<p>${escapeHtml(personalize(String(block.text || ""), profile, links)).replace(/\n/g, "<br>")}</p>`;
      if (block.type === "BUTTON")
        return `<p><a href="${escapeHtml(buttonUrls.get(block.id) || block.href)}">${escapeHtml(block.label)}</a></p>`;
      if (block.type === "IMAGE")
        return `<p><img src="${escapeHtml(block.src)}" alt="${escapeHtml(block.alt || "")}" style="max-width:100%;height:auto"></p>`;
      if (block.type === "DIVIDER") return "<hr>";
      if (block.type === "SPACER")
        return '<div aria-hidden="true" style="height:24px"></div>';
      if (block.type === "UNSUBSCRIBE")
        return `<p><a href="${escapeHtml(links.unsubscribeUrl)}">${escapeHtml(block.text || "Se désabonner")}</a></p>`;
      if (block.type === "PREFERENCE_CENTER")
        return `<p><a href="${escapeHtml(links.preferencesUrl)}">${escapeHtml(block.text || "Gérer mes préférences")}</a></p>`;
...
export class MarketingCampaignWorker {
  ...
  async run(): Promise<{ processed: number; accepted: number }> {
    ...
    try {
      ...
      const delaySeconds = Math.min(
        3600,
        2 ** Math.max(1, job.attempt_count) * 15,
      );
      await this.client
        .from("marketing_jobs")
        .update({
      ...
      logger.error("marketing_campaign_batch_failed", {
        jobId: job.id,
  ...
  private async process(job: MarketingJobRow) {
    ...
    if (sender.status !== "VERIFIED")
      throw new Error("MARKETING_SENDER_NOT_VERIFIED");
    ...
    const connection = await providerConnectionService.resolve(
      ...
      },
      {
        ...
        explicitConnectionId:
          ...
          workspace.default_provider_connection_id,
    ...
    if (
      ...
      throw new Error("MARKETING_PLATFORM_EMAIL_NOT_ENTITLED");
    if (connection.ownerType !== "PLATFORM" && !usage.entitlements.byoEmail)
      throw new Error("MARKETING_BYO_EMAIL_NOT_ENTITLED");
    const { error: quotaError } = await this.client.rpc(
      "reserve_marketing_campaign_quota",
      {
    ...
    const capabilities = await capabilityGateways.emailDelivery.getCapabilities(
      {
        tenantId: job.tenant_id,
        userId: campaign.created_by,
    ...
    const rateLimitPerSecond = Math.min(
      100,
      Math.max(1, Number(connection.configuration.rateLimitPerSecond || 10)),
    );
    ...
    const { data: recipients, error: recipientsError } = await this.client
      ...
      .select("id,profile_id,idempotency_key,variant_id")
      .eq("tenant_id", job.tenant_id)
      .eq("campaign_id", campaign.id)
    ...
    if (!recipients.length) {
      if (
        version.experiment?.enabled &&
        version.experiment.winnerMode === "AUTOMATIC" &&
        !campaign.winning_variant_id
      ) {
        const winnerAt = new Date(
          new Date(campaign.started_at ?? campaign.created_at).getTime() +
            Number(version.experiment.durationMinutes || 240) * 60_000,
        );
        if (winnerAt.getTime() > Date.now()) {
          await this.client
            ...
            .update({
              status: "QUEUED",
              available_at: winnerAt.toISOString(),
              started_at: null,
            })
            .eq("id", job.id);
          return { processed: 0, accepted: 0 };
        }
        const analytics = await this.operations.analytics(
          job.tenant_id,
          campaign.id,
        );
        const score = (variant: (typeof analytics.variants)[number]) =>
          version.experiment.winnerMetric === "CONVERSION_RATE"
            ? variant.conversionRate
            : version.experiment.winnerMetric === "OPEN_RATE"
              ? variant.delivered
        ...
        const winner =
          ...
          )[0]?.variantId ?? version.experiment.variants[0]?.id;
        if (winner)
          await this.client
            .from("marketing_campaigns")
            .update({ winning_variant_id: winner })
            .eq("tenant_id", job.tenant_id)
            .eq("id", campaign.id)
    ...
    const [recentWeekResult, recentDayResult, suppressionsResult] =
      await Promise.all([
        this.client
          .from("marketing_campaign_recipients")
          .select("profile_id")
    ...
    const suppressedEmails = new Set(
      (suppressionsResult.data ?? []).map((row: any) => row.normalized_email),
    );
    let accepted = 0;
    for (const recipient of recipients) {
      ...
      const capped =
        (dayCounts.get(profile.id) ?? 0) >=
          Number(workspace.frequency_cap_day || 3) ||
        (weekCounts.get(profile.id) ?? 0) >=
          Number(workspace.frequency_cap_week || 7);
      if (
        profile.status !== "SUBSCRIBED" ||
        suppressedEmails.has(profile.normalized_email) ||
      ...
      const variant = version.experiment?.enabled
        ? version.experiment.variants?.find(
            (item: any) => item.id === recipient.variant_id,
          )
        : undefined;
      ...
      if (workspace.settings?.clickTrackingEnabled === true)
        for (const block of content.blocks ?? [])
          if (
            block.type === "BUTTON" &&
            String(block.href).startsWith("https://")
          ) {
            ...
            trackingRows.push({
              ...
              kind: "CLICK",
              target_url: block.href,
              expires_at: expiresAt,
            });
            buttonUrls.set(
              block.id,
              `${publicApiUrl}/api/v1/marketing/track/click?token=${encodeURIComponent(token)}`,
            );
          }
      let openPixelUrl: string | undefined;
      if (workspace.settings?.openTrackingEnabled === true) {
        const token = randomBytes(32).toString("base64url");
        trackingRows.push({
          tenant_id: job.tenant_id,
          recipient_id: recipient.id,
        ...
        openPixelUrl = `${publicApiUrl}/api/v1/marketing/track/open?token=${encodeURIComponent(token)}`;
      }
      ...
      try {
        result = await capabilityGateways.emailDelivery.send(
          ...
          },
          {
            ...
            subject: personalize(subject, profile, links),
            textBody: personalize(renderPlainText(content), profile, links),
            htmlBody: renderHtml(
              content,
              profile,
              links,
        ...
        if (trackingRows.length) {
          await this.client
            ...
            .in(
              "token_hash",
              trackingRows.map((row) => row.token_hash),
            );
        }
        throw error;
      }
      await Promise.all([
        this.client
          ...
          .eq("id", recipient.id)
          .eq("send_status", "QUEUED"),
        this.client.from("marketing_delivery_events").upsert(
          {
            tenant_id: job.tenant_id,
            recipient_id: recipient.id,
            provider_connection_id: connection.id,
            provider_event_id: `accepted:${result.externalMessageId}`,
            provider_message_id: result.externalMessageId,
            event_type: "ACCEPTED",
      ...
      await enqueueMarketingWebhookEvent(
        job.tenant_id,
        "email.accepted",
        `accepted:${connection.id}:${result.externalMessageId}`,
        {
      ...
      accepted += 1;
      if (minimumSendIntervalMs > 0)
        await new Promise((resolve) =>
          setTimeout(resolve, minimumSendIntervalMs),
        );
    }
    await this.client
      .from("marketing_jobs")
