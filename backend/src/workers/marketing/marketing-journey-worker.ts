...
function interpolate(
  ...
  for (const [key, customValue] of Object.entries(
    ...
    if (
      ...
      variables[key] = String(customValue);
  }
  return value.replace(
    /\{\{\s*([a-zA-Z0-9_]+)(?:\s*\|\s*([^}]+))?\s*\}\}/g,
    (_match, key, fallback) => variables[key] || fallback?.trim() || "",
  );
}
...
function templateText(content: any) {
  return (
    ...
    (content?.blocks ?? [])
      .map((block: any) => {
        if (["HEADING", "PARAGRAPH", "FOOTER"].includes(block.type))
          return block.text ?? "";
...
function resolveConditionValue(
  ...
  if (field.startsWith("customValues."))
    return profile?.custom_values?.[field.slice(13)];
...
function conditionMatches(
  node: MarketingJourneyNode,
  profile: any,
  safeContext: Record<string, unknown>,
) {
  const actual = resolveConditionValue(
    text(node.configuration.field, 120),
    profile,
    safeContext,
  );
  ...
  switch (text(node.configuration.operator, 40).toUpperCase()) {
    ...
    case "BEFORE":
      return (
        new Date(text(actual)).getTime() < new Date(text(expected)).getTime()
      );
    case "AFTER":
      return (
        new Date(text(actual)).getTime() > new Date(text(expected)).getTime()
...
export class MarketingJourneyWorker {
  ...
  async run(): Promise<{ processed: number; completed: number }> {
    ...
    try {
      ...
      const delaySeconds = Math.min(
        3_600,
        15 * 2 ** Math.max(0, execution.attempt_count - 1),
      );
      await this.client
        ...
        .update({
          ...
          last_error_code: text(
            error?.code || error?.message || "AUTOMATION_STEP_FAILED",
            120,
          ),
  ...
  private async process(execution: ExecutionRow): Promise<boolean> {
    const { data: entitlementWorkspace, error: entitlementWorkspaceError } =
      await this.client
    ...
    const usage = await this.operations.usage(
      execution.tenant_id,
      entitlementWorkspace.market_code,
    );
    if (!usage.entitlements.enabled || !usage.entitlements.automation) {
      await this.client
        ...
        .update({
          status: "STOPPED",
          last_error_code: "MARKETING_AUTOMATION_NOT_ENTITLED",
    ...
    for (let localSteps = 0; localSteps < 20; localSteps += 1) {
      if (depth >= definition.maxExecutionDepth)
        throw new Error("AUTOMATION_MAX_DEPTH_REACHED");
      ...
      try {
        ...
        if (!result.nextNodeId)
          throw new Error("AUTOMATION_NEXT_NODE_REQUIRED");
        if (result.waitUntil) {
          await this.step(execution, node, "WAITING", result.safeResult ?? {});
          const { error: waitError } = await this.client
            .from("automation_executions")
            .update({
              status: "WAITING",
              current_node_id: result.nextNodeId,
              depth: nextDepth,
              available_at: result.waitUntil,
              last_error_code: null,
            })
            .eq("tenant_id", execution.tenant_id)
            .eq("id", execution.id);
        ...
        await this.step(
          ...
          "FAILED",
          {},
          text(error?.code || error?.message || "AUTOMATION_STEP_FAILED", 120),
        );
        throw error;
      }
    }
  ...
  private async executeNode(
    ...
    const profile = execution.subject_id
      ? await this.profile(execution.tenant_id, execution.subject_id)
      : null;
    switch (node.type) {
      case "END":
        return { completed: true };
      case "WAIT": {
        const minutes = configurationNumber(
          node.configuration.durationMinutes,
          60,
          1,
          525_600,
        );
        return {
          ...
          waitUntil: new Date(Date.now() + minutes * 60_000).toISOString(),
          safeResult: { durationMinutes: minutes },
        };
      }
      case "CONDITION":
      case "BRANCH": {
        ...
        return {
          nextNodeId: matched ? node.nextNodeId : node.alternateNodeId,
          safeResult: { matched },
        };
      }
      case "SEND_EMAIL":
        return this.sendEmail(execution, node, profile);
      case "ADD_TO_LIST": {
      ...
      case "UPDATE_FIELD": {
        ...
        if (!field.startsWith("customValues."))
          throw new Error("AUTOMATION_FIELD_NOT_ALLOWED");
        const key = field.slice(13);
        if (!/^[a-zA-Z][a-zA-Z0-9_]{0,79}$/.test(key))
          throw new Error("AUTOMATION_FIELD_NOT_ALLOWED");
        const customValues = {
          ...(profile.custom_values ?? {}),
          [key]: node.configuration.value,
        };
        const { error } = await this.client
          .from("marketing_profiles")
      ...
      case "CREATE_CRM_TASK":
        return this.createCrmTask(execution, node, profile);
      case "RECORD_CRM_ACTIVITY":
        return this.recordCrmActivity(execution, node, profile);
      case "CALL_WEBHOOK": {
        const eventType = text(
          node.configuration.eventType || "journey.action",
          120,
        );
        if (!/^[a-z][a-z0-9_.-]{1,119}$/.test(eventType))
          throw new Error("AUTOMATION_WEBHOOK_EVENT_INVALID");
        await enqueueMarketingWebhookEvent(
          execution.tenant_id,
          eventType,
          `journey:${execution.id}:${node.id}`,
          {
            executionId: execution.id,
            journeyId: execution.definition_id,
            profileId: execution.subject_id,
            nodeId: node.id,
          },
        );
      ...
      case "RUN_AI":
        return this.runAi(execution, node);
    }
  }
  ...
  private async sendEmail(
    ...
    if (
      ...
      profile.custom_values?.doNotContact === true
    )
      throw new Error("AUTOMATION_PROFILE_NOT_ELIGIBLE");
    const [
      { data: suppression, error: suppressionError },
    ...
    const [campaignDay, campaignWeek, automationDay, automationWeek] =
      await Promise.all([
    ...
    if (
      campaignDay + automationDay >= Number(workspace.frequency_cap_day || 3) ||
      campaignWeek + automationWeek >= Number(workspace.frequency_cap_week || 7)
    )
      throw new Error("AUTOMATION_FREQUENCY_CAP");
    const templateId = text(node.configuration.templateId, 80);
    const senderId = text(node.configuration.senderIdentityId, 80);
    const [{ data: template, error: templateError }, senderResult] =
      await Promise.all([
        this.client
    ...
    if (!template || template.status !== "ACTIVE")
      throw new Error("AUTOMATION_TEMPLATE_NOT_AVAILABLE");
    if (!senderResult.data || senderResult.data.status !== "VERIFIED")
      throw new Error("AUTOMATION_SENDER_NOT_VERIFIED");
    ...
    const idempotencyKey = createHash("sha256")
      .update(`${execution.id}:${node.id}:${execution.definition_version}`)
      .digest("hex");
    ...
    if (existing)
      return {
        nextNodeId: node.nextNodeId,
        safeResult: { alreadyDelivered: true },
      };
    ...
    const connection = await providerConnectionService.resolve(
      { tenantId: execution.tenant_id, marketCode: workspace.market_code },
      {
        capability: "email.marketing",
        explicitConnectionId: senderResult.data.provider_connection_id,
        feature: "marketing.journey_send",
      },
    );
    ...
    try {
      ...
      const result = await capabilityGateways.emailDelivery.send(
        {
        ...
        },
        {
          to: [profile.email],
          from: {
            email: senderResult.data.email,
            name: senderResult.data.display_name,
          },
          replyTo: senderResult.data.reply_to ?? undefined,
          subject: interpolate(templateVersion.subject, profile, links),
          textBody: body,
          purpose: "MARKETING",
          headers: {
      ...
      await providerConnectionService.recordUsage({
        tenantId: execution.tenant_id,
        connection,
        capability: "email.marketing",
        feature: "marketing.journey_send",
      ...
      await enqueueMarketingWebhookEvent(
        execution.tenant_id,
        "email.accepted",
        `journey-accepted:${connection.id}:${result.externalMessageId}`,
        {
          executionId: execution.id,
  ...
  private async createCrmTask(
    ...
    if (!profile.crm_contact_id)
      throw new Error("AUTOMATION_CRM_CONTACT_REQUIRED");
    const context = await repositories.crm.getTenantContext(
      execution.tenant_id,
    );
    if (!context) throw new Error("AUTOMATION_CRM_WORKSPACE_REQUIRED");
    const dueMinutes = configurationNumber(
    ...
    const task = await repositories.crm.createTask(context, {
      ownerId:
        typeof node.configuration.ownerId === "string"
          ? node.configuration.ownerId
          : undefined,
      ...
      priority: ["low", "medium", "high", "urgent"].includes(
        text(node.configuration.priority),
      )
        ? (node.configuration.priority as any)
        : "medium",
  ...
  private async recordCrmActivity(
    ...
    const activity = await repositories.crm.addActivity(context, {
      ...
      entityType: "contact",
      entityId: profile.crm_contact_id,
      activityType: "EXTERNAL_EVENT",
      title: text(
        node.configuration.title || "Activité du parcours marketing",
        255,
      ),
    ...
    return {
      nextNodeId: node.nextNodeId,
      safeResult: { activityId: activity.id },
    };
  }
  ...
  private async runAi(
    ...
    const { data: definition, error } = await this.client
      .from("automation_definitions")
      .select("created_by")
      .eq("tenant_id", execution.tenant_id)
      .eq("id", execution.definition_id)
      .single();
    if (error) throw error;
    const { data: workspace, error: workspaceError } = await this.client
      .from("marketing_workspaces")
      .select("market_code,default_locale")
      .eq("tenant_id", execution.tenant_id)
      .order("created_at")
      .limit(1)
      .single();
    ...
    const task: AiGenerationRequest["task"] = allowedTasks.includes(
    ...
    const connection = await providerConnectionService.resolve(
      ...
      { capability: "ai.marketing_drafting", feature: task },
    );
    const correlationId = randomUUID();
    const started = Date.now();
    try {
        ...
        },
        {
          ...
          instructions: text(
            node.configuration.instructions ||
              "Produce a concise advisory recommendation.",
            5_000,
          ),
          safeContext: execution.safe_context,
          outputSchema: { type: "object", additionalProperties: true },
          maxOutputTokens: configurationNumber(
            node.configuration.maxOutputTokens,
            1_000,
            100,
            2_000,
          ),
  ...
  private async countCampaignSends(
    tenantId: string,
    profileId: string,
    since: string,
  ) {
    const { count, error } = await this.client
      ...
      .in("send_status", ["ACCEPTED", "DELIVERED"]);
    if (error) throw error;
    return count ?? 0;
  }

  private async countAutomationSends(
    tenantId: string,
    profileId: string,
    since: string,
  ) {
  ...
  private async step(
    ...
    const { error } = await this.client
      ...
      .upsert(
        ...
        {
          onConflict: "execution_id,node_id,attempt,status",
