...
import type { SessionTokens, AuthRequestMetadata } from '../../modules/auth/session.service.js';
...
function cookie(name: string, value: string, options: { httpOnly?: boolean; maxAge?: number; path?: string } = {}): string {
...
export function setSessionCookies(res: ServerResponse, tokens: SessionTokens): string {
  const csrfToken = randomOAuthValue(24);
  appendCookie(res, cookie(ACCESS_COOKIE, tokens.token, { httpOnly: true, maxAge: config.authTokenTtlSeconds }));
  appendCookie(res, cookie(REFRESH_COOKIE, tokens.refreshToken, { httpOnly: true, maxAge: config.authRefreshTokenTtlSeconds, path: '/api/v1/auth' }));
  appendCookie(res, cookie(CSRF_COOKIE, csrfToken, { maxAge: config.authRefreshTokenTtlSeconds }));
  return csrfToken;
}

export function clearSessionCookies(res: ServerResponse): void {
  appendCookie(res, cookie(ACCESS_COOKIE, '', { httpOnly: true, maxAge: 0 }));
  appendCookie(res, cookie(REFRESH_COOKIE, '', { httpOnly: true, maxAge: 0, path: '/api/v1/auth' }));
  appendCookie(res, cookie(CSRF_COOKIE, '', { maxAge: 0 }));
  appendCookie(res, cookie(OAUTH_COMPLETION_COOKIE, '', { httpOnly: true, maxAge: 0, path: '/api/v1/auth/oauth' }));
}

export function setOAuthCompletionCookie(res: ServerResponse, handle: string): void {
  appendCookie(res, cookie(OAUTH_COMPLETION_COOKIE, handle, { httpOnly: true, maxAge: config.oauthFlowTtlSeconds, path: '/api/v1/auth/oauth' }));
}
...
export function requireCsrf(req: IncomingMessage): void {
  ...
  if (!expected || !supplied) throw new AppError({ code: 'FORBIDDEN', message: 'La requête de sécurité a expiré. Rechargez la page.' });
  const expectedBuffer = Buffer.from(expected);
  const suppliedBuffer = Buffer.from(supplied);
  if (expectedBuffer.length !== suppliedBuffer.length || !timingSafeEqual(expectedBuffer, suppliedBuffer)) {
    throw new AppError({ code: 'FORBIDDEN', message: 'La requête de sécurité a expiré. Rechargez la page.' });
  }
}
...
export function requestMetadata(req: IncomingMessage): AuthRequestMetadata {
  ...
  const rawIp = (forwarded || req.socket.remoteAddress || '').split(',')[0].trim();
    ...
    : /Android/i.test(userAgent)
      ...
      : /Windows/i.test(userAgent)
        ...
        : /Mac OS/i.test(userAgent)
          ? 'macOS'
          : 'Appareil';
  return { ipPrefix: ipPrefix || null, deviceLabel: `${family} sur ${os}`, userAgentFamily: family };
}
