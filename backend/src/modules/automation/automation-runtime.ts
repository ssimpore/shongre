...
export function validateAutomationGraph(
  ...
  if (!byId.has(graph.entryNodeId)) {
    issues.push({
      ...
      message: "Le nœud d’entrée est introuvable.",
    });
    return issues;
  }
  for (const node of graph.nodes) {
    for (const target of [node.nextNodeId, node.alternateNodeId].filter(
      Boolean,
    ) as string[]) {
      if (!byId.has(target)) {
        issues.push({
          code: "EDGE_NOT_FOUND",
          nodeId: node.id,
          message: `Le nœud ${node.id} référence ${target}, qui n’existe pas.`,
        });
      }
    }
  }
  ...
  const walk = (id: string, depth: number) => {
    if (depth > graph.maxExecutionDepth) {
      issues.push({
        ...
        nodeId: id,
        message: "Le parcours dépasse la profondeur maximale autorisée.",
      });
      return;
    }
    if (visiting.has(id)) {
      issues.push({
        code: "UNBOUNDED_CYCLE",
        nodeId: id,
        message: "Les boucles de parcours ne sont pas autorisées.",
      });
      return;
    }
  ...
  if (!endReachable) {
    issues.push({
      ...
      message:
        "Au moins une fin de parcours doit être accessible depuis le déclencheur.",
    });
  }
  return issues;
}
