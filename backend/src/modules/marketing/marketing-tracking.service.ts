...
export class MarketingTrackingService {
  ...
  async record(token: string, kind: "OPEN" | "CLICK") {
    ...
    const { data: tracking, error } = await this.client
    ...
    const { data: recipient, error: recipientError } = await this.client
      ...
      .select(
        "id,tenant_id,profile_id,campaign_id,provider_connection_id,provider_message_id",
      )
      .eq("id", tracking.recipient_id)
      .eq("tenant_id", tracking.tenant_id)
      .maybeSingle();
    if (recipientError) throw recipientError;
    if (!recipient?.provider_connection_id || !recipient.provider_message_id)
      throw new AppError({
        code: "NOT_FOUND",
        message: "Lien de suivi introuvable.",
      });
      ...
      .upsert(
        {
          ...
          provider_connection_id: recipient.provider_connection_id,
          provider_event_id: `tracking:${eventType}:${tokenHash}`,
          provider_message_id: recipient.provider_message_id,
          event_type: eventType,
          occurred_at: occurredAt,
          safe_metadata: { source: "FIRST_PARTY_TRACKING" },
        },
        {
    ...
    if (kind === "CLICK") {
      try {
        await emitMarketingJourneyEvent(recipient.tenant_id, {
          type: "CAMPAIGN_CLICKED",
          eventId: `campaign.clicked:${tokenHash}`,
          profileId: recipient.profile_id,
          safeContext: { campaignId: recipient.campaign_id },
        });
      } catch (error) {
        logger.error("marketing_click_journey_event_failed", {
          recipientId: recipient.id,
