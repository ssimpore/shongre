...
function entitlementError(entitlement: string): never {
  throw new AppError({
    code: "FORBIDDEN",
    message:
      "Cette fonctionnalité n’est pas incluse dans les droits Marketing actifs.",
    details: { entitlement },
  });
}
...
function safeAiContext(input: Record<string, unknown>) {
  const forbidden =
    /(email|recipient|subscriber|contact|phone|address|secret|credential|token)/i;
  const visit = (value: unknown, depth: number): unknown => {
    if (depth > 4) return undefined;
    if (Array.isArray(value))
      return value.slice(0, 50).map((item) => visit(item, depth + 1));
    if (!value || typeof value !== "object")
      return typeof value === "string" ? value.slice(0, 5_000) : value;
...
export class MarketingOperationsService {
  constructor(
    ...
    private readonly repository: IMarketingOperationsRepository = config.dataMode ===
    "database"
  ...
  private async context(principal: Principal) {
    const existing = await this.marketingRepository.resolveTenantContext(
      principal.userId,
    );
    if (existing) return existing;
    const tenantId = await this.marketingRepository.resolveTenantId(
      principal.userId,
    );
    if (!tenantId)
      throw new AppError({
  ...
  private async requireEntitlement(
    principal: Principal,
    key: "automation" | "advancedAnalytics" | "webhooks" | "ai",
  ) {
    ...
    if (!usage.entitlements.enabled || !usage.entitlements[key])
      entitlementError(`marketing.${key}`);
  ...
  async createJourney(principal: Principal, input: unknown) {
    ...
    try {
      assertAutomationGraph(parsed.definition);
    } catch (error: any) {
      throw new AppError({
        code: "VALIDATION_ERROR",
        message: "Le parcours contient une boucle ou une référence invalide.",
        details: { issues: error?.issues ?? [] },
      });
    }
    const journey = await this.repository.createJourney(
      context,
      parsed,
      principal.userId,
    );
    await this.marketingRepository.addAudit(
      context.tenantId,
      principal.userId,
      "marketing.journey.created",
  ...
  async setJourneyStatus(
    ...
    const { context } = await this.requireEntitlement(principal, "automation");
    const journey =
      (await this.repository.getJourney(context.tenantId, id)) ??
      notFound("Parcours marketing introuvable.");
    if (status === "ACTIVE") {
      try {
        assertAutomationGraph(journey.definition);
      } catch (error: any) {
        throw new AppError({
          code: "VALIDATION_ERROR",
          message: "Le parcours ne peut pas être activé.",
          details: { issues: error?.issues ?? [] },
        });
      }
    }
    const updated = await this.repository.setJourneyStatus(
      context.tenantId,
      id,
      status,
    );
    await this.marketingRepository.addAudit(
      context.tenantId,
      principal.userId,
      `marketing.journey.${status.toLowerCase()}`,
      "automation_definition",
      id,
      ["status"],
    );
  ...
  async emitJourneyEvent(principal: Principal, input: unknown) {
    requirePermission(principal, "marketing.automation.manage");
    const { context, usage } = await this.requireEntitlement(
      principal,
      "automation",
    );
    const parsed = marketingJourneyEventSchema.parse(input);
    if (usage.automationExecutions >= usage.entitlements.maxMonthlySends * 10)
      entitlementError("marketing.automation_execution_limit");
    const executions = await this.repository.enqueueJourneyEvent(
      context.tenantId,
      {
        ...parsed,
  ...
  async listJourneyExecutions(principal: Principal, journeyId?: string) {
    ...
    const { context } = await this.requireEntitlement(principal, "automation");
    return {
      items: await this.repository.listJourneyExecutions(
        context.tenantId,
        journeyId,
      ),
    };
  ...
  async analytics(principal: Principal, campaignId?: string) {
    ...
    const { context } = await this.requireEntitlement(
      principal,
      "advancedAnalytics",
    );
  ...
  async recordConversion(principal: Principal, input: unknown) {
    ...
    if (config.dataMode === "database") {
      await enqueueMarketingWebhookEvent(
        context.tenantId,
        "conversion.recorded",
        `conversion:${parsed.idempotencyKey}`,
        {
  ...
  async listWebhookSubscriptions(principal: Principal) {
    ...
    return {
      items: await this.repository.listWebhookSubscriptions(context.tenantId),
    };
  }
  ...
  async createWebhookSubscription(principal: Principal, input: unknown) {
    ...
    if (config.dataMode === "database" && key.length !== 32)
      throw new AppError({
        code: "NETWORK_ERROR",
        statusCode: 503,
        message: "Le coffre de secrets webhook est mal configuré.",
      });
    const envelope =
      config.dataMode === "database"
        ...
        : {
            encryptedSecret: Buffer.alloc(32),
            iv: Buffer.alloc(12),
            authTag: Buffer.alloc(16),
            keyVersion: "demo-v1",
            credentialHint: `••••${signingSecret.slice(-4)}`,
          };
    const subscription = await this.repository.createWebhookSubscription(
      context,
      parsed,
      { ...envelope, hint: envelope.credentialHint },
    );
    await this.marketingRepository.addAudit(
      context.tenantId,
      principal.userId,
      "marketing.webhook.created",
  ...
  async aiAssist(principal: Principal, input: unknown) {
    ...
    const connection = await providerConnectionService.resolve(
      ...
      { capability: "ai.marketing_drafting", feature },
    );
