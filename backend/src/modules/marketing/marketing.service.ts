...
const accountPreferencesSchema = accountSubscriptionSchema.pick({
  marketCode: true,
  topics: true,
});
...
function compare(
  actual: unknown,
  condition: MarketingSegmentCondition,
): boolean {
...
function matchesGroup(
  ...
  group: MarketingSegmentGroup,
): boolean {
  const values = [
    ...group.conditions.map((condition) =>
      compare(getValue(profile, condition.field), condition),
    ),
...
function validateSegmentGroup(group: MarketingSegmentGroup): void {
  group.conditions.forEach((condition) =>
    getValue({ customValues: {} } as MarketingProfile, condition.field),
  );
...
function renderText(
  ...
  const replace = (value: string) =>
    value.replace(
      ...
      (_match, key, fallback) =>
        String(personalization[key] ?? fallback?.trim() ?? ""),
    );
  const text =
    campaign.content.plainText ??
    campaign.content.blocks
      .map((block) => {
        ...
        if (block.type === "UNSUBSCRIBE")
          return block.text ?? "Se désabonner : {{ unsubscribe_url }}";
        if (block.type === "PREFERENCE_CENTER")
          return block.text ?? "Gérer mes préférences : {{ preferences_url }}";
...
function stableSendKey(
  ...
  return createHash("sha256")
    .update(
      `${campaign.tenantId}:${campaign.id}:${campaign.currentVersion}:${profileId}:${variant}`,
    )
    .digest("hex");
}
...
function selectVariant(campaign: MarketingCampaign, profileId: string) {
  ...
  const bucket =
    Number.parseInt(
      createHash("sha256")
        .update(`${campaign.id}:${campaign.currentVersion}:${profileId}`)
        .digest("hex")
        .slice(0, 8),
      16,
    ) % 100;
...
export class MarketingService {
  constructor(
    ...
    private readonly operationsRepository: IMarketingOperationsRepository = config.dataMode ===
    "database"
      ? new PostgresMarketingOperationsRepository()
      : new DemoMarketingOperationsRepository(),
  ) {}

  private async context(principal: Principal): Promise<MarketingTenantContext> {
    requireAuthenticated(principal);
    const existing = await this.repository.resolveTenantContext(
      principal.userId,
    );
    if (existing) return existing;
    const tenantMembership = await this.repository.resolveTenantId(
      principal.userId,
    );
    if (!tenantMembership) {
      throw new AppError({
  ...
  private async publicContext(marketCode: string) {
    ...
    if (!context) {
      throw new AppError({
        code: "NOT_FOUND",
        message: "Le programme newsletter n’est pas disponible pour ce marché.",
      });
    }
    return context;
  }
  ...
  private async usage(context: MarketingTenantContext) {
    return this.operationsRepository.usage(
      context.tenantId,
      context.marketCode,
    );
  }
  ...
  private requireMarketingEntitlement(enabled: boolean, entitlement: string) {
    if (!enabled)
      throw new AppError({
        code: "FORBIDDEN",
        message:
          "Cette capacité Marketing n’est pas incluse dans les droits actifs.",
        details: { entitlement },
      });
  }

  private async emitJourneyEvent(
    context: MarketingTenantContext,
    type: "SUBSCRIBER_CREATED" | "SUBSCRIBER_CONFIRMED" | "LIST_JOINED",
    eventId: string,
    profileId: string,
    safeContext: Record<string, unknown> = {},
  ) {
    await this.operationsRepository.enqueueJourneyEvent(context.tenantId, {
      type,
      eventId,
      profileId,
      safeContext,
  ...
  private async actionProfile(
    rawToken: string,
    purpose: "CONFIRM" | "PREFERENCES" | "UNSUBSCRIBE",
    consume: boolean,
  ) {
    const token = await this.repository.getActionToken(
      tokenHash(rawToken),
      purpose,
    );
    if (
  ...
  private async createPublicToken(
    ...
    await this.repository.createActionToken(
      ...
      purpose,
      tokenHash(raw),
      new Date(Date.now() + lifetimeDays * 86_400_000).toISOString(),
    );
    return raw;
  }

  private async sendDoubleOptIn(
    context: MarketingTenantContext,
    profile: MarketingProfile,
    locale: string,
  ) {
    const rawToken = await this.createPublicToken(
      context.tenantId,
      profile.id,
      "CONFIRM",
      2,
    ...
    const sender = (
      await this.repository.listSenderIdentities(context.tenantId)
    ...
    try {
      const connection = await providerConnectionService.resolve(
        { tenantId: context.tenantId, marketCode: context.marketCode },
        {
          capability: "email.marketing",
          explicitConnectionId: context.defaultProviderConnectionId,
          feature: "marketing.double_opt_in",
        },
      );
      const confirmationUrl = `${config.frontendUrl.replace(/\/$/, "")}/newsletter/confirmer?token=${encodeURIComponent(rawToken)}`;
      await capabilityGateways.emailDelivery.send(
        {
        ...
        },
        {
          ...
          subject: "Confirmez votre inscription à la newsletter Shongre",
          textBody: `Confirmez votre inscription : ${confirmationUrl}`,
          purpose: "TRANSACTIONAL",
          headers: { "X-Shongre-Consent-Version": MARKETING_CONSENT_VERSION },
          idempotencyKey: createHash("sha256")
            .update(
              `marketing-confirm:${profile.id}:${MARKETING_CONSENT_VERSION}`,
            )
            .digest("hex"),
        },
      );
    } catch (error) {
      logger.error("Marketing double opt-in delivery failed", {
  ...
  async subscribePublic(input: unknown) {
    ...
    const existing = await this.repository.findProfileByEmail(
      context.tenantId,
      email,
    );
    if (
    ...
    const profile = await this.repository.saveProfile(context, {
      email,
      locale: parsed.locale,
      country: parsed.marketCode,
    ...
    if (!existing)
      await this.emitJourneyEvent(
        ...
        profile.id,
        { source: parsed.source, marketCode: parsed.marketCode },
      );
    await this.repository.appendConsent(context.tenantId, {
      subjectId: profile.id,
      normalizedEmail: profile.normalizedEmail,
      status: "GRANTED",
      source: parsed.source,
      consentVersion: MARKETING_CONSENT_VERSION,
      ...(context.doubleOptIn ? {} : { confirmedAt: new Date().toISOString() }),
    });
    if (!context.doubleOptIn) {
      await this.emitJourneyEvent(
        context,
        "SUBSCRIBER_CONFIRMED",
        `profile.confirmed:${profile.id}`,
        profile.id,
        { source: parsed.source, marketCode: parsed.marketCode },
      );
      return {
  ...
  async confirmPublic(input: unknown) {
    ...
    const { token, profile } = await this.actionProfile(
      parsed.token,
      "CONFIRM",
      true,
    );
    if (
      (await this.repository.getActiveSuppressedEmails(token.tenantId)).has(
        profile.normalizedEmail,
      )
    ) {
      throw new AppError({
        code: "CONFLICT",
        message: "Cette adresse ne peut pas être réactivée automatiquement.",
      });
    }
    const updated = await this.repository.setProfileStatus(
      token.tenantId,
      profile.id,
      "SUBSCRIBED",
    );
    await this.repository.appendConsent(token.tenantId, {
    ...
    if (context)
      await this.emitJourneyEvent(
        context,
        "SUBSCRIBER_CONFIRMED",
        `profile.confirmed:${updated.id}`,
        updated.id,
        { source: "DOUBLE_OPT_IN", marketCode: updated.country },
      );
  ...
  async getPublicPreferences(rawToken: string) {
    ...
    const { profile } = await this.actionProfile(
      parsed.token,
      "PREFERENCES",
      false,
    );
  ...
  async updatePublicPreferences(input: unknown) {
    ...
    return subscriptionView(
      await this.repository.updateProfileTopics(
        token.tenantId,
        profile.id,
        parsed.topics,
      ),
  ...
  async unsubscribePublic(input: unknown) {
    ...
    const updated =
      profile.status === "UNSUBSCRIBED"
        ? profile
        : await this.repository.setProfileStatus(
            token.tenantId,
            profile.id,
            "UNSUBSCRIBED",
          );
    ...
    if (profile.status !== "UNSUBSCRIBED")
      await this.repository.appendConsent(token.tenantId, {
        subjectId: updated.id,
        normalizedEmail: updated.normalizedEmail,
        status: "WITHDRAWN",
  ...
  async getAccountSubscription(principal: Principal, marketCodeInput: unknown) {
    ...
    const marketCode = z
      ...
      .regex(/^[A-Z]{2}$/)
      .default("FR")
      .parse(marketCodeInput);
    const context = await this.publicContext(marketCode);
    const profile =
      (await this.repository.findProfileByAccountUserId(
        context.tenantId,
        principal.userId,
      )) ??
  ...
  async subscribeAccount(principal: Principal, input: unknown) {
    ...
    if (
      ...
      existing.accountUserId !== principal.userId
    ) {
      throw new AppError({
        code: "CONFLICT",
        message: "Cette adresse est déjà liée à un autre profil marketing.",
      });
    }
    if (
      existing &&
      ["BOUNCED", "COMPLAINED", "SUPPRESSED", "INVALID"].includes(
        existing.status,
      )
    ) {
      throw new AppError({
        code: "CONFLICT",
        message: "Ce profil ne peut pas être réactivé automatiquement.",
      });
    }
    if (existing?.status === "UNSUBSCRIBED") {
      await this.repository.releaseSuppression(
        context.tenantId,
    ...
    const saved = await this.repository.saveProfile(context, {
    ...
    if (!existing)
      await this.emitJourneyEvent(
        context,
        "SUBSCRIBER_CREATED",
        `profile.created:${saved.id}`,
        saved.id,
        { source: "ACCOUNT", marketCode: parsed.marketCode },
      );
    const updated = await this.repository.setProfileStatus(
      context.tenantId,
      saved.id,
      context.doubleOptIn ? "PENDING" : "SUBSCRIBED",
    );
    await this.repository.appendConsent(context.tenantId, {
      subjectId: updated.id,
      normalizedEmail: updated.normalizedEmail,
    ...
    if (context.doubleOptIn)
      await this.sendDoubleOptIn(context, updated, parsed.locale);
  ...
  async createProfile(principal: Principal, input: unknown) {
    ...
    this.requireMarketingEntitlement(
      usage.entitlements.enabled,
      "marketing.enabled",
    );
    if (!existing && usage.activeProfiles >= usage.entitlements.maxContacts)
      this.requireMarketingEntitlement(false, "marketing.max_contacts");
    if (
      existing &&
      ...
      throw new AppError({
        code: "CONFLICT",
        message:
          "Ce profil existe déjà dans un état qui interdit sa réactivation automatique.",
    ...
    await this.repository.addAudit(
      context.tenantId,
      principal.userId,
      existing ? "marketing.profile.updated" : "marketing.profile.created",
      "profile",
      profile.id,
      Object.keys(parsed),
    );
  ...
  async confirmProfile(principal: Principal, id: string) {
    ...
    if (
      (await this.repository.getActiveSuppressedEmails(context.tenantId)).has(
        profile.normalizedEmail,
      )
    ) {
      throw new AppError({
        code: "CONFLICT",
        message:
          "Une suppression active interdit la confirmation de ce profil.",
      });
    }
    const updated = await this.repository.setProfileStatus(
      context.tenantId,
  ...
  async unsubscribeProfile(principal: Principal, id: string) {
    ...
    await this.repository.addAudit(
      context.tenantId,
      principal.userId,
      "marketing.profile.unsubscribed",
      "profile",
      id,
  ...
  async createList(principal: Principal, input: unknown) {
    ...
    const context = await this.context(principal);
    const [usage, existingLists] = await Promise.all([
      this.usage(context),
      this.repository.listLists(context.tenantId),
    ]);
    this.requireMarketingEntitlement(
      usage.entitlements.enabled,
      "marketing.enabled",
    );
    if (existingLists.length >= usage.entitlements.maxLists)
      this.requireMarketingEntitlement(false, "marketing.max_lists");
    const value = await this.repository.createList(
      context,
      marketingListInputSchema.parse(input),
    );
    await this.repository.addAudit(
      context.tenantId,
      principal.userId,
  ...
  async addListMember(principal: Principal, listId: string, profileId: string) {
    ...
    if (!profile) return notFound("Profil marketing introuvable.");
    await this.repository.addListMember(
      context.tenantId,
      listId,
      profileId,
      "MANUAL",
  ...
  async createSegment(principal: Principal, input: unknown) {
    ...
    const context = await this.context(principal);
    const [usage, existingSegments] = await Promise.all([
      this.usage(context),
    ...
    if (existingSegments.length >= usage.entitlements.maxSegments)
      this.requireMarketingEntitlement(false, "marketing.max_segments");
    ...
    const count = (
      await this.repository.listAllProfiles(context.tenantId)
    ).filter((profile) => matchesGroup(profile, parsed.definition)).length;
    await this.repository.updateSegmentEstimate(
      context.tenantId,
      value.id,
      count,
    );
  ...
  async createTemplate(principal: Principal, input: unknown) {
    ...
    const usage = await this.usage(context);
    this.requireMarketingEntitlement(
      usage.entitlements.enabled && usage.entitlements.templates,
      "marketing.templates",
    );
    const value = await this.repository.createTemplate(
      context,
      marketingTemplateInputSchema.parse(input),
      principal.userId,
    );
    await this.repository.addAudit(
      context.tenantId,
  ...
  async getCampaign(principal: Principal, id: string) {
    ...
    const context = await this.context(principal);
    return (
      (await this.repository.getCampaign(context.tenantId, id)) ??
      notFound("Campagne marketing introuvable.")
    );
  }
  async createCampaign(principal: Principal, input: unknown) {
    ...
    if (parsed.experiment?.enabled)
      this.requireMarketingEntitlement(
        usage.entitlements.abTesting,
        "marketing.ab_testing",
      );
    const value = await this.repository.createCampaign(
      context,
      parsed,
      principal.userId,
    );
  ...
  private async selectedProfiles(
    context: MarketingTenantContext,
    audienceInput: MarketingAudienceDefinition,
  ) {
    ...
    const [
      ...
      this.repository.listSegments(context.tenantId),
      this.repository.getListMemberIds(
        context.tenantId,
        audience.includeListIds,
      ),
      this.repository.getListMemberIds(
        context.tenantId,
        audience.excludeListIds,
      ),
      this.repository.getActiveSuppressedEmails(context.tenantId),
      this.repository.getRecipientCounts(
        context.tenantId,
    ...
    const includeIds = new Set([
      ...audience.includeProfileIds,
      ...includeListIds,
    ]);
    const excludeIds = new Set([
      ...audience.excludeProfileIds,
      ...excludeListIds,
    ]);
    for (const segment of segments) {
      if (audience.includeSegmentIds.includes(segment.id))
        profiles
          .filter((profile) => matchesGroup(profile, segment.definition))
          .forEach((profile) => includeIds.add(profile.id));
      if (audience.excludeSegmentIds.includes(segment.id))
        profiles
          .filter((profile) => matchesGroup(profile, segment.definition))
          .forEach((profile) => excludeIds.add(profile.id));
    }
    const recent = audience.recentRecipientDays
      ? await this.repository.getRecentRecipientProfileIds(
          context.tenantId,
          new Date(
            Date.now() - audience.recentRecipientDays * 86_400_000,
          ).toISOString(),
        )
      : new Set<string>();
    ...
    const selected = [...includeIds]
      ...
      .filter((profile): profile is MarketingProfile => Boolean(profile));
    const classified = selected.map((profile) => {
      let exclusionReason: string | undefined;
      if (excludeIds.has(profile.id)) exclusionReason = "CAMPAIGN_EXCLUSION";
      else if (profile.status === "UNSUBSCRIBED")
        exclusionReason = "UNSUBSCRIBED";
      else if (
        ["SUPPRESSED", "BOUNCED", "COMPLAINED"].includes(profile.status) ||
        suppressedEmails.has(profile.normalizedEmail)
      )
        exclusionReason = "SUPPRESSED";
      else if (profile.status === "INVALID" || profile.status === "PENDING")
        exclusionReason = "INVALID";
      else if (profile.customValues.doNotContact === true)
        exclusionReason = "DO_NOT_CONTACT";
      else if (
        (dayCounts.get(profile.id) ?? 0) >= context.frequencyCapDay ||
        (weekCounts.get(profile.id) ?? 0) >= context.frequencyCapWeek
      )
        exclusionReason = "FREQUENCY_CAP";
  ...
  async estimateAudience(
    ...
    requirePermission(principal, "marketing.campaigns.read");
    const context = await this.context(principal);
    const classified = await this.selectedProfiles(
      context,
      marketingAudienceDefinitionSchema.parse(input),
    );
    const count = (reason: string) =>
      classified.filter((entry) => entry.exclusionReason === reason).length;
    const excluded = classified.filter((entry) =>
      Boolean(entry.exclusionReason),
    ).length;
    return {
      selected: classified.length,
      eligible: classified.length - excluded,
      excluded,
      ...
      frequencyCapped: count("FREQUENCY_CAP"),
  ...
  async preflight(
    ...
    const connectionId =
      campaign.providerConnectionId ?? context.defaultProviderConnectionId;
    if (!connectionId)
      blockers.push({
        code: "PROVIDER_REQUIRED",
        message:
          "Configurez un fournisseur Email Delivery pour envoyer cette campagne.",
        actionHref: "/admin/fournisseurs",
      });
    else {
      try {
        const connection = await providerConnectionService.resolve(
          ...
          },
          {
            ...
            feature: "marketing.campaign_send",
          },
        );
        const usage = await this.operationsRepository.usage(
          context.tenantId,
          context.marketCode,
        );
        if (
          connection.ownerType === "PLATFORM" &&
          !usage.entitlements.platformEmail
        )
          blockers.push({
            code: "PLATFORM_EMAIL_NOT_ENTITLED",
            message:
              "Les crédits Email Shongre ne sont pas inclus dans les droits actifs.",
          });
        if (connection.ownerType !== "PLATFORM" && !usage.entitlements.byoEmail)
          blockers.push({
            code: "BYO_EMAIL_NOT_ENTITLED",
            message:
              "Les fournisseurs Email appartenant au tenant ne sont pas inclus dans les droits actifs.",
          });
        const health = await capabilityGateways.emailDelivery.testConnection({
          tenantId: context.tenantId,
          userId: principal.userId,
          connectionId: connection.id,
          providerId: connection.providerId,
          capability: "email.marketing",
          feature: "marketing.campaign_preflight",
          correlationId: randomUUID(),
          marketCode: context.marketCode,
        ...
        if (health.status !== "HEALTHY")
          blockers.push({
            ...
            message:
              health.message ?? "Le fournisseur d’envoi n’est pas disponible.",
          });
      } catch {
        blockers.push({
          code: "PROVIDER_UNAVAILABLE",
          message:
            "La connexion Email Delivery sélectionnée n’est pas utilisable.",
          actionHref: "/admin/fournisseurs",
        });
      }
    }
    const senders = await this.repository.listSenderIdentities(
      context.tenantId,
    );
    const sender = senders.find(
      (item) => item.id === campaign.senderIdentityId,
    );
    ...
    if (!campaign.subject.trim())
      blockers.push({
        ...
        message: "Ajoutez un objet à la campagne.",
        field: "subject",
      });
    if (!campaign.content.blocks.length)
      blockers.push({
    ...
    if (!campaign.content.blocks.some((block) => block.type === "UNSUBSCRIBE"))
      blockers.push({
        ...
        message: "Le contenu marketing doit inclure un lien de désabonnement.",
        field: "content",
      });
    if (!renderText(campaign).trim())
      blockers.push({
        code: "PLAIN_TEXT_REQUIRED",
        message: "Une version texte lisible doit pouvoir être générée.",
        field: "content",
      });
    if (!audience.selected)
      blockers.push({
        code: "AUDIENCE_REQUIRED",
        message: "Sélectionnez au moins une liste, un segment ou un profil.",
      });
    else if (!audience.eligible)
      blockers.push({
        code: "AUDIENCE_INELIGIBLE",
        message:
          "Aucun destinataire sélectionné n’est éligible après consentement et suppression.",
      });
    if (audience.excluded)
      warnings.push({
        code: "AUDIENCE_EXCLUSIONS",
        message: `${audience.excluded} destinataire(s) seront exclus par les règles obligatoires.`,
      });
    const usage = await this.operationsRepository.usage(
      context.tenantId,
      context.marketCode,
    );
    if (!usage.entitlements.enabled)
      blockers.push({
        code: "MARKETING_DISABLED",
        message: "La capacité Marketing n’est pas active pour ce tenant.",
      });
    if (
      usage.attemptedSends + audience.eligible >
      usage.entitlements.maxMonthlySends
    )
      blockers.push({
        code: "MONTHLY_SEND_LIMIT",
        message: "Cet envoi dépasserait le quota mensuel autorisé.",
      });
    if (campaign.experiment?.enabled && !usage.entitlements.abTesting)
      blockers.push({
        code: "AB_TESTING_NOT_ENTITLED",
        message: "Les tests A/B ne sont pas inclus dans les droits actifs.",
      });
    if (context.approvalRequired && campaign.status !== "APPROVED")
      blockers.push({
        code: "APPROVAL_REQUIRED",
        message:
          "Cette campagne doit être approuvée avant programmation ou envoi.",
      });
    if (!["DRAFT", "APPROVED"].includes(campaign.status))
      blockers.push({
        code: "CAMPAIGN_ALREADY_DISPATCHED",
        message: "Cette campagne a déjà été soumise, programmée ou envoyée.",
      });
    info.push({
      code: "IDEMPOTENCY_ENABLED",
      message: "Chaque destinataire utilise une clé d’envoi déterministe.",
    });
    return {
    ...
    const connection = await providerConnectionService.resolve(
      ...
      },
      {
        ...
        explicitConnectionId:
          campaign.providerConnectionId ?? context.defaultProviderConnectionId,
        feature: "marketing.test_send",
      },
    );
    const sender = (
      await this.repository.listSenderIdentities(context.tenantId)
    ).find((item) => item.id === campaign.senderIdentityId);
    if (!sender || sender.status !== "VERIFIED")
      throw new AppError({
        code: "VALIDATION_ERROR",
        message: "Une identité d’expéditeur vérifiée est requise.",
      });
    return capabilityGateways.emailDelivery.send(
      {
        tenantId: context.tenantId,
        userId: principal.userId,
        connectionId: connection.id,
      ...
      },
      {
        ...
        textBody: renderText(campaign, {
          first_name: "Test",
          ...parsed.personalization,
          unsubscribe_url: "https://demo.shongre.local/preferences",
          preferences_url: "https://demo.shongre.local/preferences",
        }),
        purpose: "MARKETING",
        headers: { "X-Shongre-Test-Send": "true" },
        idempotencyKey: createHash("sha256")
          .update(
            `${campaign.id}:${parsed.recipient}:${campaign.currentVersion}`,
          )
          .digest("hex"),
      },
    );
  ...
  async send(principal: Principal, id: string) {
    ...
    if (!preflight.canSend)
      throw new AppError({
        code: "VALIDATION_ERROR",
        message: "Le pré-vol contient des blocages.",
        details: preflight,
      });
    ...
    const recipients = classified.map(({ profile, exclusionReason }) => {
      const variantId = selectVariant(campaign, profile.id);
      return {
        profileId: profile.id,
        variantId,
        idempotencyKey: stableSendKey(campaign, profile.id, variantId),
        eligibilityStatus: exclusionReason
          ? ("EXCLUDED" as const)
          : ("ELIGIBLE" as const),
        exclusionReason,
      };
    });
    await this.repository.createAudienceSnapshot(
      context.tenantId,
      campaign,
      recipients,
    );
    await this.repository.enqueueCampaign(
      context.tenantId,
      campaign.id,
      `campaign-send:${campaign.id}:${campaign.currentVersion}`,
    );
    const updated = await this.repository.setCampaignStatus(
      context.tenantId,
      id,
      "QUEUED",
    ...
    return {
      campaign: updated,
      queuedRecipients: preflight.audience.eligible,
      excludedRecipients: preflight.audience.excluded,
    };
  }
  ...
  async schedule(principal: Principal, id: string, input: unknown) {
    ...
    if (new Date(parsed.scheduledAt).getTime() <= Date.now())
      throw new AppError({
        code: "VALIDATION_ERROR",
        message: "La date de programmation doit être future.",
      });
    const preflight = await this.preflight(principal, id);
    if (!preflight.canSend)
      throw new AppError({
  ...
  async pause(principal: Principal, id: string) {
    ...
    if (!["QUEUED", "SENDING"].includes(campaign.status))
      throw new AppError({
        code: "CONFLICT",
        message: "Seule une campagne en file ou en cours peut être suspendue.",
      });
  ...
  async resume(principal: Principal, id: string) {
    ...
    if (campaign.status !== "PAUSED")
      throw new AppError({
        code: "CONFLICT",
        message: "Seule une campagne suspendue peut être reprise.",
      });
  ...
  async submitForReview(principal: Principal, id: string) {
    ...
    if (campaign.status !== "DRAFT")
      throw new AppError({
        code: "CONFLICT",
        message: "Seul un brouillon peut être soumis à validation.",
      });
    const updated = await this.repository.setCampaignApproval(
      context.tenantId,
      id,
      "REVIEW",
    );
    await this.repository.addAudit(
      context.tenantId,
      principal.userId,
      "marketing.campaign.review_requested",
      "campaign",
      id,
      ["status"],
    );
  ...
  async approve(principal: Principal, id: string) {
    ...
    if (campaign.status !== "REVIEW")
      throw new AppError({
        code: "CONFLICT",
        message: "Seule une campagne en validation peut être approuvée.",
      });
    const updated = await this.repository.setCampaignApproval(
      context.tenantId,
      id,
  ...
  async selectExperimentWinner(
    ...
    if (!experiment?.enabled)
      throw new AppError({
        code: "CONFLICT",
        message: "Cette campagne ne contient pas de test A/B.",
      });
    if (campaign.winningVariantId) return campaign;
    const parsed = z
      .object({ variantId: z.string().max(80).optional() })
      .parse(input ?? {});
    ...
    if (variantId) {
      if (!experiment.variants.some((variant) => variant.id === variantId))
        throw new AppError({
          code: "VALIDATION_ERROR",
          message:
            "La variante sélectionnée n’appartient pas à cette campagne.",
        });
    } else {
      if (experiment.winnerMode !== "AUTOMATIC")
        throw new AppError({
          code: "VALIDATION_ERROR",
          message: "Une variante doit être sélectionnée pour ce test manuel.",
        });
      if (
        !campaign.startedAt ||
        Date.now() <
          new Date(campaign.startedAt).getTime() +
            experiment.durationMinutes * 60_000
      )
        throw new AppError({
          code: "CONFLICT",
          message: "La fenêtre de mesure du test A/B n’est pas terminée.",
        });
      const analytics = await this.operationsRepository.analytics(
        context.tenantId,
        id,
      );
      const score = (variant: (typeof analytics.variants)[number]) =>
        experiment.winnerMetric === "CONVERSION_RATE"
          ? variant.conversionRate
          : experiment.winnerMetric === "OPEN_RATE"
            ? variant.delivered
              ? variant.uniqueOpens / variant.delivered
              : 0
            : variant.clickThroughRate;
      variantId =
        [...analytics.variants].sort(
          (left, right) =>
            score(right) - score(left) || right.delivered - left.delivered,
        )[0]?.variantId ?? experiment.variants[0].id;
    }
    const updated = await this.repository.setCampaignWinner(
      context.tenantId,
      id,
      variantId,
    );
    await this.repository.addAudit(
      context.tenantId,
      principal.userId,
      "marketing.campaign.experiment_winner_selected",
      "campaign",
      id,
      ["winningVariantId"],
    );
  ...
  async generateCampaignDraft(principal: Principal, input: unknown) {
    ...
    const parsed = z
      .object({
        ...
        locale: z.string().min(2).max(16).default(context.locale),
      })
      .parse(input);
    const connection = await providerConnectionService.resolve(
      {
      ...
      {
        capability: "ai.marketing_drafting",
        feature: "marketing.campaign_draft",
      },
    );
    return capabilityGateways.ai.generate(
      {
        tenantId: context.tenantId,
        userId: principal.userId,
        connectionId: connection.id,
      ...
      {
        task: "marketing.campaign_draft",
        instructions: parsed.instructions,
        safeContext: { locale: parsed.locale },
        outputSchema: {
          type: "object",
          required: ["subject", "previewText", "contentBlocks"],
        },
