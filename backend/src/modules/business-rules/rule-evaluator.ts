...
function asArray(value: CommercialCondition['value']): Array<string | number | boolean> {
...
export function conditionMatches(
  ...
  switch (condition.operator) {
    ...
    case 'not_in':
      return !expected.includes(actual as string | number | boolean);
    case 'gte':
      return typeof actual === 'number' && Number(actual) >= Number(expected[0]);
    case 'lte':
      return typeof actual === 'number' && Number(actual) <= Number(expected[0]);
    case 'between':
      return (
...
export function ruleScopeMatches(rule: CommercialRule, context: RuleEvaluationContext): boolean {
  const { scope } = rule;
  return (
...
function isEffective(rule: CommercialRule, effectiveAt: Date): boolean {
  ...
  if (rule.effectiveFrom && effectiveAt < new Date(rule.effectiveFrom)) return false;
  if (rule.effectiveUntil && effectiveAt >= new Date(rule.effectiveUntil)) return false;
  return true;
}

function explain(rule: CommercialRule, matched: boolean, reasonCode: string): RuleExplanation {
  return {
    ruleId: rule.id,
    ruleKey: rule.key,
    ruleName: rule.name,
...
function outcomeEntries(outcome: CommercialRuleOutcome): Array<[string, string | number | boolean]> {
...
export function evaluateCommercialRules(input: {
  ...
  const candidates = [...input.rules].sort((a, b) =>
  ...
  for (const rule of candidates) {
    ...
    explanation.push(explain(rule, matched, matched ? 'MATCHED' : 'CONDITION_NOT_MATCHED'));
    if (!matched) continue;
  ...
  const quotaRemaining = quotaLimit === undefined ? undefined : Math.max(0, quotaLimit - usage);
  const eligible =
    outcomes.eligible !== false && (quotaRemaining === undefined || quotaRemaining > 0);
  ...
  return {
    ...
    reasonCode:
      !eligible && quotaRemaining === 0
        ? 'QUOTA_EXHAUSTED'
        : String(outcomes.reasonCode || (eligible ? 'ELIGIBLE' : 'NOT_ELIGIBLE')),
    quotaLimit,
    quotaRemaining,
    durationDays:
      typeof outcomes.durationDays === 'number' ? outcomes.durationDays : undefined,
    outcomes,
    explanation,
  };
}
