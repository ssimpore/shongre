...
function quoteHashPayload(quote: Pick<
  MonetizationQuote,
  'accountId' | 'configurationVersionId' | 'marketCode' | 'currency' | 'promotionCode' | 'lines'
>) {
  return {
...
function isScopeInContext(scope: MonetizationProduct['scope'], context: RuleEvaluationContext) {
  ...
  return dimensions.every(([allowed, actual]) => allowed.length === 0 || allowed.includes('all') || Boolean(actual && allowed.includes(actual)));
}

function isProductInScope(product: MonetizationProduct, context: RuleEvaluationContext) {
  return isScopeInContext(product.scope, context);
}

export class BusinessRulesService {
  private readonly repository: BusinessRulesRepository;
  private readonly cache = new Map<string, { catalog: MonetizationCatalog; expiresAt: number }>();
  ...
  async getCatalog(marketCode = 'FR', options: { includeDrafts?: boolean } = {}) {
    const cached = this.cache.get(marketCode);
    if (!options.includeDrafts && cached && cached.expiresAt > Date.now()) return cached.catalog;
    try {
      const loaded = await this.repository.getActiveCatalog(marketCode);
      if (!loaded) throw new Error(`No active commercial version for ${marketCode}`);
      const catalog = monetizationCatalogSchema.parse(loaded);
      this.lastKnownValid.set(marketCode, catalog);
      this.cache.set(marketCode, { catalog, expiresAt: Date.now() + CACHE_TTL_MS });
      return catalog;
    } catch (error) {
      ...
      logger.error('commercial_catalog_load_failed', {
        ...
        fallback: lastValid ? 'last_known_valid' : marketCode === 'FR' ? 'signed_baseline' : 'none',
      });
      if (lastValid) return { ...lastValid, stale: true };
      if (marketCode === 'FR') return { ...BASELINE_MONETIZATION_CATALOG, stale: true };
      throw new AppError({ code: 'NOT_FOUND', message: 'Configuration commerciale indisponible.' });
    }
  }
  ...
  private async resolvePromotion(
    ...
    const promotion = catalog.promotions.find((candidate) => candidate.code === parsed.code);
    ...
    if (new Date(promotion.startsAt) > effectiveAt) return invalid('PROMOTION_NOT_STARTED');
    if (new Date(promotion.endsAt) <= effectiveAt) return invalid('PROMOTION_EXPIRED');
    ...
    if (!isScopeInContext(promotion.scope, context)) return invalid('PROMOTION_SCOPE_MISMATCH');
    const applicableProductIds = parsed.productIds.filter((id) => promotion.productIds.includes(id));
    if (applicableProductIds.length === 0) return invalid('PROMOTION_PRODUCT_MISMATCH');
    ...
    if (promotion.maximumRedemptions !== undefined && totalRedemptions >= promotion.maximumRedemptions) {
  ...
  async validatePromotion(accountId: string, rawRequest: PromotionValidationRequest) {
  ...
  private async resolveAccountPolicy(
    ...
    if (!quotaRule || typeof quotaRule.outcome.quotaLimit !== 'number') {
      return { context, catalog, result: preview, quotaRule: undefined, periodStart: undefined, periodEnd: undefined };
    }
  ...
  async getAccountEligibility(
    ...
    return (await this.resolveAccountPolicy(accountId, rawContext, observedUsage)).result;
  }
  ...
  async authorizePublication(
    ...
    const resolved = await this.resolveAccountPolicy(accountId, rawContext, observedUsage);
  ...
  async createQuote(accountId: string, rawRequest: QuoteRequest): Promise<MonetizationQuote> {
    const request = quoteRequestSchema.parse(rawRequest);
    const existing = await this.repository.getQuoteByIdempotency(accountId, request.idempotencyKey);
    if (existing) return existing;
    const catalog = await this.getCatalog(request.marketCode);
    const products = request.productIds.map((id) => catalog.products.find((product) => product.id === id));
    if (products.some((product) => !product || product.status !== 'active')) {
      throw new AppError({ code: 'VALIDATION_ERROR', message: 'Un produit demandé est indisponible.' });
    }
    const selected = products as MonetizationProduct[];
    const accountAudience = await this.repository.getAccountAudience(accountId);
    const incompatibleAudience = selected.find(
      (product) => product.audience !== 'all' && product.audience !== accountAudience,
    );
    if (incompatibleAudience) {
      throw new AppError({ code: 'FORBIDDEN', message: 'Cette offre ne correspond pas au type de compte.' });
    }
    const context = ruleEvaluationContextSchema.parse({
    ...
    if (selected.some((product) => !isProductInScope(product, context))) {
      throw new AppError({ code: 'FORBIDDEN', message: 'Un produit n’est pas disponible dans ce contexte.' });
    }
    const selectedIds = new Set(selected.map((product) => product.id));
    for (const product of selected) {
      if (product.compatibility.requiresProductIds.some((id) => !selectedIds.has(id))) {
        throw new AppError({ code: 'VALIDATION_ERROR', message: `${product.name} nécessite une autre offre.` });
      }
      if (product.compatibility.excludesProductIds.some((id) => selectedIds.has(id))) {
        throw new AppError({ code: 'VALIDATION_ERROR', message: `${product.name} est incompatible avec une offre sélectionnée.` });
      }
    }

    const effectiveAt = new Date();
    const promotionValidation = request.promotionCode
      ? await this.resolvePromotion(accountId, {
          code: request.promotionCode,
          productIds: request.productIds,
          marketCode: request.marketCode,
          categoryId: request.categoryId,
          subtypeId: request.subtypeId,
        }, catalog, accountAudience)
      : undefined;
    ...
    const promotion = promotionValidation?.promotionId
      ? catalog.promotions.find((candidate) => candidate.id === promotionValidation.promotionId)
      : undefined;
    ...
    const lines: QuoteLine[] = selected.map((product) => {
      ...
      const price = product.prices.find((candidate) =>
        ...
        (!candidate.effectiveFrom || new Date(candidate.effectiveFrom) <= effectiveAt) &&
        (!candidate.effectiveUntil || new Date(candidate.effectiveUntil) > effectiveAt),
      );
      if (!price) throw new AppError({ code: 'VALIDATION_ERROR', message: `${product.name} n’a pas de prix actif.` });
      const subtotalMinor = price.amount.amountMinor;
      const eligibleForPromotion = promotion?.productIds.includes(product.id) || false;
      const discountMinor = !eligibleForPromotion
        ? 0
        : promotion!.discountType === 'fixed'
          ? Math.min(subtotalMinor, promotion!.discountValue)
          : Math.min(subtotalMinor, Math.round((subtotalMinor * promotion!.discountValue) / 10_000));
      const taxableMinor = subtotalMinor - discountMinor;
      const taxMinor = price.priceIncludesTax || price.taxRateBps === 0
        ? 0
    ...
    const saved = await this.repository.saveQuote(quote, request.idempotencyKey);
  ...
  async createCheckout(accountId: string, quoteId: string, idempotencyKey: string): Promise<MonetizationOrder> {
    ...
    if (!quote || quote.accountId !== accountId) throw new AppError({ code: 'NOT_FOUND', message: 'Devis introuvable.' });
    if (hashSnapshot(quoteHashPayload(quote)) !== quote.snapshotHash) {
      ...
      throw new AppError({ code: 'CONFLICT', message: 'L’intégrité du devis ne peut pas être vérifiée.' });
    }
    ...
    if (quote.status !== 'active' || new Date(quote.expiresAt) <= new Date()) {
      throw new AppError({ code: 'VALIDATION_ERROR', message: 'Ce devis a expiré.' });
    }

    const quoteCatalog = await this.repository.getCatalogVersion(quote.configurationVersionId);
    if (!quoteCatalog) throw new AppError({ code: 'CONFLICT', message: 'La version tarifaire du devis est indisponible.' });
    const recurring = quote.lines.some((line) =>
      quoteCatalog.products.find((candidate) => candidate.id === line.productId)?.kind === 'subscription',
    );
    const provider = config.dataMode === 'database' ? 'stripe' : 'demo';
    let checkout: { id: string; url?: string } = { id: deterministicId('checkout', idempotencyKey) };
    if (provider === 'stripe') {
      checkout = await stripeCheckoutAdapter.createSession({
        ...
        lines: quote.lines.map((line) => {
          const product = quoteCatalog.products.find((candidate) => candidate.id === line.productId);
          ...
          return {
            ...
            recurring: product?.kind === 'subscription' && (period === 'month' || period === 'year') ? period : undefined,
          };
        }),
    ...
    const createdAt = new Date().toISOString();
    return this.repository.saveOrder(monetizationOrderSchema.parse({
  ...
  async getAdminOverview(marketCode = 'FR'): Promise<MonetizationAdminOverview> {
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);
    const [catalog, versions, orders, auditEvents, entitlements, subscriptions, quoteCountToday] = await Promise.all([
    ...
    const publishedVersion = versions.find((version) => version.status === 'active');
    if (!publishedVersion) throw new AppError({ code: 'NOT_FOUND', message: 'Aucune version publiée.' });
    return monetizationAdminOverviewSchema.parse({
      publishedVersion,
      versions,
      catalog,
      scheduledChanges: versions.filter((version) => version.status === 'scheduled').length,
      conflictCount: versions.reduce((sum, version) => sum + version.conflicts.filter((conflict) => conflict.severity === 'blocking').length, 0),
      quoteCountToday,
      activeSubscriptionCount: subscriptions.filter((entry) => ['trialing', 'active', 'past_due'].includes(entry.status)).length,
      orders,
      entitlements,
      auditEvents,
    });
  ...
  async getActiveEntitlements(accountId: string) {
    ...
    return entitlements.filter(
      (entry) => entry.status === 'active' && (!entry.endsAt || new Date(entry.endsAt).getTime() > at),
    );
  }
  ...
  async updateSubscriptionCancellation(accountId: string, rawRequest: SubscriptionCancellationRequest) {
    const request = subscriptionCancellationRequestSchema.parse(rawRequest);
    const subscription = (await this.repository.listSubscriptions(accountId, 100))
      .find((entry) => entry.id === request.subscriptionId);
    if (!subscription) {
      throw new AppError({ code: 'NOT_FOUND', message: 'Abonnement introuvable.' });
    }
    if (config.dataMode === 'database') {
      if (!subscription.providerSubscriptionId) {
        throw new AppError({ code: 'CONFLICT', message: 'Référence prestataire manquante pour cet abonnement.' });
      }
  ...
  async handleStripeWebhook(event: any, rawBody: string) {
    if (config.dataMode !== 'database') return { processed: false, reason: 'demo_mode' };
    ...
    if (
      ...
      const subscriptionId = String(
        (eventType.startsWith('customer.subscription.') ? object.id : undefined) ||
        object.subscription ||
      ...
      if (!eventId || !subscriptionId) {
        return { processed: false, reason: 'subscription_reference_missing', eventId };
      }
      ...
      const { data, error } = await client.rpc('process_monetization_stripe_subscription_event', {
        p_provider_event_id: eventId,
        p_event_type: eventType,
        p_payload_hash: payloadHash,
        p_subscription_id: subscriptionId,
        p_provider_status: object.status ? String(object.status) : null,
        p_period_start: isoFromUnix(object.current_period_start ?? period.start),
    ...
    if (!eventId || !eventType || !checkoutId) {
      throw new AppError({ code: 'VALIDATION_ERROR', message: 'Événement de paiement incomplet.' });
    }
    if (
    ...
    const client = getSupabaseAdminClient() as any;
    const { data, error } = await client.rpc('process_monetization_stripe_event', {
      p_provider_event_id: eventId,
      p_event_type: eventType,
      p_payload_hash: payloadHash,
      p_checkout_id: checkoutId,
      p_payment_id: object.payment_intent ? String(object.payment_intent) : null,
      p_invoice_id: object.invoice ? String(object.invoice) : null,
      p_snapshot_hash: object.metadata?.snapshot_hash || null,
      p_subscription_id: object.subscription ? String(object.subscription) : null,
    });
    if (error) throw error;
  ...
  async createDraft(actorId: string, rawPatch: CommercialDraftPatch): Promise<CommercialConfigurationVersion> {
    ...
    const catalog = monetizationCatalogSchema.parse({
      ...
      rules: (patch.rules || current.rules).map((rule) => ({ ...rule, versionId: id, status: rule.status === 'active' ? 'draft' : rule.status })),
    ...
    for (const protectedRule of current.rules.filter((rule) => rule.mandatory)) {
      const candidate = catalog.rules.find((rule) => rule.key === protectedRule.key);
      ...
      if (!candidate || JSON.stringify(invariant(candidate)) !== JSON.stringify(invariant(protectedRule))) {
        throw new AppError({
          code: 'FORBIDDEN',
    ...
    await this.audit(actorId, 'draft.created', 'configuration_version', id, patch.reason, undefined, catalog);
    return version;
  }

  async transitionVersion(input: { versionId: string; action: 'submit' | 'approve' | 'publish' | 'rollback'; actorId: string; reason: string }) {
    if (input.reason.trim().length < 8) throw new AppError({ code: 'VALIDATION_ERROR', message: 'Motif détaillé requis.' });
    const catalog = await this.repository.getCatalogVersion(input.versionId);
    if (!catalog) throw new AppError({ code: 'NOT_FOUND', message: 'Version introuvable.' });
    ...
    if (!version) throw new AppError({ code: 'NOT_FOUND', message: 'Version introuvable.' });
    ...
    if (
      ...
      throw new AppError({
        code: 'CONFLICT',
        message: 'La version contient des conflits bloquants à corriger avant publication.',
        details: { conflicts: version.conflicts },
      });
    }
    const before = structuredClone(version);
    if (input.action === 'submit' && version.status === 'draft') version.status = 'pending_approval';
    else if (input.action === 'approve' && version.status === 'pending_approval') {
      if (version.createdBy === input.actorId) throw new AppError({ code: 'FORBIDDEN', message: 'Le créateur ne peut pas approuver sa propre version.' });
      version.status = 'approved';
      version.approvedBy = input.actorId;
    } else if (input.action === 'publish' && version.status === 'approved') {
      catalog.products = catalog.products.map((product) => ({ ...product, status: product.status === 'draft' ? 'active' : product.status }));
      catalog.rules = catalog.rules.map((rule) => ({ ...rule, status: rule.status === 'draft' ? 'active' : rule.status }));
      catalog.promotions = catalog.promotions.map((promotion) => ({ ...promotion, status: promotion.status === 'draft' ? 'active' : promotion.status }));
    } else if (input.action === 'rollback' && ['archived', 'disabled'].includes(version.status)) {
      const versionNumber = Math.max(...versions.map((candidate) => candidate.versionNumber), 0) + 1;
      ...
      const rollbackCatalog = monetizationCatalogSchema.parse({
        ...
        products: catalog.products.map((product) => ({
          ...
          prices: product.prices.map((price) => ({ ...price, id: `${rollbackId}:${product.id}:${price.billingPeriod}` })),
          status: product.status === 'disabled' ? 'disabled' : 'draft',
        })),
        rules: catalog.rules.map((rule) => ({ ...rule, versionId: rollbackId, status: rule.status === 'disabled' ? 'disabled' : 'draft' })),
      ...
      await this.audit(input.actorId, 'version.rollback_prepared', 'configuration_version', rollbackId, input.reason, version, rollbackVersion);
      return rollbackVersion;
    } else {
      throw new AppError({ code: 'VALIDATION_ERROR', message: 'Transition de version invalide.' });
    }
    version.reason = input.reason;
    await this.repository.saveVersion(version, catalog);
    if (input.action === 'publish') {
      await this.repository.publishVersion(version.id, input.actorId, input.reason);
      this.cache.delete(catalog.marketCode);
      const published = (await this.repository.listVersions(catalog.marketCode)).find((candidate) => candidate.id === version.id);
      if (!published) throw new AppError({ code: 'NOT_FOUND', message: 'Version publiée introuvable.' });
      await this.audit(input.actorId, 'version.publish', 'configuration_version', published.id, input.reason, before, published, published.approvedBy);
      return published;
    }
    await this.audit(input.actorId, `version.${input.action}`, 'configuration_version', version.id, input.reason, before, version, version.approvedBy);
    return version;
  }

  private async audit(actorId: string, action: string, entityType: string, entityId: string, reason: string, before?: unknown, after?: unknown, approvalActorId?: string) {
