import type { TrendCandidate, TrendingAdminConfig, TrendingTopic } from './trending.types.js';
...
function scoreCandidate(candidate: TrendCandidate, candidates: TrendCandidate[], config: TrendingAdminConfig, now: Date): number {
  const maxViews = Math.max(1, ...candidates.map((item) => item.signals.views));
  const maxFavorites = Math.max(1, ...candidates.map((item) => item.signals.favorites));
  const maxContacts = Math.max(1, ...candidates.map((item) => item.signals.contacts));
  const signals = {
  ...
  const totalWeight = Object.values(config.weights).reduce((sum, weight) => sum + Math.max(0, weight), 0) || 1;
  const weighted = (Object.keys(config.weights) as Array<keyof typeof config.weights>).reduce(
    (sum, key) => sum + signals[key] * Math.max(0, config.weights[key]),
    0,
  );
  const volume = normalizeSignal(candidate.signals.views, maxViews) * 0.06;
  return clamp(weighted / totalWeight + volume + (candidate.override?.boostScore || 0) * 0.25);
}

export function selectTrendingTopics(candidates: TrendCandidate[], config: TrendingAdminConfig, now = new Date()): TrendingTopic[] {
  const eligible = candidates
    .filter((candidate) => !config.excludedCategories.includes(candidate.parentKey))
    .filter((candidate) => !config.excludedTopics.includes(candidate.key))
    .filter((candidate) => !candidate.override?.isHidden)
    .map((candidate) => ({ candidate, score: scoreCandidate(candidate, candidates, config, now) }))
    .filter(({ candidate }) => candidate.signals.activeListings > 0)
    .sort((a, b) => {
      const pinned = Number(Boolean(b.candidate.override?.isPinned)) - Number(Boolean(a.candidate.override?.isPinned));
  ...
  for (const item of eligible) {
    ...
    if (count >= config.maxTopicsPerParentCategory && !(item.candidate.override?.isPinned && item.score >= 0.8)) continue;
    selected.push(item);
  ...
  return selected.map(({ candidate, score }) => {
    const listings = candidate.listings
      .filter((listing) => listing.status === 'published' || listing.status === 'reserved')
      .sort((a, b) => (Number(Boolean(b.images.length)) - Number(Boolean(a.images.length))) || b.viewCount - a.viewCount)
      .filter((listing) => {
    ...
    return {
      ...
      subtitle: candidate.override?.customSubtitle || 'Très recherchés cette semaine',
      href: candidate.href,
