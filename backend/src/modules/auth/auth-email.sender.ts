...
export class AuthEmailSender {
  async send(input: { to: string; template: AuthEmailTemplate; actionUrl: string }): Promise<void> {
    if (!config.authEmailDeliveryUrl) {
      if (config.nodeEnv === 'production') throw new Error('AUTH_EMAIL_DELIVERY_URL is not configured.');
      return;
    }
