...
import type { ProviderProfile, AuthProvider } from '../../shared/auth/identity.js';
...
async function postForm(url: string, body: URLSearchParams, headers: Record<string, string> = {}): Promise<any> {
...
async function validateRs256IdToken(input: {
  ...
  if (!verifySignature('RSA-SHA256', signingInput, publicKey, signature)) throw providerFailure();
  ...
  if (
    ...
    typeof claims.nonce !== 'string' || sha256(claims.nonce) !== input.nonceHash
  ) {
    throw providerFailure();
  }
  return claims;
...
function createAppleClientSecret(): string {
  const now = Math.floor(Date.now() / 1000);
  const header = Buffer.from(JSON.stringify({ alg: 'ES256', kid: config.appleOAuth.keyId, typ: 'JWT' })).toString('base64url');
  ...
  signer.end();
  const signature = signer.sign({ key: config.appleOAuth.privateKey, dsaEncoding: 'ieee-p1363' }).toString('base64url');
...
export class OAuthProviderClient {
  ...
  async exchange(input: CallbackExchange): Promise<ProviderProfile> {
    if (!this.isEnabled(input.provider) || !input.code || !input.codeVerifier) throw providerUnavailable();
  ...
  private async exchangeGoogle(input: CallbackExchange): Promise<ProviderProfile> {
  ...
  private async exchangeApple(input: CallbackExchange): Promise<ProviderProfile> {
    const token = await postForm(APPLE_TOKEN_URL, new URLSearchParams({
      code: input.code,
      client_id: config.appleOAuth.clientId,
  ...
  private async exchangeFacebook(input: CallbackExchange): Promise<ProviderProfile> {
    const token = await postForm(new URL('/oauth/access_token', config.facebookOAuth.graphApiBaseUrl).toString(), new URLSearchParams({
      code: input.code,
      client_id: config.facebookOAuth.clientId,
    ...
    const now = Math.floor(Date.now() / 1000);
    if (!debugData?.is_valid || String(debugData.app_id) !== config.facebookOAuth.clientId || (debugData.expires_at && debugData.expires_at <= now)) {
      throw providerFailure();
    }
    ...
    if (!profile?.id || (debugData.user_id && String(debugData.user_id) !== String(profile.id))) throw providerFailure();
    return {
      provider: 'facebook',
