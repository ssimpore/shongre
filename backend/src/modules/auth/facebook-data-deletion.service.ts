...
function invalidRequest(): AppError {
  return new AppError({ code: 'FORBIDDEN', message: 'Demande de suppression invalide.' });
}

function decodeSignedRequest(signedRequest: string, appSecret: string): FacebookDeletionPayload {
  const [encodedSignature, encodedPayload, extra] = (signedRequest || '').split('.');
  if (!encodedSignature || !encodedPayload || extra || !appSecret) throw invalidRequest();

  let signature: Buffer;
  let payload: FacebookDeletionPayload;
  try {
    signature = Buffer.from(encodedSignature, 'base64url');
    payload = JSON.parse(Buffer.from(encodedPayload, 'base64url').toString('utf8')) as FacebookDeletionPayload;
  } catch {
    throw invalidRequest();
  }

  const expected = createHmac('sha256', appSecret).update(encodedPayload).digest();
  if (signature.length !== expected.length || !timingSafeEqual(signature, expected)) throw invalidRequest();
  if (String(payload.algorithm || '').toUpperCase() !== 'HMAC-SHA256') throw invalidRequest();
  if (!payload.user_id || payload.user_id.length > 255) throw invalidRequest();
  const now = Math.floor(Date.now() / 1000);
  if (typeof payload.issued_at !== 'number' || payload.issued_at > now + 300 || payload.issued_at < now - 86_400) {
    throw invalidRequest();
  }
  return payload;
}
...
export class FacebookDataDeletionService {
  constructor(
    ...
    private readonly accountDeletion: Pick<UsersService, 'deleteFromVerifiedProvider'> = usersService,
  ) {}

  async request(signedRequest: string): Promise<{ url: string; confirmation_code: string }> {
    const payload = decodeSignedRequest(signedRequest, this.appSecret);
    const identity = await this.repository.findIdentity('facebook', payload.user_id!);
  ...
  async status(confirmationCode: string): Promise<{ status: 'queued' | 'completed' | 'rejected' }> {
    const request = confirmationCode
      ? await this.repository.findProviderDeletionRequest(sha256(confirmationCode))
      : null;
    if (!request) throw new AppError({ code: 'NOT_FOUND', message: 'Demande introuvable.' });
    return { status: request.status };
  }

  async processQueued(limit = 50): Promise<{ completed: number; pending: number }> {
    const records = await this.repository.listQueuedProviderDeletionRequests(limit);
    let completed = 0;
  ...
  private async processRecord(record: ProviderDeletionRequestRecord): Promise<boolean> {
    if (!record.userId) {
      await this.repository.updateProviderDeletionRequest(record.id, 'completed');
      return true;
    }
    try {
