...
export const DEFAULT_COURSE_PLANS: CoursePlan[] = [
  {
    ...
    audience: "individual",
    description:
      "Un profil public, un cours actif et les outils essentiels pour démarrer.",
    taxRateBps: 2000,
    isActive: true,
    isRecommended: false,
  ...
  },
  {
    ...
    audience: "individual",
    description:
      "Plus de cours, un calendrier avancé et des outils de suivi des demandes.",
  ...
  },
  {
    ...
    audience: "individual",
    description:
      "Capacité renforcée, priorité encadrée et outils de fidélisation.",
  ...
  },
  {
    ...
    description:
      "Équipe, lieux, cours et demandes centralisés avec facturation unique.",
...
export const DEMO_TUTOR_PROFILES: TutorProfile[] = [
  makeProfile({
    ...
    biography:
      "Enseignant attentif, je construis chaque séance autour des acquis et du rythme de l’élève.",
    teachingApproach:
      "Diagnostic court, explications concrètes, exercices progressifs et bilan partagé.",
    rating: 4.9,
    reviewCount: 87,
    responseTimeMinutes: 120,
    experienceYears: 5,
  ...
  makeProfile({
    ...
    biography:
      "J’aide les élèves à retrouver de la méthode et de la confiance sans apprendre par cœur.",
    teachingApproach:
      "Exemples visuels, raisonnement guidé puis autonomie progressive.",
  ...
  makeProfile({
    ...
    biography:
      "Professeur certifié, j’accompagne les élèves vers des objectifs mesurables et réalistes.",
    teachingApproach:
      "Plan de travail partagé, annales ciblées et retours après chaque séance.",
    subjectIds: [
      "subject_mathematics",
      "subject_physics_chemistry",
      "subject_exam_preparation",
    ],
    rating: 4.7,
    reviewCount: 112,
  ...
  makeProfile({
    ...
    biography:
      "Étudiante en master, je propose des séances structurées et rassurantes.",
    teachingApproach:
      "Reprise du cours, exercices gradués et fiche de synthèse.",
    ...
    verifications: {
      ...makeProfile({
        ...
        slug: "y",
        displayName: "y",
        avatarUrl: "https://example.com/y.jpg",
        headline: "y",
        biography: "y",
        teachingApproach: "y",
      }).verifications,
  ...
  makeProfile({
    ...
    biography:
      "Professeure de mathématiques, je suis les collégiens et lycéens en petits groupes ou individuellement.",
    teachingApproach:
      "Objectifs définis avec la famille, séance active et synthèse après le cours.",
    serviceArea: {
      marketCode: "FR",
      cityLabel: "Lyon",
      postalCodePrefix: "69",
...
export const DEMO_LEARNER_REQUESTS: LearnerRequest[] = [
  {
    ...
    objective:
      "Reprendre les bases en mathématiques et préparer le brevet avec une méthode régulière.",
  ...
  },
  {
    ...
    levelId: "high_school",
    objective:
      "Préparer la rentrée en première et consolider les fonctions pendant les dernières semaines d’août.",
...
function matchesTutor(
  ...
  if (
    ...
    !query.levelIds.some((level) => offer.levelIds.includes(level))
  )
    return false;
  if (
  ...
  if (
    ...
    !query.languages.some((language) => offer.languages.includes(language))
  )
    return false;
  if (
    query.city &&
  ...
  if (
    ...
    return false;
  const hourly =
    offer.pricingOptions.find((price) => price.type === "hourly")?.price
      .amountMinor || 0;
  if (query.minPriceMinor !== undefined && hourly < query.minPriceMinor)
    return false;
  if (query.maxPriceMinor !== undefined && hourly > query.maxPriceMinor)
    return false;
  if (query.tutorType === "individual" && tutor.profileType !== "individual")
    return false;
...
function toSearchItem(
  ...
  return {
    tutor: clone(toPublicTutor(tutor)),
    ...
    distanceKm:
      tutor.serviceArea?.cityLabel === "Toulouse"
        ? Number((3 + DEMO_TUTOR_PROFILES.indexOf(tutor) * 1.7).toFixed(1))
        : undefined,
    relevanceReasons: [
      "Matière et niveau compatibles",
...
export class DemoCoursesRepository implements ICoursesRepository {
  ...
  private tutors = new Map(
    DEMO_TUTOR_PROFILES.map((item) => [item.id, clone(item)]),
  );
  private offers = new Map(
    DEMO_COURSE_OFFERS.map((item) => [item.id, clone(item)]),
  );
  private requests = new Map(
    DEMO_LEARNER_REQUESTS.map((item) => [item.id, clone(item)]),
  );
  ...
  async searchTutors(query: TutorSearchQuery): Promise<TutorSearchResponse> {
    ...
    matched.sort((a, b) => {
      ...
      if (query.sort === "response_time")
        return (
          (a.tutor.responseTimeMinutes || Infinity) -
          (b.tutor.responseTimeMinutes || Infinity)
        );
      return (
        relevanceScore(query, b.tutor, b.offer) -
        relevanceScore(query, a.tutor, a.offer)
      );
    });
    ...
    return {
      ...
      pageInfo: {
        ...
        nextCursor:
          offset + limit < matched.length ? String(offset + limit) : undefined,
      },
    };
  }
  ...
  async getCourseOffers(tutorProfileId: string): Promise<CourseOffer[]> {
    return Array.from(this.offers.values())
      .filter((offer) => offer.tutorProfileId === tutorProfileId)
      .map(clone);
  }
  ...
  async getTutorLeads(tutorProfileId: string): Promise<CourseLead[]> {
    return Array.from(this.leads.values())
      .filter((lead) => lead.tutorProfileId === tutorProfileId)
      .map(clone);
  }
  ...
  async getTutorWorkspace(
    ...
    return {
      ...
      plan: clone(plan),
      creditsRemaining:
        tutor.planId === "school_organization"
          ? 18
          : tutor.planId === "tutor_pro"
            ? 4
            : 0,
      analytics: {
        period: "last_30_days",
        profileViews: tutor.id === "tutor_sophie" ? 142 : 48,
        requestsReceived: tutor.id === "tutor_sophie" ? 9 : 3,
        acceptedLeads: tutor.id === "tutor_sophie" ? 4 : 1,
        medianResponseMinutes: tutor.responseTimeMinutes,
        contactConversionRate: tutor.id === "tutor_sophie" ? 0.27 : undefined,
      },
  ...
  async getOrganizationWorkspace(
    ...
    const plan =
      this.catalog.plans.find((item) => item.id === organization.planId) ||
      this.catalog.plans[0];
    return clone({
      organization,
...
export class PostgresCoursesRepository implements ICoursesRepository {
  ...
  async saveMarketConfig(
    ...
    const parsed = courseMarketConfigSchema.parse({
      ...config,
      marketCode,
      updatedAt: new Date().toISOString(),
    });
      ...
      .upsert(
        {
          ...
          booking_enabled: parsed.featureFlags.bookingEnabled,
          payments_enabled: parsed.featureFlags.paymentsEnabled,
  ...
  async searchTutors(query: TutorSearchQuery): Promise<TutorSearchResponse> {
    ...
    if (query.levelIds?.length)
      request = request.overlaps("level_ids", query.levelIds);
    if (query.deliveryModes?.length)
      request = request.overlaps("delivery_modes", query.deliveryModes);
    ...
    if (query.minPriceMinor !== undefined)
      request = request.gte("from_price_minor", query.minPriceMinor);
    if (query.maxPriceMinor !== undefined)
      request = request.lte("from_price_minor", query.maxPriceMinor);
    if (query.query)
    ...
    request = request
      .order(
        query.sort === "price_asc" || query.sort === "price_desc"
          ? "from_price_minor"
          : "relevance_baseline",
        { ascending: query.sort === "price_asc" },
      )
    ...
    const items = (data || []).map((row: any) => ({
      ...
      fromPrice: {
        amountMinor: Number(row.from_price_minor),
  ...
  async saveTutorProfile(profile: TutorProfile): Promise<TutorProfile> {
    ...
    const { error } = await supabase.from("course_tutor_profiles").upsert({
      ...
      teaching_approach: parsed.teachingApproach,
      experience_years: parsed.experienceYears,
      moderation_status: parsed.moderationStatus,
      profile_completion_percent: parsed.profileCompletionPercent,
      plan_id: parsed.planId,
      response_time_minutes: parsed.responseTimeMinutes,
      response_rate_percent: parsed.responseRatePercent,
      rating: parsed.rating,
      review_count: parsed.reviewCount,
      rating_is_statistically_meaningful:
        parsed.ratingIsStatisticallyMeaningful,
      is_featured: parsed.isFeatured,
      public_payload: publicProfile,
    ...
    await Promise.all([
      supabase
        .from("course_tutor_subjects")
        .delete()
        .eq("tutor_profile_id", parsed.id),
      supabase
        .from("course_tutor_levels")
        .delete()
        .eq("tutor_profile_id", parsed.id),
      supabase
        .from("course_tutor_languages")
        .delete()
        .eq("tutor_profile_id", parsed.id),
      supabase
        .from("course_tutor_delivery_modes")
        .delete()
        .eq("tutor_profile_id", parsed.id),
    ]);
    await Promise.all([
      parsed.subjectIds.length
        ? supabase.from("course_tutor_subjects").insert(
            parsed.subjectIds.map((subjectId) => ({
              tutor_profile_id: parsed.id,
              subject_id: subjectId,
              market_code: marketCode,
            })),
          )
        : Promise.resolve(),
      parsed.levelIds.length
        ? supabase.from("course_tutor_levels").insert(
            parsed.levelIds.map((levelId) => ({
              tutor_profile_id: parsed.id,
              level_id: levelId,
              market_code: marketCode,
      ...
      parsed.languages.length
        ? supabase.from("course_tutor_languages").insert(
            parsed.languages.map((languageCode) => ({
              tutor_profile_id: parsed.id,
        ...
        : Promise.resolve(),
      parsed.deliveryModes.length
        ? supabase.from("course_tutor_delivery_modes").insert(
            parsed.deliveryModes.map((deliveryMode) => ({
              tutor_profile_id: parsed.id,
    ...
    if (parsed.serviceArea) {
      ...
      await supabase.from("course_service_areas").insert({
        ...
        market_code: marketCode,
        city_label: parsed.serviceArea.cityLabel,
        postal_code_prefix: parsed.serviceArea.postalCodePrefix,
        region: parsed.serviceArea.region,
        center_latitude: parsed.serviceArea.latitude,
        center_longitude: parsed.serviceArea.longitude,
        radius_km: parsed.serviceArea.radiusKm,
        public_location_label: parsed.serviceArea.publicLocationLabel,
        updated_at: parsed.updatedAt,
      });
    }
    return parsed;
  ...
  async getCourseOffers(tutorProfileId: string): Promise<CourseOffer[]> {
    ...
    return (data || []).map((row: any) =>
      courseOfferSchema.parse(row.private_payload),
    );
  }
  ...
  async saveCourseOffer(offer: CourseOffer): Promise<CourseOffer> {
    ...
    const { error } = await supabase.from("course_offers").upsert({
      ...
      tutor_profile_id: parsed.tutorProfileId,
      organization_id: parsed.organizationId || null,
      schema_version: parsed.schemaVersion,
      slug: parsed.slug,
      title: parsed.title,
    ...
    await Promise.all([
      supabase
        .from("course_offer_levels")
        .delete()
        .eq("course_offer_id", parsed.id),
      supabase
        .from("course_offer_delivery_modes")
        .delete()
        .eq("course_offer_id", parsed.id),
      supabase
        .from("course_offer_languages")
        .delete()
        .eq("course_offer_id", parsed.id),
      supabase
        .from("course_pricing_options")
        .delete()
        .eq("course_offer_id", parsed.id),
    ]);
    await Promise.all([
      supabase.from("course_offer_levels").insert(
        parsed.levelIds.map((levelId) => ({
          course_offer_id: parsed.id,
          level_id: levelId,
          market_code: marketCode,
        })),
      ),
      supabase.from("course_offer_delivery_modes").insert(
        parsed.deliveryModes.map((deliveryMode) => ({
          course_offer_id: parsed.id,
          delivery_mode: deliveryMode,
        })),
      ),
      supabase.from("course_offer_languages").insert(
        parsed.languages.map((languageCode) => ({
          course_offer_id: parsed.id,
          language_code: languageCode,
        })),
      ),
      supabase.from("course_pricing_options").insert(
        parsed.pricingOptions.map((price) => ({
          id: price.id,
          course_offer_id: parsed.id,
          type: price.type,
          label: price.label,
          price_minor: price.price.amountMinor,
          currency: price.price.currency,
          duration_minutes: price.durationMinutes,
          lesson_count: price.lessonCount || null,
    ...
    if (parsed.listingId) {
      await supabase
        .from("listings")
        .update({
          vertical_type: "tutoring",
          vertical_entity_id: parsed.id,
          vertical_schema_version: parsed.schemaVersion,
        })
  ...
  async createLearnerRequest(request: LearnerRequest): Promise<LearnerRequest> {
    const parsed = learnerRequestSchema.parse(request);
    const { error } = await (getSupabaseAdminClient() as any)
      .from("course_learner_requests")
      .insert({
        id: parsed.id,
        requester_user_id: parsed.requesterUserId || null,
        market_code: parsed.marketCode,
        subject_id: parsed.subjectId,
        level_id: parsed.levelId,
        objective: parsed.objective,
        delivery_modes: parsed.deliveryModes,
        preferred_schedule: parsed.preferredSchedule,
        city: parsed.city,
        radius_km: parsed.radiusKm,
        budget_min_minor: parsed.budgetMin?.amountMinor,
        budget_max_minor: parsed.budgetMax?.amountMinor,
        currency: parsed.budgetMin?.currency || parsed.budgetMax?.currency,
        desired_start_date: parsed.desiredStartDate,
        learner_age_band: parsed.learnerAgeBand,
        guardian_user_id: parsed.guardianContact?.guardianUserId,
        guardian_consent_confirmed_at:
          parsed.guardianContact?.consentConfirmedAt,
        status: parsed.status,
        private_payload: parsed,
  ...
  async getTutorLeads(tutorProfileId: string): Promise<CourseLead[]> {
    ...
    if (error) throw error;
    return (data || []).map((row: any) =>
      courseLeadSchema.parse(row.private_payload),
    );
  }
  ...
  async saveLead(lead: CourseLead): Promise<CourseLead> {
    ...
    const { error } = await (getSupabaseAdminClient() as any)
      ...
      .upsert({
        id: parsed.id,
        learner_request_id: parsed.learnerRequestId,
        tutor_profile_id: parsed.tutorProfileId,
        organization_id: parsed.organizationId || null,
        state: parsed.state,
        relevance_score: parsed.relevanceScore,
        relevance_reasons: parsed.relevanceReasons,
        contact_release_status: parsed.contactReleaseStatus,
        credit_cost: parsed.creditCost,
        decline_reason: parsed.declineReason,
        credit_restored_at: parsed.creditRestoredAt,
        expires_at: parsed.expiresAt,
  ...
  async getTutorWorkspace(
    ...
    return {
      ...
      analytics: {
        ...
        acceptedLeads: leads.filter((lead) => lead.state === "accepted").length,
        medianResponseMinutes: tutor.responseTimeMinutes,
      },
  ...
  async getOrganizationWorkspace(
    ...
    if (!organization) return null;
    const supabase = getSupabaseAdminClient() as any;
    const [catalog, membersResult, tutorsResult, locationsResult, leadsResult] =
      await Promise.all([
        this.getCatalog(organization.marketCode),
        supabase
          .from("course_organization_members")
          .select("id, organization_id, user_id, role, permissions, status")
          .eq("organization_id", organization.id),
        supabase
          .from("course_tutor_profiles")
          .select("id, user_id, public_payload")
          .eq("organization_id", organization.id),
        supabase
          .from("course_service_areas")
          .select("id, public_location_label")
          .eq("organization_id", organization.id),
        supabase
          .from("course_leads")
    ...
    const members: CourseOrganizationMember[] = (membersResult.data || []).map(
      (row: any) => {
        ...
        return {
          ...
          tutorProfileId: tutor?.id,
          displayName:
            tutor?.public_payload?.displayName || "Membre de l’équipe",
          role: row.role,
    ...
    return {
      ...
      members,
      plan:
        catalog.plans.find((plan) => plan.id === organization.planId) ||
        catalog.plans[0],
      featureFlags: catalog.config.featureFlags,
      ...
      analytics: {
        ...
        leadsAccepted: leads.filter((lead: any) => lead.state === "accepted")
          .length,
