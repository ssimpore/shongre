...
import type { TrendingActivitySignals, TrendingAdminConfig, TrendingQuery, TrendingSectionResponse, TrendingTopicOverride } from '../../../modules/trending/trending.types.js';
...
export interface ITrendingRepository {
  ...
  getActivitySignals(marketCode: string, since: string): Promise<Map<string, TrendingActivitySignals>>;
  getCachedSection(query: TrendingQuery): Promise<TrendingSectionResponse | null>;
  saveCachedSection(marketCode: string, response: TrendingSectionResponse): Promise<void>;
}
...
export class DemoTrendingRepository implements ITrendingRepository {
  ...
  async getConfig(marketCode: string): Promise<TrendingAdminConfig> {
    return structuredClone(this.configs.get(marketCode) || createDefaultTrendingConfig());
  }

  async saveConfig(marketCode: string, updates: Partial<TrendingAdminConfig>): Promise<TrendingAdminConfig> {
    const current = await this.getConfig(marketCode);
    const next = { ...current, ...updates, weights: { ...current.weights, ...(updates.weights || {}) }, updatedAt: new Date().toISOString() };
    this.configs.set(marketCode, next);
    return structuredClone(next);
  }

  async upsertOverride(marketCode: string, override: TrendingTopicOverride): Promise<TrendingAdminConfig> {
    const current = await this.getConfig(marketCode);
    return this.saveConfig(marketCode, {
      overrides: [...current.overrides.filter((item) => item.topicKey !== override.topicKey), { ...override, marketCode }],
    });
  }

  async refreshActivityWindow(_marketCode: string, _windowStart: string, _windowEnd: string): Promise<void> {
    return Promise.resolve();
  }

  async getActivitySignals(_marketCode: string, _since: string): Promise<Map<string, TrendingActivitySignals>> {
    return new Map();
  }

  async getCachedSection(_query: TrendingQuery): Promise<TrendingSectionResponse | null> {
    return null;
  }

  async saveCachedSection(_marketCode: string, _response: TrendingSectionResponse): Promise<void> {
    return Promise.resolve();
  }
}
...
export class PostgresTrendingRepository implements ITrendingRepository {
  async getConfig(marketCode: string): Promise<TrendingAdminConfig> {
    try {
      ...
      const { data, error } = await supabase.from('trending_section_configs').select('*').eq('market_code', marketCode).maybeSingle();
      if (error || !data) return createDefaultTrendingConfig();
      ...
      return {
        ...
        maxTopicsPerParentCategory: Number(data.max_topics_per_parent_category ?? defaults.maxTopicsPerParentCategory),
        minimumActivity: Number(data.minimum_activity ?? defaults.minimumActivity),
        displayPeriodDays: Number(data.display_period_days ?? defaults.displayPeriodDays),
        cacheTtlMinutes: Number(data.cache_ttl_minutes ?? defaults.cacheTtlMinutes),
        personalizationWeight: Number(data.personalization_weight ?? defaults.personalizationWeight),
      ...
      logger.warn(`PostgresTrendingRepository.getConfig fallback: ${String(error)}`);
      return createDefaultTrendingConfig();
    }
  }
  ...
  async saveConfig(marketCode: string, updates: Partial<TrendingAdminConfig>): Promise<TrendingAdminConfig> {
    ...
    try {
      const supabase = getSupabaseAdminClient() as any;
      await supabase.from('trending_section_configs').upsert({
      ...
      logger.warn(`PostgresTrendingRepository.saveConfig skipped: ${String(error)}`);
    }
    return next;
  }
  ...
  async upsertOverride(marketCode: string, override: TrendingTopicOverride): Promise<TrendingAdminConfig> {
    try {
      const supabase = getSupabaseAdminClient() as any;
      await supabase.from('trending_topic_overrides').upsert({
        market_code: marketCode,
      ...
      logger.warn(`PostgresTrendingRepository.upsertOverride skipped: ${String(error)}`);
    }
    return this.getConfig(marketCode);
  }

  async refreshActivityWindow(marketCode: string, windowStart: string, windowEnd: string): Promise<void> {
    try {
      const supabase = getSupabaseAdminClient() as any;
      ...
      if (rows.length > 0) {
        const { error: upsertError } = await supabase
          ...
          .upsert(rows, { onConflict: 'market_code,topic_type,topic_key,window_start,window_end' });
        if (upsertError) throw upsertError;
      }
    } catch (error) {
      logger.warn(`PostgresTrendingRepository.refreshActivityWindow skipped: ${String(error)}`);
    }
  }

  async getActivitySignals(marketCode: string, since: string): Promise<Map<string, TrendingActivitySignals>> {
    ...
    try {
      ...
      logger.warn(`PostgresTrendingRepository.getActivitySignals fallback: ${String(error)}`);
    }
    return result;
  }

  async getCachedSection(query: TrendingQuery): Promise<TrendingSectionResponse | null> {
    ...
    try {
      ...
      const [{ data, error }, { data: config }] = await Promise.all([
        supabase
          ...
          .limit(query.limit || 8),
        supabase.from('trending_section_configs').select('enabled, title, subtitle').eq('market_code', query.marketCode).maybeSingle(),
      ]);
      ...
      return {
        ...
        subtitle: config?.subtitle || 'Découvrez ce qui attire le plus les acheteurs en ce moment.',
        topics: data.map((row: { topic_payload: TrendingSectionResponse['topics'][number] }) => row.topic_payload),
      };
    } catch (error) {
      logger.warn(`PostgresTrendingRepository.getCachedSection fallback: ${String(error)}`);
      return null;
    }
  }

  async saveCachedSection(marketCode: string, response: TrendingSectionResponse): Promise<void> {
    try {
      const supabase = getSupabaseAdminClient() as any;
      await supabase.from('trending_topics').delete().eq('market_code', marketCode);
      const expiresAt = response.expiresAt || new Date(Date.now() + 20 * 60 * 1000).toISOString();
      await supabase.from('trending_topics').insert(response.topics.map((topic, sortOrder) => ({
        market_code: marketCode,
      ...
      logger.warn(`PostgresTrendingRepository.saveCachedSection skipped: ${String(error)}`);
    }
  }
}
