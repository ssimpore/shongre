...
export interface IAuthRepository {
  ...
  createProviderDeletionRequest(input: Omit<ProviderDeletionRequestRecord, 'id' | 'status' | 'requestedAt' | 'completedAt'>): Promise<ProviderDeletionRequestRecord>;
  findProviderDeletionRequest(codeHash: string): Promise<ProviderDeletionRequestRecord | null>;
  updateProviderDeletionRequest(id: string, status: ProviderDeletionRequestRecord['status']): Promise<void>;
  listQueuedProviderDeletionRequests(limit?: number): Promise<ProviderDeletionRequestRecord[]>;
  deleteIdentities(userId: string): Promise<void>;
  findIdentity(provider: AuthProvider, subject: string): Promise<ExternalIdentityRecord | null>;
  listIdentities(userId: string): Promise<ExternalIdentityRecord[]>;
  linkIdentity(input: Omit<ExternalIdentityRecord, 'id' | 'linkedAt' | 'lastAuthenticatedAt'>): Promise<ExternalIdentityRecord>;
  touchIdentity(id: string): Promise<void>;
  unlinkIdentity(userId: string, provider: AuthProvider): Promise<void>;
  createSession(input: Omit<AuthSessionRecord, 'id' | 'issuedAt' | 'lastUsedAt' | 'revokedAt' | 'revokedReason'>): Promise<AuthSessionRecord>;
  ...
  revokeSessions(userId: string, reason: string, exceptId?: string): Promise<void>;
  ...
  createPendingRegistration(input: Omit<PendingOAuthRegistration, 'id'>): Promise<void>;
  consumePendingRegistration(handleHash: string): Promise<PendingOAuthRegistration | null>;
  createActionToken(input: Omit<AuthActionTokenRecord, 'id'>): Promise<void>;
  consumeActionToken(tokenHash: string, purpose: AuthActionTokenRecord['purpose']): Promise<AuthActionTokenRecord | null>;
  consumeRateLimit(keyHash: string, action: string, limit: number, windowSeconds: number, lockSeconds: number): Promise<RateLimitDecision>;
...
function mapActionToken(row: any): AuthActionTokenRecord {
  return { id: row.id, userId: row.user_id, purpose: row.purpose, tokenHash: row.token_hash, expiresAt: row.expires_at };
}
...
export class DemoAuthRepository implements IAuthRepository {
  ...
  private providerDeletionRequests = new Map<string, ProviderDeletionRequestRecord>();
  private rateLimits = new Map<string, { attempts: number; resetAt: number; lockedUntil: number }>();
  ...
  async createProviderDeletionRequest(input: Omit<ProviderDeletionRequestRecord, 'id' | 'status' | 'requestedAt' | 'completedAt'>): Promise<ProviderDeletionRequestRecord> {
  ...
  async findProviderDeletionRequest(codeHash: string): Promise<ProviderDeletionRequestRecord | null> {
    return this.providerDeletionRequests.get(codeHash) || null;
  }

  async updateProviderDeletionRequest(id: string, status: ProviderDeletionRequestRecord['status']): Promise<void> {
  ...
  async listQueuedProviderDeletionRequests(limit = 50): Promise<ProviderDeletionRequestRecord[]> {
  ...
  async findIdentity(provider: AuthProvider, subject: string): Promise<ExternalIdentityRecord | null> {
  ...
  async linkIdentity(input: Omit<ExternalIdentityRecord, 'id' | 'linkedAt' | 'lastAuthenticatedAt'>): Promise<ExternalIdentityRecord> {
    const key = this.identityKey(input.provider, input.providerSubject);
    const existingForProvider = [...this.identities.values()].find(
      (identity) => identity.userId === input.userId && identity.provider === input.provider,
    );
    if (this.identities.has(key) || existingForProvider) throw new Error('identity_conflict');
    const now = new Date().toISOString();
    const identity: ExternalIdentityRecord = { ...input, id: randomUUID(), linkedAt: now, lastAuthenticatedAt: now };
  ...
  async touchIdentity(id: string): Promise<void> {
    for (const [key, identity] of this.identities) {
      if (identity.id === id) this.identities.set(key, { ...identity, lastAuthenticatedAt: new Date().toISOString() });
    }
  }

  async unlinkIdentity(userId: string, provider: AuthProvider): Promise<void> {
    for (const [key, identity] of this.identities) {
      if (identity.userId === userId && identity.provider === provider) this.identities.delete(key);
    }
  }

  async createSession(input: Omit<AuthSessionRecord, 'id' | 'issuedAt' | 'lastUsedAt' | 'revokedAt' | 'revokedReason'>): Promise<AuthSessionRecord> {
  ...
  async findSessionByRefreshHash(hash: string): Promise<AuthSessionRecord | null> {
  ...
  async markSessionReauthenticated(id: string): Promise<void> {
    ...
    if (current) this.sessions.set(id, { ...current, lastReauthenticatedAt: new Date().toISOString() });
  }

  async touchSession(id: string): Promise<void> {
    const current = this.sessions.get(id);
    if (current && !current.revokedAt) this.sessions.set(id, { ...current, lastUsedAt: new Date().toISOString() });
  }
  ...
  async revokeSession(id: string, reason: string): Promise<void> {
    const current = this.sessions.get(id);
    if (current && !current.revokedAt) this.sessions.set(id, { ...current, revokedAt: new Date().toISOString(), revokedReason: reason });
  }
  ...
  async revokeSessions(userId: string, reason: string, exceptId?: string): Promise<void> {
    await Promise.all([...this.sessions.values()]
      .filter((session) => session.userId === userId && session.id !== exceptId)
      .map((session) => this.revokeSession(session.id, reason)));
  }
  ...
  async createNativeExchange(input: Omit<NativeExchangeRecord, 'id'>): Promise<void> {
    this.exchanges.set(input.codeHash, { ...input, id: randomUUID() });
  }

  async consumeNativeExchange(codeHash: string): Promise<NativeExchangeRecord | null> {
  ...
  async createPendingRegistration(input: Omit<PendingOAuthRegistration, 'id'>): Promise<void> {
    for (const [key, pending] of this.pendingRegistrations) {
      if (pending.provider === input.provider && pending.providerSubject === input.providerSubject) {
        this.pendingRegistrations.delete(key);
      }
    }
    this.pendingRegistrations.set(input.handleHash, { ...input, id: randomUUID() });
  }

  async consumePendingRegistration(handleHash: string): Promise<PendingOAuthRegistration | null> {
  ...
  async createActionToken(input: Omit<AuthActionTokenRecord, 'id'>): Promise<void> {
    this.actionTokens.set(input.tokenHash, { ...input, id: randomUUID() });
  }

  async consumeActionToken(tokenHash: string, purpose: AuthActionTokenRecord['purpose']): Promise<AuthActionTokenRecord | null> {
    const token = this.actionTokens.get(tokenHash);
    this.actionTokens.delete(tokenHash);
    if (!token || token.purpose !== purpose || Date.parse(token.expiresAt) <= Date.now()) return null;
    return token;
  }

  async consumeRateLimit(keyHash: string, action: string, limit: number, windowSeconds: number, lockSeconds: number): Promise<RateLimitDecision> {
    const key = `${action}:${keyHash}`;
    const now = Date.now();
    const current = this.rateLimits.get(key);
    if (!current || current.resetAt <= now) {
      this.rateLimits.set(key, { attempts: 1, resetAt: now + windowSeconds * 1000, lockedUntil: 0 });
    ...
    current.attempts += 1;
    if (current.attempts > limit) current.lockedUntil = now + lockSeconds * 1000;
...
export class PostgresAuthRepository implements IAuthRepository {
  ...
  async provisionOAuthProfile(input: OAuthProfileProvisionInput): Promise<boolean> {
    ...
    if (error || data !== input.userId) {
      throw new Error(`OAuth profile provisioning failed: ${error?.message || 'unexpected result'}`);
    }
    return true;
  }

  async createProviderDeletionRequest(input: Omit<ProviderDeletionRequestRecord, 'id' | 'status' | 'requestedAt' | 'completedAt'>): Promise<ProviderDeletionRequestRecord> {
    const { data, error } = await this.client().from('oauth_provider_deletion_requests').insert({
      provider: input.provider,
      provider_subject: input.providerSubject,
      user_id: input.userId,
      confirmation_code_hash: input.confirmationCodeHash,
    }).select('*').single();
    if (error || !data) throw new Error(`provider deletion request failed: ${error?.message || 'missing row'}`);
    return {
      id: data.id,
  ...
  async findProviderDeletionRequest(codeHash: string): Promise<ProviderDeletionRequestRecord | null> {
    const { data, error } = await this.client().from('oauth_provider_deletion_requests')
      .select('*').eq('confirmation_code_hash', codeHash).maybeSingle();
    if (error) throw new Error(`provider deletion status failed: ${error.message}`);
    return data ? {
      id: data.id,
      provider: data.provider,
      providerSubject: data.provider_subject,
  ...
  async updateProviderDeletionRequest(id: string, status: ProviderDeletionRequestRecord['status']): Promise<void> {
    const { error } = await this.client().from('oauth_provider_deletion_requests').update({
      status,
      completed_at: status === 'completed' ? new Date().toISOString() : null,
    }).eq('id', id);
    if (error) throw new Error(`provider deletion status update failed: ${error.message}`);
  }

  async listQueuedProviderDeletionRequests(limit = 50): Promise<ProviderDeletionRequestRecord[]> {
    const { data, error } = await this.client().from('oauth_provider_deletion_requests')
      .select('*').eq('status', 'queued').order('requested_at').limit(limit);
    if (error) throw new Error(`provider deletion queue read failed: ${error.message}`);
  ...
  async deleteIdentities(userId: string): Promise<void> {
    const { error } = await this.client().from('user_identities').delete().eq('user_id', userId);
    if (error) throw new Error(`identity deletion failed: ${error.message}`);
  }

  async findIdentity(provider: AuthProvider, subject: string): Promise<ExternalIdentityRecord | null> {
    const { data, error } = await this.client().from('user_identities').select('*').eq('provider', provider).eq('provider_subject', subject).maybeSingle();
    if (error) throw new Error(`identity lookup failed: ${error.message}`);
    return data ? mapIdentity(data) : null;
  }

  async listIdentities(userId: string): Promise<ExternalIdentityRecord[]> {
    const { data, error } = await this.client().from('user_identities').select('*').eq('user_id', userId).order('linked_at');
    if (error) throw new Error(`identity list failed: ${error.message}`);
    return (data || []).map(mapIdentity);
  }

  async linkIdentity(input: Omit<ExternalIdentityRecord, 'id' | 'linkedAt' | 'lastAuthenticatedAt'>): Promise<ExternalIdentityRecord> {
    const { data, error } = await this.client().from('user_identities').insert({
      user_id: input.userId,
      provider: input.provider,
      provider_subject: input.providerSubject,
    ...
    if (error || !data) throw new Error(`identity link failed: ${error?.message || 'missing row'}`);
    return mapIdentity(data);
  }

  async touchIdentity(id: string): Promise<void> {
    const { error } = await this.client().from('user_identities').update({ last_authenticated_at: new Date().toISOString() }).eq('id', id);
    if (error) throw new Error(`identity update failed: ${error.message}`);
  }

  async unlinkIdentity(userId: string, provider: AuthProvider): Promise<void> {
    const { error } = await this.client().from('user_identities').delete().eq('user_id', userId).eq('provider', provider);
    if (error) throw new Error(`identity unlink failed: ${error.message}`);
  }

  async createSession(input: Omit<AuthSessionRecord, 'id' | 'issuedAt' | 'lastUsedAt' | 'revokedAt' | 'revokedReason'>): Promise<AuthSessionRecord> {
    const { data, error } = await this.client().from('auth_sessions').insert({
      user_id: input.userId,
    ...
    if (error || !data) throw new Error(`session creation failed: ${error?.message || 'missing row'}`);
    return mapSession(data);
  }

  async findSessionById(id: string): Promise<AuthSessionRecord | null> {
    const { data, error } = await this.client().from('auth_sessions').select('*').eq('id', id).maybeSingle();
    if (error) throw new Error(`session lookup failed: ${error.message}`);
    return data ? mapSession(data) : null;
  }

  async findSessionByRefreshHash(hash: string): Promise<AuthSessionRecord | null> {
    const { data, error } = await this.client().from('auth_sessions').select('*').eq('refresh_token_hash', hash).maybeSingle();
    if (error) throw new Error(`refresh lookup failed: ${error.message}`);
    return data ? mapSession(data) : null;
  }

  async markSessionReauthenticated(id: string): Promise<void> {
    const { error } = await this.client().from('auth_sessions').update({ last_reauthenticated_at: new Date().toISOString() }).eq('id', id).is('revoked_at', null);
    if (error) throw new Error(`session reauthentication update failed: ${error.message}`);
  }

  async touchSession(id: string): Promise<void> {
    const { error } = await this.client().from('auth_sessions')
      .update({ last_used_at: new Date().toISOString() }).eq('id', id).is('revoked_at', null);
    if (error) throw new Error(`session activity update failed: ${error.message}`);
  }

  async listSessions(userId: string): Promise<AuthSessionRecord[]> {
    const { data, error } = await this.client().from('auth_sessions').select('*').eq('user_id', userId).order('issued_at', { ascending: false });
    if (error) throw new Error(`session list failed: ${error.message}`);
    return (data || []).map(mapSession);
  }

  async revokeSession(id: string, reason: string): Promise<void> {
    const { error } = await this.client().from('auth_sessions').update({ revoked_at: new Date().toISOString(), revoked_reason: reason }).eq('id', id).is('revoked_at', null);
    if (error) throw new Error(`session revocation failed: ${error.message}`);
  }

  async revokeSessionFamily(familyId: string, reason: string): Promise<void> {
    const { error } = await this.client().from('auth_sessions').update({ revoked_at: new Date().toISOString(), revoked_reason: reason }).eq('family_id', familyId).is('revoked_at', null);
    if (error) throw new Error(`session family revocation failed: ${error.message}`);
  }

  async revokeSessions(userId: string, reason: string, exceptId?: string): Promise<void> {
    let query = this.client().from('auth_sessions').update({ revoked_at: new Date().toISOString(), revoked_reason: reason }).eq('user_id', userId).is('revoked_at', null);
  ...
  async createOAuthFlow(input: Omit<OAuthFlowRecord, 'id'>): Promise<void> {
    const { error } = await this.client().from('oauth_authorization_flows').insert({
  ...
  async consumeOAuthFlow(stateHash: string): Promise<OAuthFlowRecord | null> {
    const { data, error } = await this.client().rpc('consume_oauth_authorization_flow', { p_state_hash: stateHash });
    if (error) throw new Error(`OAuth state consumption failed: ${error.message}`);
    return data?.[0] ? mapFlow(data[0]) : null;
  }

  async createNativeExchange(input: Omit<NativeExchangeRecord, 'id'>): Promise<void> {
    const { error } = await this.client().from('oauth_native_exchanges').insert({
    ...
    if (error) throw new Error(`native exchange creation failed: ${error.message}`);
  }

  async consumeNativeExchange(codeHash: string): Promise<NativeExchangeRecord | null> {
    const { data, error } = await this.client().rpc('consume_oauth_native_exchange', { p_code_hash: codeHash });
  ...
  async createPendingRegistration(input: Omit<PendingOAuthRegistration, 'id'>): Promise<void> {
    ...
    await this.client().from('oauth_pending_registrations').delete()
      .eq('provider', input.provider).eq('provider_subject', input.providerSubject).is('consumed_at', null);
    const { error } = await this.client().from('oauth_pending_registrations').insert({
    ...
    if (error) throw new Error(`pending OAuth registration failed: ${error.message}`);
  }

  async consumePendingRegistration(handleHash: string): Promise<PendingOAuthRegistration | null> {
    const { data, error } = await this.client().rpc('consume_oauth_pending_registration', { p_handle_hash: handleHash });
    if (error) throw new Error(`pending OAuth registration consumption failed: ${error.message}`);
  ...
  async createActionToken(input: Omit<AuthActionTokenRecord, 'id'>): Promise<void> {
    ...
    if (error) throw new Error(`auth action token creation failed: ${error.message}`);
  }

  async consumeActionToken(tokenHash: string, purpose: AuthActionTokenRecord['purpose']): Promise<AuthActionTokenRecord | null> {
    const { data, error } = await this.client().rpc('consume_auth_action_token', { p_token_hash: tokenHash, p_purpose: purpose });
    if (error) throw new Error(`auth action token consumption failed: ${error.message}`);
  ...
  async consumeRateLimit(keyHash: string, action: string, limit: number, windowSeconds: number, lockSeconds: number): Promise<RateLimitDecision> {
    ...
    if (error || !data?.[0]) throw new Error(`rate limit failed: ${error?.message || 'missing result'}`);
    return { allowed: Boolean(data[0].allowed), retryAfterSeconds: Number(data[0].retry_after_seconds || 0) };
  }

  async clearRateLimit(keyHash: string, action: string): Promise<void> {
    const { error } = await this.client().from('auth_rate_limits').delete().eq('key_hash', keyHash).eq('action', action);
...
export const authRepository: IAuthRepository = config.dataMode === 'database'
