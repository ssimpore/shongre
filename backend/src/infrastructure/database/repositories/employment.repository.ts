...
export interface EmploymentRepository {
  ...
  findActiveApplication(
    jobId: string,
    candidateId: string,
  ): Promise<EmploymentApplication | null>;
  saveApplication(
...
const text = (job: JobPostingDetail) =>
  `${job.title} ${job.professionLabel} ${job.industryLabel} ${job.employer.name} ${job.requiredSkills.join(" ")} ${job.primaryLocation.city}`.toLocaleLowerCase(
    "fr",
  );
...
const matches = (query: EmploymentSearchQuery, job: JobPostingDetail) => {
  ...
  if (
    query.keywords &&
    !text(job).includes(query.keywords.toLocaleLowerCase("fr"))
  )
    return false;
  if (
  ...
  if (query.industryIds.length && !query.industryIds.includes(job.industryId))
    return false;
  if (
...
const relevance = (query: EmploymentSearchQuery, job: JobPostingDetail) => {
  ...
  if (needle) {
    ...
    if (job.professionLabel.toLocaleLowerCase("fr").includes(needle))
      score += 60;
    if (
  ...
  if (
    ...
    job.primaryLocation.city.toLocaleLowerCase("fr") ===
      query.location.toLocaleLowerCase("fr")
  )
    score += 20;
  return score;
};
...
export class DemoEmploymentRepository implements EmploymentRepository {
  ...
  protected candidateWorkspaces = new Map([
  ...
  protected dataSubjectRequests = new Map<
    string,
    EmploymentDataSubjectRequest
  >();
  ...
  async search(input: EmploymentSearchQuery): Promise<EmploymentSearchResult> {
    ...
    rows.sort((a, b) => {
      if (query.sort === "salary") {
        return (
          (b.salary?.maximum?.amountMinor ||
            b.salary?.minimum?.amountMinor ||
            0) -
          (a.salary?.maximum?.amountMinor ||
            a.salary?.minimum?.amountMinor ||
            0)
        );
      }
  ...
  async countActiveJobs(owner: { ownerUserId?: string; employerId?: string }) {
    if (owner.employerId)
      return Array.from(this.jobs.values()).filter(
        (job) =>
          job.employer.id === owner.employerId &&
  ...
  async saveCandidateProfile(profile: CandidateProfile) {
    ...
    const workspace =
      this.candidateWorkspaces.get(parsed.userId) ||
      clone(EMPLOYMENT_DEMO_CANDIDATE_WORKSPACE);
  ...
  async findActiveApplication(jobId: string, candidateId: string) {
    const found = Array.from(this.applications.values()).find(
      (item) =>
        item.jobId === jobId &&
  ...
  async getRecruiterWorkspace(
    employerId: string,
  ): Promise<RecruiterWorkspace | null> {
  ...
  async isRecruiterMember(userId: string, employerId: string) {
    return (await this.listRecruiterEmployers(userId)).some(
  ...
  async getImportByIdempotency(organizationId: string, key: string) {
    const found = Array.from(this.imports.values()).find(
      (item) =>
        item.organizationId === organizationId && item.idempotencyKey === key,
    );
    return found ? clone(found) : null;
  }
  ...
  async saveJobAlert(alert: JobAlert) {
    ...
    workspace.alerts = [
      clone(parsed),
      ...workspace.alerts.filter((item) => item.id !== parsed.id),
    ];
    return clone(parsed);
  }
  ...
  async saveJobReport(report: EmploymentJobReport) {
    ...
    const existing = Array.from(this.jobReports.values()).find(
      (item) =>
        item.jobId === parsed.jobId &&
        item.reporterUserId === parsed.reporterUserId &&
  ...
  async getAdminOverview(marketCode: string): Promise<EmploymentAdminOverview> {
    return {
      ...
      importErrorCount: Array.from(this.imports.values()).reduce(
  ...
  async updateMarketConfig(
    ...
    if (this.catalog.config.marketCode !== marketCode.toUpperCase())
      throw new Error("Marché Emploi introuvable.");
...
export class PostgresEmploymentRepository extends DemoEmploymentRepository {
  ...
  private async hydrateJobs(rows: any[]): Promise<JobPostingDetail[]> {
    ...
    const labels = new Map(
      (dictionaryResult.data || []).map((entry: any) => [
        entry.id,
        entry.label,
      ]),
    );
    return rows.map((row) => {
      ...
      const primary =
        locations.find((location: any) => location.is_primary) || locations[0];
      if (!employer || !primary)
        throw new Error(
          `Offre Emploi ${row.id} incomplète: employeur ou localisation absent.`,
        );
      const skills = [...(row.skills || [])].sort(
        (a: any, b: any) => a.sort_order - b.sort_order,
      );
      const requiredSkills = skills.filter(
        (skill: any) => skill.requirement === "required",
      );
      const preferredSkills = skills.filter(
        (skill: any) => skill.requirement === "preferred",
      );
      const verified = [
        "domain_verified",
        "manually_verified",
        "provider_verified",
      ].includes(employer.verification_level);
      return jobPostingDetailSchema.parse({
        ...
        contractTypeLabel:
          labels.get(row.contract_type_id) || row.contract_type_id,
        ...
        primaryLocation: {
          ...
          latitude:
            primary.latitude == null ? undefined : Number(primary.latitude),
          longitude:
            primary.longitude == null ? undefined : Number(primary.longitude),
          isPrimary: true,
        ...
        additionalLocations: locations
          ...
          .map((location: any) => ({
            ...
            latitude:
              location.latitude == null ? undefined : Number(location.latitude),
            longitude:
        ...
        screeningQuestions: (row.questions || [])
  ...
  override async search(
    ...
    if (query.location) {
      const safeLocation = query.location
      ...
      if (query.radiusKm && center) {
        ...
        jobIds = Array.from(
          new Set((data || []).map((row: any) => row.job_id)),
        );
      }
      if (!jobIds.length)
        return {
    ...
    if (query.jobFamilyIds.length) {
      ...
      query.professionIds = Array.from(
        new Set([...query.professionIds, ...professions]),
      );
    }
    if (query.languageIds.length) {
      ...
      const languageIds = new Set<string>(
        (data || []).map((row: any) => String(row.job_id)),
      );
      jobIds = (jobIds || Array.from(languageIds)).filter((id) =>
        languageIds.has(id),
      );
      if (!jobIds?.length)
        return {
    ...
    if (query.employerTypeIds.length || query.verifiedEmployerOnly) {
      let employers = this.db()
        .from("employment_employer_profiles")
        .select("id")
        .eq("status", "active");
      if (query.employerTypeIds.length)
        employers = employers.in("employer_type_id", query.employerTypeIds);
      if (query.verifiedEmployerOnly)
        employers = employers.in("verification_level", [
          "domain_verified",
          "manually_verified",
          "provider_verified",
        ]);
    ...
    if (query.industryIds.length)
      statement = statement.in("industry_id", query.industryIds);
    if (query.contractTypeIds.length)
      statement = statement.in("contract_type_id", query.contractTypeIds);
    if (query.workingArrangementIds.length)
      statement = statement.in(
        "working_arrangement_id",
        query.workingArrangementIds,
      );
    if (query.workingTimeIds.length)
      statement = statement.in("working_time_id", query.workingTimeIds);
    if (query.experienceLevelIds.length)
      statement = statement.in(
        "required_experience_id",
        query.experienceLevelIds,
      );
    if (query.educationLevelIds.length)
      statement = statement.in("education_level_id", query.educationLevelIds);
    if (query.scheduleIds.length)
      statement = statement.overlaps("work_schedule_ids", query.scheduleIds);
    if (query.salaryMinimumMinor !== undefined)
      statement = statement
        .gte("salary_maximum_minor", query.salaryMinimumMinor)
        .eq("salary_is_public", true);
    if (query.salaryFrequencyId)
      statement = statement.eq("salary_frequency_id", query.salaryFrequencyId);
    if (query.publishedSince)
      statement = statement.gte("published_at", query.publishedSince);
    if (query.accessibilityOnly)
      statement = statement.not("accessibility_information", "is", null);
    if (query.sort === "salary")
      statement = statement.order("salary_maximum_minor", {
        ascending: false,
        nullsFirst: false,
      });
    else if (query.sort === "deadline")
      statement = statement.order("application_deadline", {
        ascending: true,
        nullsFirst: false,
      });
    ...
    if (result.error) throw result.error;
    const orderedIds: string[] = (result.data || []).map((row: any) =>
      String(row.id),
    );
  ...
  override async saveJob(job: JobPostingDetail, actorUserId?: string) {
    ...
    if (!actorUserId)
      throw new Error(
        "Un auteur est requis pour enregistrer une offre Emploi.",
      );
    const fingerprint = createHash("sha256")
      .update(
        [
          parsed.employer.id,
          parsed.title.trim().toLocaleLowerCase("fr"),
          parsed.professionId,
          parsed.primaryLocation.city.trim().toLocaleLowerCase("fr"),
        ].join("|"),
      )
      .digest("hex");
    const row = {
    ...
    try {
      ...
      const skills = [
        ...parsed.requiredSkillIds.map((skillId, index) => ({
          job_id: parsed.id,
          skill_id: skillId,
          requirement: "required",
          sort_order: index * 10,
        })),
        ...parsed.preferredSkillIds.map((skillId, index) => ({
          job_id: parsed.id,
          skill_id: skillId,
      ...
      if (parsed.screeningQuestions.length) {
        const questionResult = await this.db()
          .from("employment_screening_questions")
          .insert(
    ...
    if (!result)
      throw new Error("L’offre Emploi enregistrée ne peut pas être relue.");
    return result;
  }
  ...
  override async saveDraft(draft: JobDraft) {
    ...
    if (!saved)
      throw new Error("Le brouillon Emploi enregistré ne peut pas être relu.");
    return saved;
  }
  ...
  override async countActiveJobs(owner: {
    ...
    if (owner.employerId)
      statement = statement.eq("employer_id", owner.employerId);
    if (owner.ownerUserId)
      statement = statement.eq("created_by_user_id", owner.ownerUserId);
  ...
  override async findDuplicateJob(input: {
    ...
    const fingerprint = createHash("sha256")
      .update(
        [
          input.employerId,
          input.title.trim().toLocaleLowerCase("fr"),
          input.professionId,
  ...
  override async saveApplication(
    ...
    if (result.error) throw result.error;
    if (expectedStageId && !result.data?.length)
      throw new Error("EMPLOYMENT_STAGE_CONFLICT");
    ...
    if (!saved)
      throw new Error("La candidature enregistrée ne peut pas être relue.");
    return saved;
  }
  ...
  override async getCandidateWorkspace(userId: string) {
    ...
    const [documents, savedRows, applications, consents, alerts] =
      await Promise.all([
        this.db()
          .from("employment_candidate_documents")
          .select("*")
    ...
    for (const result of [documents, savedRows, applications, consents, alerts])
      if (result.error) throw result.error;
    ...
    if (savedIds.length) {
      ...
      savedJobs = savedIds
        ...
        .filter((job: JobPostingDetail | undefined): job is JobPostingDetail =>
          Boolean(job),
        );
    }
    const mappedApplications: EmploymentApplication[] = (
      applications.data || []
    ).map((row: any) => this.mapApplication(row));
    const applicationIds = mappedApplications.map(
      (application: EmploymentApplication) => application.id,
    );
    ...
    if (applicationIds.length) {
      const result = await this.db()
        ...
        .limit(200);
      if (result.error) throw result.error;
      interviews = (result.data || []).map((row: any) =>
    ...
    return {
      profile,
      cvs: (documents.data || [])
        .filter((row: any) => row.document_type === "cv")
        .map((row: any) => ({
          id: row.id,
          candidateId: row.candidate_id,
          label: row.label,
      ...
      applications: mappedApplications.map(
        ({
      ...
      alerts: (alerts.data || []).map((row: any) =>
  ...
  override async getOpenDataSubjectRequest(
    ...
    if (error) throw error;
    return data
      ? employmentDataSubjectRequestSchema.parse({
          id: data.id,
          subjectUserId: data.subject_user_id,
          requestType: data.request_type,
          status: data.status,
  ...
  override async saveJobReport(report: EmploymentJobReport) {
    ...
    const { data, error } = await this.db()
      ...
      .upsert(
        ...
        { onConflict: "job_id,reporter_user_id,reason" },
      )
      .select("*")
      .single();
    if (error) throw error;
  ...
  override async canManageApplications(userId: string, employerId: string) {
    ...
    return Boolean(
      data &&
      (["owner", "recruitment_admin", "recruiter", "hiring_manager"].includes(
        data.role,
      ) ||
  ...
  override async getRecruiterWorkspace(employerId: string) {
    ...
    if (jobIds.length) {
      const result = await this.db()
        ...
        .limit(1000);
      if (result.error) throw result.error;
      applications = (result.data || []).map((row: any) =>
    ...
    if (applicationIds.length) {
      ...
      interviews = (interviewResult.data || []).map((row: any) =>
        interviewSchema.parse({
          id: row.id,
          applicationId: row.application_id,
          modeId: row.mode,
      ...
      recruiterNotes = (noteResult.data || []).map((row: any) =>
        recruiterNoteSchema.parse({
          id: row.id,
          applicationId: row.application_id,
    ...
    const memberResult = await this.db()
      .from("employment_recruiter_memberships")
      .select(
        "id,user_id,role,branch_ids,client_employer_ids,permissions,status",
      )
      .eq("employer_id", employerId)
      .order("created_at", { ascending: true })
      .limit(250);
    ...
    return {
      employer: {
        ...
        isPubliclyVerified: [
          ...
          "manually_verified",
          "provider_verified",
        ].includes(employerRow.verification_level),
      },
      jobs: jobs.map(card),
      applications,
      stages,
  ...
  private async listImportsForOrganization(
    organizationId: string,
    limit = 200,
  ): Promise<EmploymentImport[]> {
    ...
    const bySource = new Map(
      (sources.data || []).map((row: any) => [row.id, row]),
    );
  ...
  override async saveImport(job: EmploymentImport, actorUserId?: string) {
    ...
    if (!employer.data)
      throw new Error("Employeur Emploi introuvable pour cet import.");
      ...
      .upsert(
        {
          ...
          payload_hash: createHash("sha256")
            .update(
              `${parsed.sourceType}:${parsed.sourceIdentifier}:${parsed.idempotencyKey}`,
            )
            .digest("hex"),
          status: parsed.status,
    ...
    const saved = await this.getImportByIdempotency(
      parsed.organizationId,
      parsed.idempotencyKey,
    );
    if (!saved)
      throw new Error("L’import Emploi enregistré ne peut pas être relu.");
    return saved;
  }
  ...
  override async getAdminOverview(
    ...
    const [
      ...
      count("employment_employer_profiles", (query) =>
        query.eq("status", "active"),
      ),
      count("employment_employer_profiles", (query) =>
        query.in("verification_level", [
          "domain_verified",
          "manually_verified",
          "provider_verified",
        ]),
      ...
      count("employment_employer_profiles", (query) =>
        query.eq("employer_type_id", "employment.fr.employer_type.private"),
      ),
      count("employment_jobs", (query) =>
        query.eq("market_code", code).eq("lifecycle", "published"),
      ),
      count("employment_jobs", (query) =>
        query.eq("market_code", code).eq("lifecycle", "pending_review"),
      ),
      count("employment_jobs", (query) =>
        query.eq("market_code", code).eq("lifecycle", "expired"),
      ),
      count("employment_jobs", (query) =>
        query.eq("market_code", code).eq("moderation_status", "flagged"),
      ),
      count("employment_applications", (query) =>
        query.eq("system_state", "received"),
      ),
      count("employment_applications", (query) =>
        query.eq("system_state", "interview"),
      ),
      count("employment_applications", (query) =>
        query.eq("system_state", "offer"),
      ),
      count("employment_applications", (query) =>
        query.eq("system_state", "hired"),
      ),
      count("employment_jobs", (query) =>
        query.eq("market_code", code).eq("moderation_status", "pending"),
      ),
      count("employment_moderation_flags", (query) =>
  ...
  override async updateMarketConfig(
    ...
    if (!actorUserId)
      throw new Error(
        "Un administrateur est requis pour modifier Shongre Emploi.",
      );
    ...
    const mappings: Array<[keyof EmploymentMarketConfig, string]> = [
      ...
      [
        "applicationResubmissionCooldownDays",
        "application_resubmission_cooldown_days",
      ],
    ...
    const update = await this.db()
      .from("employment_market_configs")
      .update(snake)
      .eq("market_code", marketCode);
    ...
    if (patch.isEnabled !== undefined) {
      const activation = await this.db()
        ...
        .update({
          is_active: patch.isEnabled,
          updated_at: new Date().toISOString(),
        })
        .eq("vertical_type", "employment")
        .eq("market_code", marketCode);
  ...
  override async updateOffer(
    ...
    const after = (await this.getCatalog(marketCode, true)).offers.find(
      (offer) => offer.id === offerId,
    )!;
    const audit = await this.db().from("employment_audit_logs").insert({
      actor_user_id: actorUserId,
  ...
  override async trackAnalyticsEvent(event: EmploymentAnalyticsEvent) {
    ...
    for (const key of [
      ...
      delete dimensions[key];
    const result = await this.db()
      .from("employment_analytics_events")
      .insert({
  ...
  override async getCatalog(marketCode: string, includeInactive = false) {
    ...
    if (!activation.data || !config.data)
      throw new Error(
        `Configuration Shongre Emploi absente pour le marché ${code}.`,
      );
    const catalog = employmentCatalogSchema.parse({
      ...
      config: {
        ...
        defaultPublicationDurationDays:
          config.data.default_publication_duration_days,
        ...
        applicationResubmissionCooldownDays:
          config.data.application_resubmission_cooldown_days,
        regulatoryContentVersion: config.data.regulatory_content_version,
        prohibitedLanguagePolicyVersion:
          config.data.prohibited_language_policy_version,
        ...
        entitlements: Object.fromEntries(
          (entitlements.data || [])
            ...
            .map((entry: any) => [
              ...
              parseEntitlementValue(entry.entitlement_value),
            ]),
        ),
        isActive: row.is_active,
