...
export async function bootstrapApp(): Promise<void> {
  ...
  await providerDataDeletionWorker.run().catch((err) => {
    logger.error(`Provider data-deletion worker failed at startup: ${err.message}`);
  });
  await commercialConfigurationWorker.run().catch((err) => {
    logger.error(`Commercial configuration worker failed at startup: ${err.message}`);
  });
  setInterval(() => {
  ...
  setInterval(() => {
    ...
    providerDataDeletionWorker.run().catch((err) => {
      logger.error(`Periodic provider data-deletion worker failed: ${err.message}`);
    });
  }, 60 * 60 * 1000);
