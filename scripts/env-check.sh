...
if [[ ! -f "$SHONGRE_ROOT/.env" && ! -f "$SHONGRE_ROOT/.env.local" ]]; then
  shongre_fail "No root .env or .env.local exists. Run: make env-init"
  exit 1
fi

required=(
  APP_ENV FRONTEND_HOST FRONTEND_PORT BACKEND_HOST BACKEND_PORT EXPO_HOST SUPABASE_HOST
...
ports=(
  FRONTEND_PORT BACKEND_PORT EXPO_METRO_PORT EXPO_WEB_PORT STORYBOOK_PORT
