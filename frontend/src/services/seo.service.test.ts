...
describe("resolveTitle", () => {
  it("brands a page title", () => {
    expect(resolveTitle("Vélos à Bordeaux")).toBe(
      `Vélos à Bordeaux | ${SITE_NAME}`,
    );
  });
...
describe("resolveCanonical", () => {
  ...
  it("drops the query string and hash", () => {
    expect(resolveCanonical("/recherche?query=velo&page=3", origin)).toBe(
      `${origin}/recherche`,
    );
  ...
  it("normalises slashes without collapsing the root", () => {
    expect(resolveCanonical("/annonce/list-117/", origin)).toBe(
      `${origin}/annonce/list-117`,
    );
