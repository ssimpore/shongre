    ...
    <div className="bg-stone-50 rounded-3xl border border-stone-200/60 p-5 sm:p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-black text-stone-900 uppercase tracking-wider">
            {t("profile.sellerTrustIndicators.garantiesSignauxDeConfiance")}
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
        ...
        <div className="flex items-start gap-3 bg-white p-4 rounded-2xl border border-stone-200/60 shadow-2xs hover:shadow-sm transition-shadow">
          ...
          <div>
            <span className="font-bold text-stone-900 block mb-0.5">
              {isPro
                ? isKybVerified
                  ? "Société Vérifiée RCS"
                  : "Identité Pro"
                : isIdentityVerified
                  ? "Identité KYC Certifiée"
                  : "Profil vérifié"}
            </span>
            <span className="text-stone-500 text-xs leading-relaxed">
              {isPro
                ? seller.siret
                  ? `RCS SIRET ${seller.siret.slice(0, 9)} • KBIS Validé`
                  : "Professionnel enregistré"
                : isIdentityVerified
                  ? "Pièce d'identité CNI / Passeport validée"
        ...
        <div className="flex items-start gap-3 bg-white p-4 rounded-2xl border border-stone-200/60 shadow-2xs hover:shadow-sm transition-shadow">
          ...
          <div>
            <span className="font-bold text-stone-900 block mb-0.5">
              {t("profile.sellerTrustIndicators.paiementSecurise")}
            </span>
            <span className="text-stone-500 text-xs leading-relaxed">
              {isBankVerified
        ...
        <div className="flex items-start gap-3 bg-white p-4 rounded-2xl border border-stone-200/60 shadow-2xs hover:shadow-sm transition-shadow">
          ...
          <div>
            <span className="font-bold text-stone-900 block mb-0.5">
              {t("profile.sellerTrustIndicators.reactiviteCertifiee")}
            </span>
            <span className="text-stone-500 text-xs leading-relaxed">
              {seller.responseRatePercent}% de réponses{" "}
