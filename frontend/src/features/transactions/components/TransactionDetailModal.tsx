...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const [meetingDate, setMeetingDate] = useState(
    tx.pickupDetails?.scheduledDate || "",
  );
  const [meetingPlace, setMeetingPlace] = useState(
    tx.pickupDetails?.meetingPlace || "",
  );
  ...
  const handleSellerAccept = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.sellerAcceptReservation(
        tx.id,
        currentUser,
      );
      setTx(updated);
      ...
      setFeedbackMsg({
        type: "error",
        text: err.message || "Erreur lors de l'acceptation.",
      });
    } finally {
  ...
  const handleConfirmSellerReject = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.sellerRejectReservation(
        tx.id,
        currentUser,
      );
      setTx(updated);
      ...
      setFeedbackMsg({
        type: "success",
        text: "Réservation refusée. Remboursement automatique exécuté.",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
  ...
  const handleVerifyPin = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.confirmHandoverWithPin(
        tx.id,
        currentUser,
        sellerInputPin,
      );
      ...
      setFeedbackMsg({
        type: "success",
        text: "Code validé ! Vente clôturée et fonds débloqués.",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
        text: err.message || "Code secret incorrect.",
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const handleConfirmBuyerReceipt = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.confirmBuyerReceipt(
        tx.id,
        currentUser,
      );
      setTx(updated);
      ...
      setFeedbackMsg({
        type: "success",
        text: "Réception confirmée ! Les fonds ont été débloqués au vendeur.",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
        text: err.message || "Erreur de confirmation.",
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const handleShipOrder = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.shipOrder(
        tx.id,
        currentUser,
        trackingInput,
        tx.carrierName,
      ...
      setFeedbackMsg({
        type: "success",
        text: "Expédition enregistrée et acheteur notifié !",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
        text: err.message || "Erreur d'expédition.",
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const handleSaveMeeting = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.updatePickupSchedule(
        tx.id,
        currentUser,
        {
      ...
      setFeedbackMsg({
        type: "success",
        text: "Rendez-vous de remise enregistré !",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
  ...
  const handleConfirmCancelByBuyer = async () => {
    ...
    setFeedbackMsg(null);
    try {
      const updated = await transactionService.cancelReservationByBuyer(
        tx.id,
        currentUser,
      );
      setTx(updated);
      ...
      setFeedbackMsg({
        type: "success",
        text: "Réservation annulée. Remboursement automatique sous 24-48h.",
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: "error",
        text: err.message || "Impossible d'annuler la réservation.",
      });
    } finally {
      setActionLoading(false);
    }
        ...
        return {
          label:
            tx.deliveryMethod === "hand_delivery"
              ...
              : "Réservé - En préparation d'envoi",
    ...
    >
      <div className="space-y-4 text-xs">
          ...
          <div className="p-4 bg-stone-50 border border-stone-200/60 rounded-2xl flex items-center gap-4 shadow-2xs">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-xs font-bold uppercase tracking-wider text-stone-500 block mb-0.5">
                {t("transactions.transactionDetailModal.articleReserve")}
              </span>
        ...
        {isSeller && tx.status === "pending_seller_confirmation" && (
          <div className="p-4 bg-warning-surface border border-warning-border rounded-2xl space-y-3">
            ...
            <p className="text-micro text-warning leading-relaxed">
              L'acheteur a payé {formatPrice(tx.totalAmount)} qui sont
              actuellement garantis sous séquestre. En acceptant, vous vous
              engagez à remettre ou expédier l'article.
            </p>
            <div className="flex gap-2.5 pt-1">
              ...
              >
                {t("transactions.transactionDetailModal.refuserEtRembourser")}
              </Button>
              <Button
                variant="primary"
                size="sm"
              ...
              >
                ...
                {formatPrice(tx.sellerPayoutAmount || tx.amount)} à percevoir)
              </Button>
            </div>
          </div>
        )}
            ...
            <div className="p-4 bg-stone-900 text-white rounded-2xl space-y-3 shadow-xs">
              <div className="flex items-center justify-between">
                ...
                <span className="text-micro bg-success/20 text-emerald-300 font-bold px-2 py-0.5 rounded-full">
                  {t("transactions.transactionDetailModal.securiteMainPropre")}
                </span>
              </div>
              ...
              {isSeller && (
                <div className="space-y-2.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {t("transactions.transactionDetailModal.validerLaRemise")}
                    </Button>
                  </div>
                </div>
              )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-stone-900">
                  Suivi d'expédition ({tx.carrierName})
                </span>
              </div>
              {tx.trackingNumber && (
              ...
              (tx.status === "shipped" || tx.status === "delivered") && (
                <div className="p-3 bg-success-surface border border-success-border rounded-xl space-y-2">
                  <p className="font-bold text-success">
                    {t("transactions.transactionDetailModal.avezVousBienRecuL")}
                  </p>
                  ...
                  >
                    {t("transactions.transactionDetailModal.jAiBienRecuL")}
                  </Button>
                </div>
              )}
          </div>
              ...
              <div className="text-xs text-stone-700 space-y-1">
                <div>
                  <span className="text-stone-500">
                    {t("transactions.transactionDetailModal.datePrevue")}{" "}
                  </span>
                  <span className="font-semibold">
                    {tx.pickupDetails?.scheduledDate || "À définir"}
                  </span>
                </div>
                <div>
                ...
                {(tx.pickupDetails?.sellerPhone ||
                  tx.pickupDetails?.buyerPhone) && (
                  <div>
                    <span className="text-stone-500">
                      {t(
              ...
              <div className="space-y-2 pt-2 text-xs">
                <div>
                  <label className="font-semibold text-stone-700 block mb-1">
                    {t("transactions.transactionDetailModal.dateEtHeure")}
                  </label>
                  <input
                    type="text"
                    value={meetingDate}
                ...
                <div>
                  <label className="font-semibold text-stone-700 block mb-1">
                    {t("transactions.transactionDetailModal.lieuDeRencontre")}
                  </label>
                  <input
                    type="text"
                    value={meetingPlace}
        ...
        <div className="p-5 bg-white border border-stone-200/60 rounded-2xl space-y-2 shadow-2xs font-medium text-sm">
          <span className="font-black text-stone-800 block mb-3">
            {t("transactions.transactionDetailModal.recapitulatifFinancier")}
          </span>
        ...
        {tx.statusHistory && tx.statusHistory.length > 0 && (
          <div className="p-3.5 bg-stone-50 border border-stone-200 rounded-xl space-y-2">
            <span className="font-bold text-stone-800 block">
              {t("transactions.transactionDetailModal.historiqueDuDossier")}
            </span>
            <div className="space-y-2">
        ...
        <div className="pt-2 flex flex-wrap items-center justify-between gap-2 border-t border-stone-100">
          ...
          {isBuyer &&
            (tx.status === "pending_seller_confirmation" ||
              ...
              >
                {t("transactions.transactionDetailModal.annulerMaReservation")}
              </button>
            )}
          ...
          {tx.status === "completed" && !tx.reviewId && isBuyer && (
            ...
            >
              {t("transactions.transactionDetailModal.laisserUneEvaluation")}
            </Button>
          )}
      ...
      {isReviewModalOpen && (
        <LeaveReviewModal
          ...
          onReviewSubmitted={(reviewId) => {
            ...
            setFeedbackMsg({
              type: "success",
              text: "Évaluation publiée avec succès !",
            });
          }}
        />
      )}
      ...
      {isDisputeModalOpen && (
        <DisputeModal
          ...
          onSuccess={(updated) => {
            ...
            setFeedbackMsg({
              type: "success",
              text: "Litige ouvert. Les fonds sous séquestre sont protégés.",
            });
          }}
        />
      )}
