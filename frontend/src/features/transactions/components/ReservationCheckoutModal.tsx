...
> = ({ isOpen, onClose, listing, currentUser, onReservationComplete }) => {
  const { t } = useTranslation();
  const { currentUser: authUser } = useAuth();
  const buyerUser: UserProfile =
    currentUser || authUser || DEMO_USERS.buyer_thomas;

  const [step, setStep] = useState<1 | 2 | 3 | 4>(1);
  const [deliveryMethod, setDeliveryMethod] =
    useState<DeliveryType>("hand_delivery");
  ...
  const [relayPoint, setRelayPoint] = useState(
    "Tabac Presse des Halles (MR-13001)",
  );
  const [meetingNotes, setMeetingNotes] = useState(
    "Disponible en fin de journée ou le week-end",
  );
  ...
  // Price calculations
  const breakdown = transactionService.calculateAmounts(
    listing.price,
    deliveryMethod,
    listing.sellerType,
  );
        ...
        deliveryAddress:
          deliveryMethod !== "hand_delivery"
            ? {
                ...
                city,
                relayPointName:
                  deliveryMethod === "relay_point" ? relayPoint : undefined,
              }
            : undefined,
        pickupDetails:
          deliveryMethod === "hand_delivery"
    ...
    <Modal
      ...
      title={
        step === 4
          ...
          : "Réserver avec Paiement Sécurisé Shongre"
      }
      description={
        step === 4
          ? "Vos fonds sont protégés sous séquestre bancaire jusqu'à la validation de la transaction."
          : "Vos fonds sont bloqués sous séquestre et reversés au vendeur uniquement après votre confirmation."
      }
    >
        ...
        {step < 4 && (
          <div className="flex items-center justify-between border-b border-stone-100 pb-3">
            <div className="flex items-center gap-2">
              <span
                className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-micro ${
                  step >= 1
              ...
              <span
                className={`font-semibold ${step === 1 ? "text-stone-900 font-bold" : "text-stone-500"}`}
              >
                Remise
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`font-semibold ${step === 2 ? "text-stone-900 font-bold" : "text-stone-500"}`}
              >
                {t("transactions.reservationCheckoutModal.detailsCouts")}
              </span>
            </div>
            <div className="w-8 h-px bg-stone-200" />
            <div className="flex items-center gap-2">
              ...
              <span
                className={`font-semibold ${step === 3 ? "text-stone-900 font-bold" : "text-stone-500"}`}
              >
                {t("transactions.reservationCheckoutModal.paiementSequestre")}
              </span>
            </div>
          </div>
        )}
        ...
        {step < 4 && (
          <div className="p-4 bg-stone-50 border border-stone-200/60 rounded-2xl flex items-center gap-4 shadow-2xs">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-black text-sm text-stone-900 truncate mb-1">
                {listing.title}
              </h4>
              <p className="text-stone-500 text-xs font-medium flex items-center gap-2 mb-1">
                <span>
                  {t("transactions.reservationCheckoutModal.vendeur")}{" "}
            ...
            <div className="space-y-2">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-black text-stone-900 shrink-0 whitespace-nowrap">
                    Gratuit
                  </span>
                </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-stone-200 space-y-3 text-sm">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full h-control-md px-3 bg-white border border-stone-200 rounded-control text-stone-900"
                      >
                        <option value="Tabac Presse des Halles (MR-13001)">
                          {t(
                            "transactions.reservationCheckoutModal.tabacPresseDesHalles15",
                          )}
                        </option>
                        <option value="Relais Colis City Express (MR-13002)">
                          Relais Colis City Express (8 bd Longchamp, 13001
                          Marseille)
                        </option>
                        <option value="Épicerie Bio du Vieux-Port (MR-13003)">
                          {t(
                  ...
                  <div className="mt-4 pt-4 border-t border-stone-200 grid grid-cols-2 gap-3 text-sm">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        placeholder={t(
                          "transactions.reservationCheckoutModal.nomEtPrenom",
                        )}
                        className="w-full h-control-md px-3 bg-white border border-stone-200 rounded-control text-stone-900"
                      />
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-5 bg-stone-50 border border-stone-200/60 rounded-2xl space-y-3 text-sm font-medium shadow-inner">
              <div className="flex justify-between text-stone-600">
                <span>
                  {t("transactions.reservationCheckoutModal.prixDeLArticle")}
                </span>
                <span className="font-black text-stone-900">
                  {formatPrice(breakdown.itemPrice)}
                </span>
              </div>
              <div className="flex justify-between text-stone-600 items-center">
                ...
                <span className="font-black text-stone-900">
                  {formatPrice(breakdown.protectionFee)}
                </span>
              </div>
              <div className="flex justify-between text-stone-600">
                <span>
              ...
              <div className="border-t border-stone-200 pt-3 flex justify-between font-black text-stone-900 text-base">
                <span>
                  {t("transactions.reservationCheckoutModal.totalARegler")}
                </span>
          ...
          <div className="space-y-4">
            ...
            {paymentMethod === "card" && (
              <div className="space-y-4 p-5 bg-stone-50 border border-stone-200/60 rounded-2xl shadow-inner text-sm">
                ...
                <div>
                  <label className="font-bold text-stone-700 block mb-1.5">
                    {t("transactions.reservationCheckoutModal.numeroDeCarte")}
                  </label>
            ...
            <div className="flex gap-2.5 pt-2">
              ...
              >
                {isProcessing
                  ...
                  : `Payer et Réserver (${formatPrice(breakdown.totalAmount)})`}
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 text-center pt-2">
            ...
            <div>
              ...
              <p className="text-stone-500 text-xs mt-1">
                {t("transactions.reservationCheckoutModal.referenceDossier")}
              ...
              createdTx.verificationCode && (
                <div className="p-4 bg-warning-surface border border-warning-border rounded-2xl text-left space-y-2">
                  ...
                  <p className="text-micro text-warning leading-relaxed">
                    ...
                    Ne transmettez ce code à 6 chiffres au vendeur qu'une fois
                    sur place après avoir inspecté et validé la conformité de
                    l'article.
                  </p>
                </div>
              )}
            ...
            {createdTx.deliveryMethod !== "hand_delivery" && (
              <div className="p-3.5 bg-info-surface border border-info-border rounded-xl text-left text-info space-y-1">
                <p className="font-bold flex items-center gap-1.5">
                  <Truck className="w-4 h-4 text-info" /> Préparation de votre
                  colis
                </p>
                <p className="text-micro text-info leading-relaxed">
                  Le vendeur a été notifié et dispose de 48h pour valider la
                  réservation et déposer le colis avec l'étiquette{" "}
                  {createdTx.carrierName}. Vous recevrez un numéro de suivi dès
                  l'expédition.
                </p>
              </div>
            )}
            ...
            <div className="pt-2">
              <Button
                ...
                onClick={onClose}
              >
                {t("transactions.reservationCheckoutModal.accederAuSuiviDeMa")}
              </Button>
            </div>
          </div>
        )}
