  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Item Summary Card */}
        <div className="p-3.5 bg-stone-50 rounded-2xl border border-border-base flex items-center justify-between gap-3.5">
          <div className="flex items-center gap-3.5 min-w-0">
            <Image
              src={listing.coverImageUrl}
              alt={listing.title}
              className="w-16 h-16 rounded-xl object-cover shrink-0 border border-border-subtle"
            />
            <div className="min-w-0">
              ...
              <h4 className="text-sm font-bold text-stone-900 truncate mt-0.5">{listing.title}</h4>
        ...
        {step === 'delivery' && (
          <div className="space-y-5">
              ...
              {quotes.map((quote) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right font-black text-xs text-stone-900">
            ...
            {/* Pricing Summary */}
            <div className="p-4 bg-stone-50 rounded-xl border border-border-base space-y-2 text-xs">
              ...
              <div className="pt-2 border-t border-border-subtle flex justify-between text-sm font-black text-stone-900">
                <span>Total à régler</span>
                <span className="text-primary">{formatPrice(pricing.buyerTotal)}</span>
              </div>
            </div>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-xs font-bold transition-all cursor-pointer flex flex-col items-center gap-1 ${
                      selectedPaymentMethod === 'card'
                        ? 'border-primary bg-primary-light/40 ring-1 ring-primary text-primary'
        ...
        {step === 'success' && (
          <div className="text-center py-4 space-y-5">
            ...
            <div>
              <h3 className="text-lg font-black text-stone-900">Achat direct confirmé !</h3>
            ...
            {selectedQuote?.deliveryType === 'hand_delivery' ? (
              <div className="p-4 bg-primary-light/40 border border-primary-border rounded-2xl max-w-sm mx-auto text-left space-y-2">
                <div className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
                  <QrCode className="w-4 h-4" />
                  <span>Code secret de remise en main propre</span>
                </div>
                <div className="text-2xl font-black font-mono text-center tracking-widest text-stone-900 py-1 bg-white rounded-lg border border-border-base">
                  {pinCode}
                </div>
                <p className="text-micro text-stone-600">
                  Communiquez ce code au vendeur lors du rendez-vous uniquement après avoir vérifié le produit.
                </p>
              </div>
            ) : (
              <div className="p-3.5 bg-stone-50 border border-border-base rounded-xl max-w-sm mx-auto text-xs text-stone-600 text-left">
                <span className="font-bold text-stone-900 block mb-0.5">Expédition en cours</span>
