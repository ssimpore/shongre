      ...
      case "verified":
        return (
          <span className="inline-flex items-center gap-1 text-xs font-bold text-success bg-success-surface border border-success-border px-2.5 py-1 rounded-full">
            ...
            {t("verification.verificationCenterPage.verifie")}
          </span>
        );
      case "pending":
        return (
      ...
      case "rejected":
        return (
          <span className="inline-flex items-center gap-1 text-xs font-bold text-danger bg-danger-surface border border-danger-border px-2.5 py-1 rounded-full">
            ...
            {t("verification.verificationCenterPage.refuse")}
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-stone-600 bg-stone-100 border border-stone-200 px-2.5 py-1 rounded-full">
            {t("verification.verificationCenterPage.nonCommence")}
          </span>
        );
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 shrink-0 flex flex-col items-center justify-center min-w-[200px]">
            <div className="text-micro font-bold uppercase tracking-wider text-stone-500 mb-1">
              {t("verification.verificationCenterPage.indiceDeConfiance")}
            </div>
        ...
        {showDemoPresets && (
          <div className="mt-6 pt-6 border-t border-stone-200 animate-in fade-in duration-fast">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
              {[
                {
                  id: "tier_0_unverified",
                  label: "1. Non vérifié",
                  color: "border-stone-200",
                },
                {
                  id: "tier_1_email_only",
                  label: "2. Email seul",
                  color: "border-stone-200",
                },
                {
                  id: "tier_2_phone_verified",
                  label: "3. Téléphone SMS",
                  color: "border-info-border",
                },
                {
                  id: "kyc_pending",
                  label: "4. KYC en cours",
                  color: "border-warning-border",
                },
                {
                  id: "tier_3_kyc_verified",
                  label: "5. KYC Validé (CNI)",
                ...
                {
                  id: "kyb_pending",
                  label: "7. Pro KYB en cours",
                  color: "border-warning-border",
                },
                {
                  id: "tier_4_kyb_verified",
                  label: "8. Pro Certifié KBIS",
                  color: "border-warning-border",
                },
                {
                  id: "kyb_rejected",
      ...
      <div className="flex gap-2 border-b border-stone-200 pb-2">
        ...
        >
          {t("verification.verificationCenterPage.checklistDesVerifications2")}
        </button>
        <button
          type="button"
        ...
        >
          Capacités du compte (
          {Object.values(capabilities).filter(Boolean).length}/9)
        </button>
        <button
          type="button"
      ...
      {activeTab === "overview" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(Object.keys(dimensions) as VerificationDimensionId[]).map(
              (dimKey) => {
                    ...
                    <div className="space-y-3">
                      ...
                      {dim.rejectionReason && (
                        <div className="p-3 rounded-xl bg-danger-surface border border-danger-border text-micro text-danger">
                          <strong>
                            {t(
                              "verification.verificationCenterPage.motifDuRejet",
                            )}
                          </strong>{" "}
                          {dim.rejectionReason}
                        </div>
                    ...
                    <div className="pt-4 mt-3 border-t border-stone-100 flex items-center justify-between">
                      {dim.completedAt ? (
                        <span className="text-micro text-stone-500 font-semibold">
                          Validé le{" "}
                          {new Date(dim.completedAt).toLocaleDateString(
                            "fr-FR",
                          )}
                        </span>
                      ) : (
                        <span className="text-micro text-stone-500 font-semibold">
                          {dim.isRequiredForPro
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item.active ? (
                      <span className="text-micro font-bold text-success bg-success-surface px-2 py-0.5 rounded-md">
                        {t("verification.verificationCenterPage.debloque")}
                      </span>
                    ) : (
                      <span className="text-micro font-semibold text-stone-500 bg-stone-100 px-2 py-0.5 rounded-md">
                        {t("verification.verificationCenterPage.verrouille")}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-micro text-stone-500">
                        {log.notes || "Mise à jour du statut"} • Par{" "}
                        {log.performedBy}
                      </div>
                    </div>
                  </div>
