...
export const ProStorefrontEditorPage: React.FC = () => {
  ...
  const [companyName, setCompanyName] = useState(currentUser?.companyName || 'Atelier Nordique SAS');
  const [siret, setSiret] = useState(currentUser?.sirenSiret || currentUser?.siret || '842 194 883 00019');
  const [bio, setBio] = useState(currentUser?.bio || 'Spécialiste du mobilier scandinave et vintage certifié.');
  const [address, setAddress] = useState('14 rue du Faubourg Saint-Antoine');
  const [city, setCity] = useState(currentUser?.city || 'Paris');
  const [postalCode, setPostalCode] = useState(currentUser?.postalCode || '75011');
  const [phone, setPhone] = useState(currentUser?.phone || '01 42 68 90 12');
  const [website, setWebsite] = useState(currentUser?.websiteUrl || 'https://atelier-nordique.fr');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast.success('Les informations de votre vitrine pro ont été enregistrées.');
  };

  return (
    <form onSubmit={handleSave} className="bg-white rounded-2xl border border-border-base p-6 sm:p-8 space-y-6 shadow-xs">
      <div>
        ...
        <div className="relative h-32 rounded-xl bg-gradient-to-r from-stone-800 to-stone-900 flex items-end p-4 border border-border-base">
          <div className="flex items-center gap-3">
            ...
            <div className="text-white">
              ...
              <div className="text-xs text-stone-300">Boutique officielle Shongre Pro</div>
            </div>
          </div>
        </div>
      </div>
