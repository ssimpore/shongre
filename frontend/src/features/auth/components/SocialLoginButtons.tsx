...
function FacebookMark() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4">
      <path fill="#1877F2" d="M22 12a10 10 0 1 0-11.6 9.9v-7H7.9V12h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.3.2 2.3.2v2.5h-1.3c-1.3 0-1.7.8-1.7 1.6V12h2.8l-.4 2.9h-2.4v7A10 10 0 0 0 22 12Z" />
      <path fill="#fff" d="m15.9 14.9.4-2.9h-2.8v-1.8c0-.8.4-1.6 1.7-1.6h1.3V6.1s-1.2-.2-2.3-.2c-2.3 0-3.8 1.4-3.8 3.9V12H7.9v2.9h2.5v7a10.4 10.4 0 0 0 3.1 0v-7h2.4Z" />
    </svg>
  );
}
...
export function SocialLoginButtons(
  ...
  useEffect(() => {
    let active = true;
    void services.auth.getSocialAuthAvailability()
      .then((result) => {
        if (active) setAvailability(result);
      })
      .catch(() => {
        if (active) setAvailability({ google: false, apple: false, facebook: false, linking: false });
      });
    return () => {
      active = false;
    };
  ...
  const visibleProviders = PROVIDERS.filter((provider) =>
    availability?.[provider.id] !== false && !(props.intent === "link" && availability?.linking === false),
  );
      ...
      <div className="grid gap-2">
        {visibleProviders.map((provider) => (
          <Button
            ...
            disabled={availability === null || (pending !== null && pending !== provider.id)}
