  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(
          res.errorMessage ||
            "Erreur lors de la mise à niveau vers le statut Professionnel.",
        );
      }
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <h3 id={titleId} className="text-xl font-extrabold text-stone-900 mb-1">
          {t("auth.upgradeToProModal.passerEnCompteProfessionnel")}
        </h3>
        <p className="text-xs text-stone-600 mb-5 leading-relaxed">
          {t("auth.upgradeToProModal.conservezToutesVosAnnoncesAvis")}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-warning-surface/70 border border-warning-border/80 text-xs text-warning flex items-start gap-2.5">
            ...
            <div className="leading-relaxed">
              ...
              conformité. Votre badge Vendeur Pro sera délivré dès validation du
              dossier.
            </div>
          </div>
