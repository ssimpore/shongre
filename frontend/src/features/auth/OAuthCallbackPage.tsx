...
type CallbackState = "loading" | "success" | "cancelled" | "link_required" | "email_required" | "error";

function socialProvider(value: string | null): SocialAuthProvider | null {
  return value === "google" || value === "apple" || value === "facebook" ? value : null;
}
...
export function OAuthCallbackPage() {
  ...
  const onboarding = searchParams.get("onboarding") ||
    (searchParams.get("accountType") === "professional" ? "professional" : null);
  const destination = onboarding === "professional"
  ...
  useEffect(() => {
    ...
    if (status !== "authenticated" && status !== "linked" && status !== "success") {
      setView("error");
      return;
    }
    ...
    void (async () => {
      try {
        if (searchParams.get("demo") === "true") {
          if (!provider || !services.auth.completeDemoSocialAuth) throw new Error("invalid_demo_callback");
  ...
  const completeProfile = async (event: React.FormEvent) => {
    ...
    try {
      await services.auth.completeOAuthProfile({
        email: email.trim(),
        accountType: searchParams.get("accountType") === "professional" ? "professional" : "individual",
      });
      navigate("/verification-email", { replace: true });
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t("auth.social.failed"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const message =
    view === "success"
      ? status === "linked" ? t("auth.callback.linked") : t("auth.callback.success")
      : view === "cancelled"
  ...
  return (
    <AuthLayout title={t("auth.callback.title")} subtitle={t("auth.callback.subtitle")}>
      <div aria-live="polite" aria-busy={view === "loading"} className="space-y-5 text-center">
        <div
          className={`mx-auto flex h-14 w-14 items-center justify-center rounded-2xl ${
            view === "success" ? "bg-success-surface text-success" : "bg-stone-100 text-stone-600"
          }`}
        >
          {view === "success" ? <CheckCircle2 className="h-7 w-7" /> : view === "email_required" ? <Mail className="h-7 w-7" /> : <AlertCircle className="h-7 w-7" />}
        </div>
        <p className="text-sm leading-relaxed text-stone-700">{message}</p>

        {view === "email_required" ? (
          <form onSubmit={completeProfile} className="space-y-3 text-left">
            <label htmlFor="oauth-email" className="block text-xs font-bold text-stone-800">
        ...
        {view === "cancelled" || view === "error" ? (
          <Button to="/connexion" variant="primary" className="w-full" rightIcon={<ArrowRight className="h-4 w-4" />}>
            {t("auth.callback.backToLogin")}
          </Button>
        ) : null}
        {view === "link_required" ? (
          <Link className="inline-flex font-bold text-primary underline" to={`/connexion?returnTo=${encodeURIComponent(returnTo)}`}>
