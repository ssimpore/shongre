...
export function AdminSolutionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        reason instanceof Error ? reason.message : "Catalogue non chargé.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const selected =
    solutions.find((solution) => solution.id === selectedId) || null;
  useEffect(() => {
  ...
  return (
    <div className="space-y-4">
      ...
      <section
        aria-label={t("admin.adminSolutionsPage.resumeDuCatalogue")}
        className="grid divide-y divide-border-base rounded-control border border-border-base bg-bg-surface shadow-xs sm:grid-cols-3 sm:divide-x sm:divide-y-0"
      >
        {[
          ...
          [
            Archive,
            `${counts.retired} retirée${counts.retired > 1 ? "s" : ""}`,
          ],
        ].map(([Icon, label]) => (
          <div
            key={String(label)}
      ...
      {loading ? (
        <div className="grid gap-4 xl:grid-cols-5">
          <Skeleton className="h-160 rounded-control xl:col-span-2" />
          <Skeleton className="h-160 rounded-control xl:col-span-3" />
        </div>
      ) : (
        <div className="grid gap-4 xl:grid-cols-5">
          <section
            className="overflow-hidden rounded-control border border-border-base bg-bg-surface shadow-xs xl:col-span-2"
            aria-labelledby="solutions-list-title"
          >
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
                <label className="flex h-control-touch items-center gap-2 rounded-control border border-border-base px-3">
                  ...
                  <span className="sr-only">
                    {t("admin.adminSolutionsPage.rechercherUneSolution")}
                  </span>
                  <input
                    value={query}
                    onChange={(event) => setQuery(event.target.value)}
                    placeholder={t(
                      "admin.adminSolutionsPage.rechercherUneSolution",
                    )}
                    className="min-w-0 flex-1 bg-transparent text-xs outline-none h-control-touch"
                  />
                </label>
                <Select
                  value={filter}
                  onChange={(event) =>
                    setFilter(event.target.value as "ALL" | SolutionLifecycle)
                  }
                  aria-label={t(
                    "admin.adminSolutionsPage.filtrerParCycleDeVie",
                  )}
                >
                  <option value="ALL">
              ...
              <table className="w-full min-w-120 text-left text-xs">
                ...
                <tbody className="divide-y divide-border-subtle">
                  {filtered.map((solution) => (
                    ...
                    >
                      <td
                        className={`border-l-4 px-3 py-4 font-bold text-text-main ${selectedId === solution.id ? "border-primary" : "border-transparent"}`}
                      >
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      <td className="px-3 py-4 text-text-secondary">
                        {solution.markets.join(", ")}
                      </td>
                      <td className="px-3 py-4 font-mono text-micro text-text-secondary">
            ...
            <div className="space-y-4 xl:col-span-3">
              ...
              >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <FormField
                    label={t("publishing.publishWizard.categorie")}
                    required
                  >
                    <Input
                  ...
                  <FormField label={t("admin.adminSolutionsPage.icone")}>
                    <Select
                      labelledByAncestor
                      value={draft.icon}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          icon: event.target
                            .value as SolutionDefinition["icon"],
                        })
                      }
                    >
                  ...
                  <FormField
                    label={t("admin.adminFeatureFlagsPage.cycleDeVie")}
                  >
                    <Select
                      labelledByAncestor
                  ...
                  <FormField
                    label={t("admin.adminSolutionsPage.disponibleAPartirDu")}
                  >
                    <Input
                      type="datetime-local"
                      value={draft.availableFrom?.slice(0, 16) || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                  ...
                  <FormField
                    label={t("admin.adminSolutionsPage.disponibleJusquAu")}
                  >
                    <Input
                      type="datetime-local"
                      value={draft.availableUntil?.slice(0, 16) || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                  ...
                  <FormField
                    label="Langues"
                    hint="Codes BCP 47 séparés par des virgules"
                  >
                    <Input
                  ...
                  <FormField label="Audiences" hint="Séparées par des virgules">
                    <Input
                  ...
                  <FormField label="Entitlement">
                    <Input
                      value={draft.entitlementKey || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          entitlementKey: event.target.value || undefined,
                        })
                      }
                    />
                  </FormField>
                  <FormField label="Destination applicative">
                    <Select
                      labelledByAncestor
                      value={draft.launchApplicationId || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          launchApplicationId: (event.target.value ||
                            undefined) as ShongreApplicationId | undefined,
                        })
                      }
                    >
                  ...
                  <FormField label="Documentation HTTPS">
                    <Input
                      type="url"
                      ...
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          documentationUrl: event.target.value || undefined,
                        })
                      }
                      placeholder="https://documentation.example/…"
                    />
                  </FormField>
                  <FormField
                    label={t("admin.adminSolutionsPage.solutionDeRemplacement")}
                    hint="Slug interne, sans URL"
                  >
                    <Input
                      value={draft.replacementSlug || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          replacementSlug: event.target.value || undefined,
                        })
                      }
                    />
                  </FormField>
                ...
                <FormField
                  label={t("admin.adminSolutionsPage.descriptionComplete")}
                  required
                >
                  <Textarea
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FormField label="Notice publique">
                    <Textarea
                      value={draft.notice || ""}
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          notice: event.target.value || undefined,
                        })
                      }
                    />
                  </FormField>
                  <FormField
                    label={t("admin.adminSolutionsPage.messageDeMaintenance")}
                    hint="Obligatoire avant le passage en maintenance"
                  >
                    <Textarea
                      ...
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          maintenanceMessage: event.target.value || undefined,
                        })
                      }
                    />
                  </FormField>
                  ...
                  <label className="inline-flex min-h-control-touch items-center gap-2 text-xs font-bold">
                    <input
                      ...
                      onChange={(event) =>
                        setDraft({
                          ...draft,
                          requiresAuthentication: event.target.checked,
                        })
                      }
                      className="h-4 w-4 accent-primary"
                    />{" "}
                ...
                <FormField
                  label={t("admin.adminSolutionsPage.cheminDeLancement")}
                  hint="Chemin local uniquement, par exemple /app"
                >
                  <Input
                    value={draft.launchPath || "/"}
                    onChange={(event) =>
                      setDraft({
                        ...draft,
                ...
                <fieldset className="space-y-3 rounded-control border border-border-base p-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <FormField
                      label={t("admin.adminSolutionsPage.titreDeLaNote")}
                    >
                      <Input
                        value={draft.releaseNotes[0]?.title || ""}
                        onChange={(event) =>
                          updateReleaseNote("title", event.target.value)
                        }
                      />
                    </FormField>
                    <FormField
                      label={t("admin.adminSolutionsPage.dateDePublication")}
                    >
                      <Input
                        type="datetime-local"
                        value={
                          draft.releaseNotes[0]?.publishedAt.slice(0, 16) || ""
                        }
                        onChange={(event) =>
                          updateReleaseNote(
                            "publishedAt",
                  ...
                  <FormField
                    label={t("admin.adminSolutionsPage.contenuDeLaNote")}
                  >
                    <Textarea
                      value={draft.releaseNotes[0]?.body || ""}
                      onChange={(event) =>
                        updateReleaseNote("body", event.target.value)
                      }
                    />
                  </FormField>
                  {draft.releaseNotes.length ? (
                    <Button
                      type="button"
                      variant="ghost"
                      onClick={() =>
                        setDraft({
                          ...draft,
                          releaseNotes: draft.releaseNotes.slice(1),
                        })
                      }
                    >
                      {t("admin.adminSolutionsPage.retirerCetteNote")}
                    </Button>
                  ) : null}
                </fieldset>
                <div className="flex flex-wrap justify-end gap-2 border-t border-border-subtle pt-4">
                  ...
                  {!creating ? (
                    <Button
                      ...
                      leftIcon={<History className="h-icon-sm w-icon-sm" />}
                    >
                      Historique
                    </Button>
                  ) : null}
              ...
              {!creating && selected ? (
                <section
                  className="rounded-control border border-border-base bg-bg-surface p-4 shadow-xs sm:p-5"
                ...
                >
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Select
                      value={transition}
                      onChange={(event) =>
                        setTransition(event.target.value as SolutionLifecycle)
                      }
                      aria-label={t(
                        "admin.adminSolutionsPage.nouveauCycleDeVie",
                      )}
                    >
                      {SOLUTION_LIFECYCLES.map((value) => (
                        <option key={value} value={value}>
                    ...
                    <Input
                      ...
                      aria-label={t("admin.monetization.transitionReason")}
                    />
                  </div>
                  <div className="mt-3 flex justify-end">
                    <Button type="button" onClick={requestTransition}>
                      {t("admin.adminSolutionsPage.appliquerLaTransition")}
                    </Button>
                  </div>
                </section>
              ) : null}
      ...
      <Modal
        ...
        title={`Historique — ${selected?.name || "Solution"}`}
        maxWidth="lg"
      >
        {history.length ? (
          ...
          <p className="py-8 text-center text-sm text-text-muted">
            {t("admin.adminSolutionsPage.aucuneTransitionEnregistree")}
          </p>
        )}
      </Modal>
    </div>
