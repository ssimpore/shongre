...
export const AttributeEditModal: React.FC<AttributeEditModalProps> = ({
  ...
  const [publicationGroup, setPublicationGroup] =
    useState<TaxonomyAttribute["publicationGroup"]>("general");
  ...
  useEffect(() => {
    if (attribute) {
      ...
      setOptions(
        attribute.options ? JSON.parse(JSON.stringify(attribute.options)) : [],
      );
    } else {
      setId("");
      setCode("");
  ...
  const handleAddOption = () => {
    setOptions((prev) => [
      ...prev,
      { value: `opt_${prev.length + 1}`, label: `Option ${prev.length + 1}` },
    ]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Attribut "${saved.label}" enregistré dans le registre central.`,
      );
      onSuccess(saved);
      onClose();
    } catch (err: any) {
      toast.error(
        err?.message || "Erreur lors de l'enregistrement de l'attribut.",
      );
    } finally {
  ...
  return (
    <Modal
      ...
      title={
        isEditing
          ? `Éditer l'attribut "${attribute?.label}"`
          : "Créer un nouvel attribut dans le Registre"
      }
      description={t(
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField
            label={t("admin.attributeEditModal.libelleDeLAttributFrancais")}
            required
          >
            <Input
              value={label}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            ...
            >
              ...
              <option value="number">
                {t("admin.attributeEditModal.nombreNumerique")}
              </option>
              <option value="select">
                {t("admin.attributeEditModal.menuDeroulantSelectUnique")}
              </option>
              <option value="multi_select">
                Choix multiples (Multi-select)
              </option>
              <option value="boolean">
                {t("admin.attributeEditModal.booleenOuiNon")}
              </option>
          ...
          <FormField label={t("admin.attributeEditModal.groupeDePublication")}>
            ...
            >
              ...
              <option value="specifications">
                {t("admin.attributeEditModal.specificationsTechniques")}
              </option>
        ...
        <FormField
          label={t("admin.attributeEditModal.texteDAideOuPlaceholder")}
        >
          <Input
            value={helpText}
              ...
              {options.length === 0 ? (
                ...
                options.map((opt, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <Button
                      aria-label={t(
                        "admin.attributeEditModal.retirerCetteOption",
                      )}
                      variant="ghost"
                      size="sm"
                      type="button"
