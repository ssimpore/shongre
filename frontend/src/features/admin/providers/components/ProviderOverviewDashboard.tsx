...
export const ProviderOverviewDashboard: React.FC<ProviderOverviewDashboardProps> = ({
  providers,
  configurations,
  onSelectCategory,
  onNavigateToTab,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <span className="text-2xl font-black text-stone-900">{metrics.total}</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <span className="text-2xl font-black text-stone-900">
              {metrics.degraded === 0 ? '100%' : `${metrics.total - metrics.degraded}/${metrics.total}`}
            </span>
            <span
            ...
            >
              {metrics.degraded === 0 ? 'Tous opérationnels' : `${metrics.degraded} dégradé(s)`}
            </span>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-xs p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {criticalCapabilities.map((item) => {
            const health = providerService.resolveCapabilityHealth(item.capability, 'FR');
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-micro text-stone-500 mb-2">{item.description}</p>
                </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-stone-200 shadow-xs p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {Object.values(PROVIDER_CATEGORIES)
              .filter((c) => c.isCore || providers.some((p) => p.category === c.id))
              .map((cat) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <span className={`px-2 py-1 rounded text-xs font-bold border shrink-0 ${cat.badgeClass}`}>
                        {cat.shortLabel}
                      </span>
        ...
        <div className="bg-white rounded-xl border border-stone-200 shadow-xs p-5">
          ...
          {recentAudit.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentAudit.map((evt) => (
                <div key={evt.id} className="text-xs border-b border-stone-100 pb-2.5 last:border-0 last:pb-0">
                  <div className="flex items-center justify-between text-micro text-stone-500 mb-0.5">
                    <span className="font-semibold text-stone-700">{evt.providerName}</span>
                    <span>{new Date(evt.timestamp).toLocaleDateString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <p className="text-stone-600 text-micro leading-relaxed">{evt.details}</p>
                </div>
              ))}
            </div>
          )}
