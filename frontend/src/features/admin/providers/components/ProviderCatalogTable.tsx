...
export const ProviderCatalogTable: React.FC<ProviderCatalogTableProps> = ({
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "active" | "disabled" | "requires_action"
  >("all");
  const [envFilter, setEnvFilter] = useState<"all" | ProviderEnvironment>(
    "all",
  );
  const [healthFilter, setHealthFilter] = useState<
    "all" | ProviderHealthStatus
  >("all");
  ...
  const filteredProviders = useMemo(() => {
    return providers.filter((p) => {
      ...
      // Environment filter
      if (envFilter !== "all" && config?.environment !== envFilter)
        return false;

      // Health filter
      if (healthFilter !== "all" && config?.health !== healthFilter)
        return false;
          ...
          <div>
            ...
            >
              ...
              <option value="healthy">
                {t("admin.providerCatalogTable.operationnel")}
              </option>
          ...
          <table className="w-full text-left text-xs text-stone-700">
            <thead className="bg-stone-50 text-stone-500 uppercase tracking-wider font-semibold border-b border-stone-200">
              <tr>
                ...
                <th scope="col" className="py-3 px-3">
                  {t("admin.providerCatalogTable.marchesSupportes")}
                </th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3">
                        <span
                          className={`px-2 py-0.5 rounded text-micro font-bold border ${catMeta.badgeClass}`}
                        >
                          {catMeta.shortLabel}
                        </span>
                      </td>
