...
export const ProviderHealthSimulator: React.FC<ProviderHealthSimulatorProps> = ({
  provider,
  configuration,
  onUpdated,
}) => {
  ...
  const handleRunTest = async () => {
    ...
    try {
      const result = await providerService.testProvider(provider.id, selectedScenario);
      setLastTestResult(result);
      if (result.success) {
        toast.success(`Test réussi pour ${provider.name} (${result.latencyMs} ms).`);
      } else {
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 pb-2">
              <span className="font-bold flex items-center gap-2">
                ...
                {lastTestResult.success ? 'RÉSULTAT : SUCCÈS' : 'RÉSULTAT : ÉCHEC'}
              </span>
