...
export const CrmAiProspectingPage: React.FC = () => {
  ...
  const [selectedCandidateForEvidence, setSelectedCandidateForEvidence] = useState<ProspectResearchCandidate | null>(null);

  // Duplicate Conflict Modal state
  const [conflictCandidate, setConflictCandidate] = useState<ProspectResearchCandidate | null>(null);
  ...
  const handleRunSearch = async (queryText?: string) => {
    ...
    if (!activeQuery.trim()) {
      toast.error('Veuillez décrire le type d\'entreprises que vous recherchez.');
      return;
    }
    ...
    setIsSearching(true);
    setSearchStep('Recherche dans les annuaires et sites professionnels publics...');

    try {
      setTimeout(() => setSearchStep('Analyse des catalogues et signaux d\'activité...'), 200);
      setTimeout(() => setSearchStep('Calcul du Fit Shongre & déduplication...'), 450);
      ...
      setSelectedCandidateIds(updatedCandidates.filter((c) => !c.isDuplicate).map((c) => c.id));
    } catch (err: any) {
  ...
  const handleImportSingle = async (candidate: ProspectResearchCandidate) => {
    if (candidate.isDuplicate) {
      const existingCompanies = await crmRepository.listCompanies();
      const dup = crmService.detectDuplicate(candidate.company, existingCompanies);
    ...
    try {
      ...
      toast.success(`${candidate.company.name} a été ajouté au CRM.`, 'Prospect importé');
    } catch (err: any) {
  ...
  const handleBatchImport = async () => {
    ...
    const candidatesToImport = results.candidates.filter(
      (c) => selectedCandidateIds.includes(c.id) && !importedCandidateIds.includes(c.id)
    );
    ...
    setImportedCandidateIds((prev) => [...prev, ...candidatesToImport.map((c) => c.id)]);
    toast.success(`${candidatesToImport.length} prospect(s) importé(s) dans le CRM.`, 'Import par lot réussi');
  };
          ...
          >
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                ...
                <span>{isSearching ? 'Recherche en cours...' : 'Lancer la prospection'}</span>
              </Button>
            </div>

            {/* Example Prompts Chips */}
            <div className="flex items-center gap-2 flex-wrap text-xs text-stone-500 pt-1">
              <span className="font-bold text-stone-500 text-micro">Exemples :</span>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
            ...
            {selectedCandidateIds.length > 0 && (
              ...
              >
                ...
                <span>Ajouter les {selectedCandidateIds.length} sélectionnés au CRM</span>
              </Button>
            )}
          </div>
                ...
                >
                  ...
                  <div className="pt-3 border-t border-border-subtle flex items-center justify-between gap-2">
                    ...
                    {isImported ? (
                      ...
                      >
                        {cand.isDuplicate ? 'Examiner doublon' : 'Ajouter au CRM'}
                      </Button>
                    )}
                  </div>
                </div>
      ...
      {conflictCandidate && (
        <DuplicateConflictModal
          ...
          onAssociate={async () => {
            ...
            toast.success('Recherche associée à la fiche entreprise existante.', 'Association réussie');
          }}
          onCreateSeparate={async () => {
            ...
            setConflictCandidate(null);
            toast.success('Fiche distincte créée dans le CRM.', 'Prospect créé');
          }}
        />
      )}
    </div>
