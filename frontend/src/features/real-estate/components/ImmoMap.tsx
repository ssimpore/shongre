  ...
  useEffect(() => {
    ...
    properties.forEach((property, index) => {
      ...
      const icon = L.divIcon({
        ...
        html: `<button type="button" aria-label="Afficher le bien ${index + 1}" class="grid h-9 w-9 place-items-center rounded-full border-2 border-white bg-primary text-xs font-black text-white shadow-card ${selected ? "ring-4 ring-primary-border scale-110" : ""}">${index + 1}</button>`,
        iconSize: [36, 36],
        iconAnchor: [18, 18],
      });
