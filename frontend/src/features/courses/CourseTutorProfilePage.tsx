...
import type { CoursePublicOffer, TutorPublicProfile } from "@shongre/contracts/courses";
...
function VerificationRow({
  ...
  return (
    <li className="flex items-center justify-between gap-3 py-2 text-xs">
      ...
      >
        {verified && <CheckCircle2 className="h-icon-xs w-icon-xs" aria-hidden="true" />}
...
export const CourseTutorProfilePage: React.FC = () => {
  ...
  usePageMeta({
    title: tutor ? `${tutor.displayName} — Professeur particulier` : "Profil professeur",
    description:
      tutor?.headline ||
  ...
  const hourlyPrice = primaryOffer?.pricingOptions.find((price) => price.type === "hourly");
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <main className="min-w-0 space-y-5">
            ...
            <div className="grid gap-5 p-5 sm:grid-cols-[10rem_minmax(0,1fr)] sm:p-6">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {tutor.organizationId && <Badge variant="pro">Organisme</Badge>}
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-text-secondary">
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    Répond en moyenne en {Math.max(1, Math.round((tutor.responseTimeMinutes || 60) / 60))} h
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <Languages className="h-icon-xs w-icon-xs" aria-hidden="true" />
                    {tutor.languages.map((language) => language.toUpperCase()).join(", ")}
                  </span>
                </div>
                {tutor.ratingIsStatisticallyMeaningful && tutor.rating ? (
                  <p className="mt-4 flex items-center gap-1.5 text-xs text-text-secondary">
                    <Star className="h-icon-sm w-icon-sm fill-warning text-warning" aria-hidden="true" />
                    <strong className="text-text-main">{tutor.rating}</strong>
                    <span>· {tutor.reviewCount} avis issus d’interactions vérifiées</span>
                  </p>
                ) : (
          ...
          <section className="rounded-card border border-border-base bg-bg-surface p-5 shadow-xs sm:p-6">
            ...
            <h3 className="mt-5 text-sm font-bold text-text-main">Approche pédagogique</h3>
          ...
          <section className="rounded-card border border-border-base bg-bg-surface shadow-xs">
            <div className="flex items-center justify-between border-b border-border-subtle px-5 py-4">
              <h2 className="flex items-center gap-2 text-base font-black text-text-main">
                <BookOpen className="h-icon-md w-icon-md text-primary" aria-hidden="true" />
                Cours proposés
              </h2>
              <span className="text-xs text-text-muted">{offers.length} offre{offers.length > 1 ? "s" : ""}</span>
            </div>
            <div className="divide-y divide-border-subtle">
              {offers.map((offer) => {
                const price = offer.pricingOptions.find((option) => option.type === "hourly");
                return (
                  <article key={offer.id} className="grid gap-4 p-5 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <div>
                      <h3 className="text-sm font-bold text-text-main">{offer.title}</h3>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-micro font-semibold text-text-secondary">
                        {offer.deliveryModes.includes("online") && (
                          <span className="inline-flex items-center gap-1 rounded-pill border border-border-base px-2 py-1">
                            <Laptop className="h-icon-xs w-icon-xs" aria-hidden="true" /> En ligne
                          </span>
                        )}
                        {offer.deliveryModes.includes("in_person") && (
                          <span className="inline-flex items-center gap-1 rounded-pill border border-border-base px-2 py-1">
                            <Users className="h-icon-xs w-icon-xs" aria-hidden="true" /> Présentiel
                          </span>
                        )}
                        <span className="inline-flex items-center gap-1 rounded-pill border border-border-base px-2 py-1">
                          <CalendarDays className="h-icon-xs w-icon-xs" aria-hidden="true" /> {offer.availabilitySummary}
                        </span>
                      </div>
                    </div>
                    {price && (
                      <div className="text-left sm:text-right">
                        <p className="text-lg font-black text-text-main">
                          ...
                          <span className="text-xs font-medium text-text-muted"> / h</span>
                        </p>
                        {offer.trialLessonAvailable && (
                          <p className="mt-1 text-xs font-semibold text-success">Séance d’essai proposée</p>
                        )}
                      </div>
                    )}
                  </article>
          ...
          <section className="grid gap-5 md:grid-cols-2">
            <div className="rounded-card border border-border-base bg-bg-surface p-5 shadow-xs">
              <h2 className="flex items-center gap-2 text-sm font-black text-text-main">
                <Award className="h-icon-sm w-icon-sm text-primary" aria-hidden="true" />
                Qualifications
              </h2>
              <div className="mt-3 space-y-3">
                {tutor.qualifications.map((qualification) => (
                  <div key={qualification.id} className="rounded-control border border-border-subtle bg-bg-subtle p-3">
                    ...
                    <Badge
                      className="mt-2"
                      variant={qualification.verificationStatus === "verified" ? "verified" : "neutral"}
                    >
            ...
            <div className="rounded-card border border-border-base bg-bg-surface p-5 shadow-xs">
              ...
              <ul className="mt-2 divide-y divide-border-subtle">
                <VerificationRow label="Adresse e-mail" status={tutor.verifications.email} />
                <VerificationRow label="Téléphone" status={tutor.verifications.phone} />
                <VerificationRow label="Identité" status={tutor.verifications.identity} />
                <VerificationRow label="Qualifications" status={tutor.verifications.qualifications} />
              </ul>
              <div className="mt-4 rounded-control border border-warning-border bg-warning-surface p-3">
                <p className="text-xs font-bold text-text-main">Services à la personne</p>
                <p className="mt-1 text-micro leading-relaxed text-text-secondary">
                  {tutor.taxEligibility.publicWording} Aucun avantage fiscal n’est garanti par cette fiche.
                </p>
              </div>
            </div>
          </section>
        ...
        <aside className="self-start lg:sticky lg:top-24">
          <div className="rounded-card border border-border-base bg-bg-surface p-5 shadow-sm">
            ...
            <p className="mt-2 text-xs leading-relaxed text-text-secondary">
              Échangez dans la messagerie Shongre avant de partager des coordonnées personnelles.
            </p>
          ...
          <div className="mt-4 rounded-card border border-border-base bg-bg-subtle p-4">
            <h2 className="text-xs font-black text-text-main">Premier cours en sécurité</h2>
            <ul className="mt-2 space-y-2 text-xs leading-relaxed text-text-secondary">
              <li>• Pour un mineur, le responsable légal organise l’échange.</li>
