...
import {
  ...
  toPublicTopics,
} from "../../../domains/trending/trending.engine";
import type {
  TrendingQuery,
...
function buildSignals(
  ...
  const views = listings.reduce(
    (sum, listing) => sum + (listing.viewsCount || listing.viewCount || 0),
    0,
  );
  const favorites = listings.reduce(
    (sum, listing) => sum + listing.favoritesCount,
    0,
  );
  const contacts = listings.reduce(
    (sum, listing) => sum + listing.contactCount,
    0,
  );
  const newlyPublished = listings.filter(
    (listing) => daysSince(listing.createdAt, now) <= 7,
  ).length;
  const recentActivity =
    newlyPublished +
    Math.round(views * (0.05 + hashSeed(`${key}:activity`) * 0.08));
  const previousActivity = Math.max(
    1,
    Math.round(activeListings * (0.7 + hashSeed(`${key}:previous`) * 1.3)),
  );
  const demandGrowth =
    recentActivity + previousActivity === 0
      ? 0
      : Math.min(
          1,
          Math.max(
            0,
            0.5 + (recentActivity - previousActivity) / (2 * previousActivity),
          ),
        );
  const matchingListings = listings.filter((listing) =>
    cityMatches(listing, query.city),
  ).length;
  const conversionRate = views > 0 ? Math.min(1, contacts / views) : 0;
  const lastActivityAt = [...listings].sort(
  ...
  return {
    ...
    searches: Math.round(
      recentActivity * (2.2 + hashSeed(`${key}:searches`) * 2.8),
    ),
    searchClicks: Math.round(
      recentActivity * (1.1 + hashSeed(`${key}:clicks`) * 1.8),
    ),
    favorites,
    ...
    publicationVelocity: Math.min(
      1,
      newlyPublished / Math.max(1, activeListings),
    ),
    demandGrowth,
    supplyGrowth: Math.min(
      1,
      (newlyPublished + 1) / Math.max(2, activeListings),
    ),
    priceActivity: Math.min(
      1,
      listings.filter(
        (listing) =>
          listing.originalPrice && listing.originalPrice > listing.price,
      ).length / Math.max(1, activeListings),
    ),
    lastActivityAt,
    geographicRelevance: query.city
      ? matchingListings > 0
        ? 0.65 + (matchingListings / Math.max(1, activeListings)) * 0.35
        : 0.15
      : 0.45,
...
function buildCandidate(
  ...
  const personalMatches =
    query.personalization?.favoriteListingIds?.filter((id) =>
      listings.some((listing) => listing.id === id),
    ).length || 0;
  const personalizationScore = Math.min(
    1,
    personalMatches / Math.max(1, listings.length) +
      Number(
        query.personalization?.recentCategorySlugs?.includes(key) || false,
      ) *
        0.35,
  );
  const signals = buildSignals(listings, key, query, now);
  const subtitle =
    query.city && signals.geographicRelevance > 0.65
  ...
  return {
    ...
    parentKey: key,
    categorySlug: key,
    image: first.coverImageUrl
      ? { src: first.coverImageUrl, alt: title }
      : undefined,
    signals,
    listings,
...
export function buildDemoTrendingCandidates(
  listings: Listing[],
  query: TrendingQuery,
  now = new Date(),
): TrendingTopicCandidate[] {
  ...
  return Array.from(grouped.values()).map((group) =>
    buildCandidate(group, query, now),
  );
}
