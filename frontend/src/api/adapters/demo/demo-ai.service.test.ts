...
describe('GeminiService - Listing Assistance & Anti-Fraud Safety Analysis', () => {
  it('generates structured listing content with title, description, category and price range', async () => {
    const result = await geminiService.generateListingAssistance({
  ...
  it('detects prohibited items or scam keywords in listing safety audit', async () => {
    ...
    const analysis = await geminiService.analyzeListingSafety(suspiciousListing);
  ...
  it('marks a legitimate listing as compliant with low risk score', async () => {
    ...
    const analysis = await geminiService.analyzeListingSafety(legitListing);
