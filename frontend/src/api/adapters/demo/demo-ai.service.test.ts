...
describe("DemoAiService - Listing Assistance & Anti-Fraud Safety Analysis", () => {
  ...
  it("detects prohibited items or scam keywords in listing safety audit", async () => {
    const suspiciousListing = {
      ...
      description:
        "Paiement uniquement par coupon PCS ou mandat cash western union sans voir le produit.",
      price: 25,
    };

    const analysis =
      await demoAiService.analyzeListingSafety(suspiciousListing);
  ...
  it("marks a legitimate listing as compliant with low risk score", async () => {
    const legitListing = {
      ...
      description:
        "Très belle table basse en chêne des années 70, quelques traces d'usage normales. Remise en main propre sur Bordeaux.",
      price: 120,
    };
