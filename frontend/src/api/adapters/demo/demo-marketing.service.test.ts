...
describe("DemoMarketingService", () => {
  ...
  it("models public double opt-in without browser storage", async () => {
    ...
    const confirmed = await service.confirmPublic(
  ...
  it("uses scoped preference and unsubscribe tokens", async () => {
    ...
    const updated = await service.updatePublicPreferences({
  ...
  it("keeps signed-in account preferences behind account-scoped methods", async () => {
    ...
    expect((await service.getAccountSubscription(identity))?.status).toBe(
      "SUBSCRIBED",
    );
    expect(
      (
    ...
    ).toEqual(["local_trends"]);
    expect((await service.unsubscribeAccount(identity)).status).toBe(
      "UNSUBSCRIBED",
    );
    expect(
      (
        await service.subscribeAccount({
          ...identity,
          topics: ["editorial"],
          consentGiven: true,
        })
  ...
  it("blocks campaigns missing a legal unsubscribe block", async () => {
    ...
    const campaign = await service.createCampaign({
      ...
      audience: {
        includeListIds: ["12000000-0000-4000-8000-000000000001"],
        includeSegmentIds: [],
        includeProfileIds: [],
        excludeListIds: [],
        excludeSegmentIds: [],
    ...
    expect(preflight.blockers.map((item) => item.code)).toContain(
