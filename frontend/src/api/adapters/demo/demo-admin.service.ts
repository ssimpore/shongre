...
export class DemoAdminService implements AdminServiceContract {
  async getPlatformStats(): Promise<AdminStatsSummary> {
    ...
    const activeTxs = [...purchases, ...sales].filter((t) => t.status === 'payment_escrowed' || t.status === 'escrow_secured');
  ...
  async updateUserStatus(userId: string, status: 'active' | 'suspended' | 'banned'): Promise<UserProfile> {
    ...
    const normalizedStatus: AccountStatus = status === 'banned' ? 'suspended' : status;
  ...
  async resolveReport(_reportId: string, _action: 'dismiss' | 'remove_listing' | 'ban_user'): Promise<void> {
    await simulateNetworkDelay();
  }
