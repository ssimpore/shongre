...
export class HttpMessagingService implements MessagingServiceContract {
  ...
  async getConversationById(id: string): Promise<Conversation | null> {
    return httpClient.get<Conversation>(`/messaging/conversations/detail/${id}`);
  }
  ...
  async makeOffer(conversationId: string, senderId: string, senderName: string, amount: number): Promise<Message> {
    return httpClient.post<Message>('/messaging/offer', { conversationId, senderId, senderName, amount });
  }

  async respondToOffer(conversationId: string, userId: string, userName: string, accept: boolean): Promise<Message> {
    return httpClient.post<Message>('/messaging/offer-response', { conversationId, userId, userName, accept });
  }

  async schedulePickup(conversationId: string, date: string, timeSlot: string, address: string): Promise<Message> {
    return httpClient.post<Message>('/messaging/schedule-pickup', { conversationId, date, timeSlot, address });
  }
