...
export class HttpOrdersService implements OrdersServiceContract {
  ...
  async confirmHandoverPIN(orderId: string, enteredPin: string): Promise<{ success: boolean; message: string }> {
    return httpClient.post<{ success: boolean; message: string }>(`/orders/${orderId}/confirm-pin`, { pin: enteredPin });
  }
  ...
  async openDispute(orderId: string, reason: string, details: string): Promise<Transaction> {
    return httpClient.post<Transaction>(`/orders/${orderId}/dispute`, { reason, details });
  }
}
