...
export class HttpPaymentsService implements PaymentsServiceContract {
  async createPaymentIntent(amount: number, currency: string, metadata?: Record<string, string>): Promise<PaymentIntentResult> {
    return httpClient.post<PaymentIntentResult>('/payments/intent', { amount, currency, metadata });
  }

  async requestSellerPayout(sellerId: string, amount: number, iban: string): Promise<{ payoutId: string; status: 'completed' | 'processing' }> {
    return httpClient.post<{ payoutId: string; status: 'completed' | 'processing' }>('/payments/payout', { sellerId, amount, iban });
  }

  async getSellerBalance(sellerId: string): Promise<{ available: number; pending: number; currency: string }> {
    return httpClient.get<{ available: number; pending: number; currency: string }>(`/payments/balance/${sellerId}`);
  }
}
