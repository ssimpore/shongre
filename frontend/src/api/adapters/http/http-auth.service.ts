import { AuthServiceContract, LoginCredentials, RegisterInput } from '../../contracts/auth.contract';
...
export class HttpAuthService implements AuthServiceContract {
  ...
  async login(credentials: LoginCredentials): Promise<UserProfile> {
    const res = await httpClient.post<{ user: UserProfile; token: string }>('/auth/login', credentials);
  ...
  async register(input: RegisterInput): Promise<UserProfile> {
    const res = await httpClient.post<{ user: UserProfile; token: string }>('/auth/register', input);
    if (res.token && typeof localStorage !== 'undefined') {
      localStorage.setItem('shongre_auth_token', res.token);
    }
    return res.user;
  ...
  async verifyPhone(phone: string, code: string): Promise<boolean> {
    const res = await httpClient.post<{ verified: boolean }>('/auth/verify-phone', { phone, code });
    return res.verified;
  }

  async verifyEmail(token: string): Promise<boolean> {
    const res = await httpClient.post<{ verified: boolean }>('/auth/verify-email', { token });
    return res.verified;
  }
}
