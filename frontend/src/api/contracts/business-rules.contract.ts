...
export interface BusinessRulesServiceContract {
  ...
  validatePromotion(request: PromotionValidationRequest): Promise<PromotionValidationResult>;
  getActiveEntitlements(): Promise<ActiveEntitlement[]>;
  getSubscriptions(): Promise<MonetizationSubscription[]>;
  updateSubscriptionCancellation(request: SubscriptionCancellationRequest): Promise<MonetizationSubscription>;
  getAdminOverview(marketCode?: string): Promise<MonetizationAdminOverview>;
  createDraft(patch: CommercialDraftPatch): Promise<CommercialConfigurationVersion>;
  transitionVersion(
    versionId: string,
