...
describe('Shongre API Service Contracts & Demo Adapters', () => {
  ...
  it('retrieves canonical taxonomy categories and attributes', async () => {
    ...
    expect(rootCats.some((c) => c.id === 'vehicles' || c.slug === 'vehicules')).toBe(true);
  });

  it('looks up companies by SIRET deterministically', async () => {
    const company = await services.verification.lookupCompanyBySiret('98765432100012');
  ...
  it('formats normalized AppErrors into user-friendly messages', () => {
    ...
    expect(getFriendlyErrorMessage(authErr)).toBe('Veuillez vous connecter pour effectuer cette action.');

    const kycErr = new AppError({
      code: 'KYC_REQUIRED',
      message: 'Verification required',
    });
    expect(getFriendlyErrorMessage(kycErr)).toBe('Une vérification d’identité est requise pour finaliser cette transaction.');
  });
});
