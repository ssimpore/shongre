...
export interface MarketingServiceContract {
  subscribePublic(input: MarketingPublicSubscriptionInput): Promise<MarketingSubscriptionReceipt>;
  ...
  updatePublicPreferences(input: MarketingPublicPreferencesUpdate): Promise<MarketingSubscriptionView>;
  unsubscribePublic(token: string): Promise<MarketingSubscriptionReceipt>;
  getAccountSubscription(identity: MarketingAccountIdentity): Promise<MarketingSubscriptionView | null>;
  subscribeAccount(input: MarketingAccountSubscriptionInput): Promise<MarketingSubscriptionView>;
  updateAccountPreferences(input: MarketingAccountIdentity & { topics: string[] }): Promise<MarketingSubscriptionView>;
  unsubscribeAccount(identity: MarketingAccountIdentity): Promise<MarketingSubscriptionView>;
  getDashboard(): Promise<MarketingDashboard>;
  listProfiles(options?: { limit?: number; cursor?: string; query?: string; status?: string }): Promise<MarketingPage<MarketingProfile>>;
  ...
  estimateAudience(audience: MarketingAudienceDefinition): Promise<MarketingAudienceEstimate>;
  preflight(id: string): Promise<MarketingPreflight>;
  testSend(id: string, recipient: string): Promise<{ externalMessageId: string; acceptedAt: string }>;
  send(id: string): Promise<{ campaign: MarketingCampaign; queuedRecipients: number; excludedRecipients: number }>;
  ...
  selectExperimentWinner(id: string, variantId?: string): Promise<MarketingCampaign>;
  cancel(id: string): Promise<MarketingCampaign>;
  listSuppressions(): Promise<MarketingSuppression[]>;
  generateCampaignDraft(instructions: string, locale?: string): Promise<AiGenerationResult>;
  aiAssist(input: MarketingAiAssistInput): Promise<AiGenerationResult & { draftOnly: true }>;
  getAnalytics(campaignId?: string): Promise<MarketingAnalytics>;
  recordConversion(input: MarketingConversionInput): Promise<{ accepted: true; duplicate: boolean }>;
  ...
  listJourneyExecutions(journeyId?: string): Promise<MarketingJourneyExecution[]>;
  listWebhookSubscriptions(): Promise<MarketingWebhookSubscription[]>;
  createWebhookSubscription(input: MarketingWebhookSubscriptionInput): Promise<{ subscription: MarketingWebhookSubscription; signingSecret: string }>;
}
