...
describe('VerificationService - KYC / KYB / Payout & Trust Score', () => {
  ...
  describe('Identity Verification (KYC)', () => {
    ...
    it('approves identity verification and marks identity as verified', () => {
      ...
      const approval = verificationService.reviewIdentityVerification(mockUser.id, 'approve', {
    ...
    it('rejects identity verification and stores rejection reason', () => {
      ...
      const rejection = verificationService.reviewIdentityVerification(mockUser.id, 'reject', {
      ...
      expect(rejection.user?.identityVerification?.rejectionReason).toBe('Document flou et illisible.');
    });
  });
  ...
  describe('Business Verification (KYB)', () => {
    ...
    it('approves business verification and sets pro certification', () => {
      ...
      const approval = verificationService.reviewBusinessVerification(mockUser.id, 'approve', {
  ...
  describe('Bank Payout Setup (IBAN)', () => {
    it('validates and registers bank payout details for escrow withdrawals', () => {
      ...
      expect(res.user?.bankPayoutVerification?.iban).toBe('FR7630004000011234567890143');
    });
