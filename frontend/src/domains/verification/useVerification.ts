...
export function useVerification() {
  ...
  const submitKyc = useCallback(
    async (data: KycSubmissionData, instantApprove = false) => {
      ...
      const res = verificationService.submitIdentityVerification(currentUser.id, data, instantApprove);
      if (res.success) {
        refreshUser?.();
      }
      return res;
  ...
  const submitKyb = useCallback(
    async (data: KybSubmissionData, instantApprove = false) => {
      if (!currentUser) throw new Error('Utilisateur non connecté');
      const res = verificationService.submitBusinessVerification(currentUser.id, data, instantApprove);
      if (res.success) {
        refreshUser?.();
      }
      return res;
  ...
  const submitBankPayout = useCallback(
    async (data: BankPayoutSubmissionData) => {
      if (!currentUser) throw new Error('Utilisateur non connecté');
      const res = verificationService.submitBankPayoutVerification(currentUser.id, data);
      if (res.success) {
        refreshUser?.();
      }
      return res;
  ...
  const setPreset = useCallback(
    (
      ...
      const res = verificationService.setDemoVerificationState(currentUser.id, preset);
      refreshUser?.();
      return res;
    },
    [currentUser, refreshUser]
