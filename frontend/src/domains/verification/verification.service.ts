...
export class VerificationService {
  ...
  public getAuditLogs(userId?: string): VerificationAuditEntry[] {
    const all = storageService.get<VerificationAuditEntry[]>(VERIFICATION_AUDIT_KEY, []);
    if (userId) {
  ...
  public logAudit(entry: Omit<VerificationAuditEntry, 'id' | 'timestamp'>): VerificationAuditEntry {
  ...
  // -------------------------------------------------------------
  public getUserVerificationSummary(user: UserProfile | null): UserVerificationSummary {
    if (!user) {
      return {
    ...
    const emailState: VerificationState = user.isEmailVerified ? 'verified' : 'not_started';
    const phoneState: VerificationState = user.isPhoneVerified ? 'verified' : (user.phone ? 'pending' : 'not_started');
    ...
    // MFA state
    const mfaState: VerificationState = (user.mfaEnabled || user.mfa?.isEnabled) ? 'verified' : 'not_started';
    ...
    const dimensions: Record<VerificationDimensionId, VerificationRequirement> = {
      email: {
        ...
        label: 'Adresse email',
        shortLabel: 'Email',
        description: 'Vérification de réception pour les notifications de commandes et messages.',
        state: emailState,
        isRequiredForPro: true,
        isRequiredForHighValue: true,
        completedAt: user.isEmailVerified ? (user.createdAt || new Date().toISOString()) : undefined,
        actionLabel: emailState === 'verified' ? 'Vérifié' : 'Confirmer mon email',
      },
      phone: {
        id: 'phone',
        label: 'Numéro de téléphone portable (SMS)',
        shortLabel: 'Téléphone',
        description: 'Authentification 2FA par SMS pour la sécurisation des remises en main propre.',
        state: phoneState,
        isRequiredForPro: true,
        isRequiredForHighValue: true,
        completedAt: user.isPhoneVerified ? new Date().toISOString() : undefined,
        actionLabel: phoneState === 'verified' ? 'Certifié' : 'Vérifier par SMS',
      },
      identity: {
        id: 'identity',
        label: 'Identité personnelle (KYC)',
        shortLabel: 'Pièce d\'identité',
        description: 'Contrôle officiel de la CNI, du Passeport ou du Titre de séjour (norme ACPR).',
        state: identityState,
        isRequiredForPro: true,
        isRequiredForHighValue: true,
        completedAt: user.identityVerification?.verifiedAt,
        rejectionReason: user.identityVerification?.rejectionReason,
        actionLabel: identityState === 'verified' ? 'Identité validée' : (identityState === 'pending' ? 'Examen en cours' : 'Vérifier mon identité'),
      },
      business: {
        id: 'business',
        label: 'Entreprise & Immatriculation (KYB)',
        shortLabel: 'Extrait KBIS / SIREN',
        description: 'Vérification légale du registre du commerce et des sociétés (RCS/INSEE).',
        state: businessState,
        isRequiredForPro: true,
        ...
        actionLabel: businessState === 'verified' ? 'Boutique certifiée' : (businessState === 'pending' ? 'Dossier en revue' : 'Valider mon entreprise'),
      },
      bank_payout: {
        id: 'bank_payout',
        label: 'Coordonnées bancaires (IBAN SEPA)',
        shortLabel: 'Compte de virement',
        description: 'Vérification du compte bancaire récepteur pour le virement des fonds de séquestre.',
        ...
        actionLabel: bankPayoutState === 'verified' ? 'IBAN vérifié' : 'Ajouter un IBAN',
      },
      mfa: {
        id: 'mfa',
        label: 'Double authentification (2FA)',
        shortLabel: 'Sécurité 2FA',
        description: 'Protection renforcée du compte contre les tentatives d\'accès non autorisées.',
        state: mfaState,
    ...
    } else if (identityState === 'verified' && (phoneState === 'verified' || bankPayoutState === 'verified')) {
    ...
    const capabilities: MarketplaceCapabilityStatus = {
      ...
      canAccessProStorefront: user.accountType === 'professional' && isBusinessOk,
    };
    ...
    const pendingReviewsCount = [identityState, businessState, bankPayoutState].filter((s) => s === 'pending').length;
  ...
  public computeTrustScore(user: UserProfile | null): {
    ...
    return {
      ...
      breakdown: {
        ...
        bank_payout: summary.dimensions.bank_payout.state === 'verified' ? 10 : 0,
  ...
  public submitIdentityVerification(
    ...
    const previousState = (user.identityVerification?.status || 'not_started') as VerificationState;
    const nextState: VerificationState = instantApprove ? 'verified' : 'pending';
    ...
    user.identityVerification = {
      ...
      notes: instantApprove ? 'Validation biométrique instantanée réussie.' : 'Dossier soumis pour examen de conformité.',
    };
  ...
  public reviewIdentityVerification(
    ...
    const now = new Date().toISOString();
    const previousState = (user.identityVerification?.status || 'pending') as VerificationState;
    ...
    return {
      success: true,
      message: decision === 'approve' ? 'Identité approuvée avec succès.' : 'Statut de l\'identité mis à jour.',
      user,
    };
  }
  ...
  public submitBusinessVerification(
    ...
    const now = new Date().toISOString();
    const previousState = (user.professionalVerification?.status || 'not_started') as VerificationState;
    const nextState: VerificationState = instantApprove ? 'verified' : 'pending';

    const calculatedVat = data.vatNumber?.trim() || calculateVatNumber(cleanSiret, data.country || 'FR');
    ...
    user.storeSlug = user.storeSlug || data.companyName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

    user.professionalVerification = {
      status: nextState,
      submittedAt: now,
      reviewedAt: instantApprove ? now : undefined,
      reviewedBy: instantApprove ? 'Automated INSEE / Sirene Validation' : undefined,
      documentType: 'kbis',
      companyName: data.companyName.trim(),
      siret: cleanSiret,
      legalForm: data.legalForm,
      vatNumber: calculatedVat,
      notes: instantApprove ? 'Immatriculation validée automatiquement.' : 'Dossier KBIS soumis à examen.',
    };
  ...
  public reviewBusinessVerification(
    ...
    const now = new Date().toISOString();
    const previousState = (user.professionalVerification?.status || 'pending') as VerificationState;
    let nextState: VerificationState = 'verified';
    ...
    return {
      success: true,
      message: decision === 'approve' ? 'Dossier entreprise approuvé avec succès.' : 'Statut KYB mis à jour.',
      user,
    };
  }
  ...
  public submitBankPayoutVerification(
    ...
    const now = new Date().toISOString();
    const previousState = (user.bankPayoutVerification?.status || 'not_started') as VerificationState;
    ...
    return {
      success: true,
      message: 'Coordonnées bancaires enregistrées et validées pour vos virements de séquestre.',
      user,
    };
  }
  ...
  public lookupCompanyBySiret(input: string): CompanyRegistryResult | null {
    ...
    if (preset) {
      return {
        ...
        nafLabel: preset.nafLabel || 'Vente à distance sur catalogue spécialisé',
    ...
    switch (preset) {
      ...
      case 'kyc_rejected':
        ...
        user.identityVerification = {
          ...
          documentType: 'national_id',
          rejectionReason: 'Document illisible ou reflets masquant la bande MRZ.',
          notes: 'Veuillez reprendre une photo nette et non tronquée de votre document original.',
        };
        break;
      ...
      case 'kyb_rejected':
        ...
        user.professionalVerification = {
          ...
          rejectionReason: 'Numéro SIRET radié ou non concordant avec la raison sociale déclarée.',
          notes: 'Merci de fournir un extrait KBIS à jour datant de moins de 3 mois.',
        };
        break;
  ...
  // Helpers
  // -------------------------------------------------------------
  private getDefaultDimensions(): Record<VerificationDimensionId, VerificationRequirement> {
    return {
      email: {
        id: 'email',
        label: 'Adresse email',
