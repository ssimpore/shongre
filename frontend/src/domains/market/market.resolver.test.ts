...
describe('Multi-Market Inheritance Engine (MarketResolver)', () => {
  ...
  beforeEach(() => {
    ...
    franceMarket = JSON.parse(JSON.stringify(INITIAL_MARKETS.find((m) => m.code === 'FR')!));
    belgiumMarket = JSON.parse(JSON.stringify(INITIAL_MARKETS.find((m) => m.code === 'BE')!));
    spainMarket = JSON.parse(JSON.stringify(INITIAL_MARKETS.find((m) => m.code === 'ES')!));
  });

  describe('Deep Merge & Override Semantics', () => {
    it('inherits unconfigured properties from France baseline', () => {
      const config = resolver.resolveEffectiveConfig(belgiumMarket, franceMarket);
    ...
    it('strictly preserves explicit 0 override and does not fall back', () => {
      ...
      const resolved = resolver.resolveEffectiveConfig(customMarket, franceMarket);
    ...
    it('dynamically propagates updates to France to inheriting markets without duplicating state', () => {
      ...
      let beConfig = resolver.resolveEffectiveConfig(belgiumMarket, franceMarket);
    ...
    it('resets an override by deleting the local delta and resumes dynamic inheritance', () => {
      ...
      expect(belgiumMarket.overrides.payments?.buyerProtectionFixedFee).toBe(0.80);
      let beConfig = resolver.resolveEffectiveConfig(belgiumMarket, franceMarket);
      expect(beConfig.payments.buyerProtectionFixedFee).toBe(0.80);

      // 2. Delete the override
      deleteNestedValue(belgiumMarket.overrides, 'payments.buyerProtectionFixedFee');

      // 3. Belgium now immediately resolves to France standard (0.70)
      beConfig = resolver.resolveEffectiveConfig(belgiumMarket, franceMarket);
      expect(beConfig.payments.buyerProtectionFixedFee).toBe(0.70);
    });
  });
  ...
  describe('Nested Object and Granular Sibling Overrides', () => {
    it('allows overriding a single sibling leaf without overwriting other sibling keys', () => {
      ...
      const config = resolver.resolveEffectiveConfig(customMarket, franceMarket);
      expect(config.monetization.boostPricing.urgent).toBe(9.99); // Overridden
      expect(config.monetization.boostPricing.highlight).toBe(7.90); // Inherited from FR
      expect(config.monetization.boostPricing.top_of_list).toBe(3.90); // Inherited from FR
      expect(config.monetization.boostPricing.gallery_boost).toBe(6.90); // Inherited from FR
    });
  });
  ...
  describe('Provenance and Setting Resolution', () => {
    it('correctly tracks provenance for inherited vs overridden settings', () => {
      ...
      expect(overriddenRes.frenchReferenceValue).toBe(0.20);
    });

    it('calculates inheritance metrics accurately', () => {
      const frMetrics = resolver.getInheritanceMetrics(franceMarket, franceMarket);
      expect(frMetrics.percentInherited).toBe(0);
      expect(frMetrics.percentOverridden).toBe(100);

      const beMetrics = resolver.getInheritanceMetrics(belgiumMarket, franceMarket);
      expect(beMetrics.percentInherited).toBeGreaterThan(80); // Over 80% inherited
      expect(beMetrics.inheritedFieldsCount).toBeGreaterThan(beMetrics.overriddenFieldsCount);
    });
    ...
    it('calculates impacted markets when a French setting is modified', () => {
      ...
      const impactedReservation = resolver.getImpactedMarkets('reservation.enabled', allMarkets);
