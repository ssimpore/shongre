...
describe("Multi-Market Inheritance Engine (MarketResolver)", () => {
  ...
  describe("Nested Object and Granular Sibling Overrides", () => {
    it("allows overriding a single sibling leaf without overwriting other sibling keys", () => {
      ...
      expect(config.monetization.boostPricing.highlight).toBe(7.9); // Inherited from FR
      expect(config.monetization.boostPricing.top_of_list).toBe(3.9); // Inherited from FR
      expect(config.monetization.boostPricing.gallery_boost).toBe(6.9); // Inherited from FR
    });
  });
