...
export class PublicationService {
   ...
   */
  validateDraft(draft: Partial<PublicationDraftState>, user?: UserProfile | null): ValidationResult {
    ...
    if (!draft.taxonomyNodeId) {
      errors.push({ field: 'taxonomyNodeId', code: 'REQUIRED', message: 'Veuillez sélectionner une catégorie.' });
    } else {
      const node = taxonomyService.getNode(draft.taxonomyNodeId);
      if (!node) {
        errors.push({ field: 'taxonomyNodeId', code: 'INVALID_NODE', message: 'La catégorie sélectionnée est invalide.' });
      } else if (!taxonomyService.isPublishable(node.id)) {
        errors.push({
          field: 'taxonomyNodeId',
          code: 'NOT_PUBLISHABLE',
          message: 'Veuillez sélectionner une sous-catégorie finale (feuille de l\'arborescence).',
        });
      }
    }
    ...
    if (!draft.title || draft.title.trim().length < 3) {
      errors.push({ field: 'title', code: 'TITLE_TOO_SHORT', message: 'Le titre doit comporter au moins 3 caractères.' });
    } else if (draft.title.trim().length > 120) {
      errors.push({ field: 'title', code: 'TITLE_TOO_LONG', message: 'Le titre ne doit pas dépasser 120 caractères.' });
    }
    ...
    if (!draft.photos || draft.photos.length === 0) {
      errors.push({ field: 'photos', code: 'PHOTOS_REQUIRED', message: 'Veuillez ajouter au moins une photo.' });
    }

    // 4. Location
    if (!draft.location?.city || !draft.location?.postalCode) {
      errors.push({ field: 'location', code: 'LOCATION_REQUIRED', message: 'La localisation (ville et code postal) est requise.' });
    }

    // 5. Pricing & Stock
    const pricing = draft.pricing;
    if (!pricing) {
      errors.push({ field: 'pricing', code: 'PRICING_REQUIRED', message: 'Les informations de prix sont requises.' });
    } else if (!pricing.isFreeDonation && (pricing.amount === undefined || pricing.amount < 0)) {
      errors.push({ field: 'pricing.amount', code: 'INVALID_AMOUNT', message: 'Le prix doit être supérieur ou égal à 0 €.' });
    }

    if (user?.role === 'pro_seller' || draft.proInventory) {
      const stock = draft.proInventory?.stock ?? 1;
      if (stock < 1) {
        errors.push({ field: 'proInventory.stock', code: 'INVALID_STOCK', message: 'Le stock disponible doit être d\'au moins 1 unité.' });
      }
    }
        ...
        if (draft.attributes) {
          Object.keys(draft.attributes).forEach((key) => {
            if (!allowedAttrCodes.has(key) && !allowedAttrIds.has(key)) {
              warnings.push(`Attribut "${key}" ignoré car non rattaché à cette catégorie.`);
            }
          });
        }
      }
    ...
    if (user) {
      const activeListings = storageService.getListings().filter((l) => l.sellerId === user.id && l.status === 'active');
   ...
   */
  async publishListing(draft: PublicationDraftState, user: UserProfile): Promise<Listing> {
    ...
    if (draft.fulfillment.allowHandDelivery) {
      deliveryOptions.push({ type: 'hand_delivery', available: true, price: 0 });
    }
    ...
    const rawSelectedMarkets =
      draft.selectedMarkets && draft.selectedMarkets.length > 0
        ...
        : [draft.marketCode || 'FR'];
    const selectedMarkets = Array.from(new Set(rawSelectedMarkets.map((m) => m.toUpperCase())));
    const primaryMarket = (draft.marketCode || selectedMarkets[0] || 'FR').toUpperCase();
    ...
    const newListing: Listing = {
      ...
      department: draft.location.department || `${draft.location.postalCode.slice(0, 2)}`,
      ...
      coverImageUrl: draft.photos[0]?.url || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800',
      deliveryOptions,
      ...
      marketPublications,
      currency: draft.pricing.currency || (primaryMarket === 'CH' ? 'CHF' : 'EUR'),
  ...
  async updateListing(
    ...
    if (existing.sellerId !== user.id && user.role !== 'admin' && user.role !== 'super_admin') {
    ...
    if (existing.status === 'reserved' && updates.pricing && updates.pricing.amount !== existing.price) {
      throw new Error('Impossible de modifier le prix d\'une annonce faisant l\'objet d\'une transaction en cours.');
    }
    ...
    const mergedDraft: PublicationDraftState = {
      ...
      fulfillment: updates.fulfillment || {
        ...
        allowParcelShipping: existing.deliveryOptions.some((d) => d.type !== 'hand_delivery'),
  ...
  saveDraft(draft: PublicationDraftState, userId?: string): void {
    ...
    const serialized = JSON.stringify({ ...draft, updatedAt: new Date().toISOString() });
  ...
  getDraft(userId?: string): PublicationDraftState | null {
    ...
    if (!raw && typeof localStorage !== 'undefined' && typeof localStorage.getItem === 'function') {
      try {
