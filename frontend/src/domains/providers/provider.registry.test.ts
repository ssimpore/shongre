...
describe('Canonical Provider Registry', () => {
  it('has unique IDs and codes for all registered providers', () => {
    ...
    CANONICAL_PROVIDER_REGISTRY.forEach((provider) => {
      expect(ids.has(provider.id), `Duplicate provider id: ${provider.id}`).toBe(false);
      expect(codes.has(provider.code), `Duplicate provider code: ${provider.code}`).toBe(false);
      ids.add(provider.id);
  ...
  it('assigns valid categories recognized by PROVIDER_CATEGORIES', () => {
    CANONICAL_PROVIDER_REGISTRY.forEach((provider) => {
      expect(PROVIDER_CATEGORIES[provider.category], `Unknown category for ${provider.id}`).toBeDefined();
    });
  });
  ...
  it('assigns valid capabilities recognized by PROVIDER_CAPABILITIES', () => {
    CANONICAL_PROVIDER_REGISTRY.forEach((provider) => {
      ...
      provider.capabilities.forEach((cap) => {
        expect(PROVIDER_CAPABILITIES[cap], `Unknown capability "${cap}" on ${provider.id}`).toBeDefined();
      });
    });
  });
  ...
  it('ensures configuration schemas contain valid fields', () => {
    CANONICAL_PROVIDER_REGISTRY.forEach((provider) => {
      ...
      provider.configurationSchema.fields.forEach((field) => {
        ...
        expect(['text', 'number', 'boolean', 'select', 'multi-select', 'url', 'password']).toContain(
          field.type
        );
      });
    });
