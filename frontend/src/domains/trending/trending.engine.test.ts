...
import { calculateGrowth, calculateTrendScore, deduplicateListings, growthScore, normalizeSignal, selectDiverseCandidates, timeDecay } from './trending.engine';
...
const listing = (id: string, categorySlug: string): Listing => ({
  ...
  photos: [{ id: `${id}-photo`, url: 'https://example.com/photo.jpg', isCover: true }],
...
const candidate = (id: string, parentKey: string, listingIds: string[]): TrendingTopicCandidate => ({
  id,
  type: 'category',
  key: id,
  title: id,
...
describe('trending engine', () => {
  ...
  it('decays old activity and normalizes volume logarithmically', () => {
    ...
    expect(timeDecay('2026-08-19T00:00:00.000Z', now)).toBeGreaterThan(timeDecay('2026-07-20T00:00:00.000Z', now));
  ...
  it('combines signals using configurable weights', () => {
    expect(calculateTrendScore({
      ...
      seasonality: 0,
      editorial: 0,
    }, { ...DEFAULT_TRENDING_ADMIN_CONFIG.weights, searchGrowth: 1 })).toBeGreaterThan(0.5);
  });

  it('keeps diversity across parent categories', () => {
    const config = { ...DEFAULT_TRENDING_ADMIN_CONFIG, maxTopics: 4, minTopics: 2, maxTopicsPerParentCategory: 1 };
    ...
    ].map((item, index) => ({ ...item, score: 0.9 - index * 0.1, direction: 'up' as const })), config);
    expect(chosen.map((item) => item.parentKey)).toEqual(['electronics', 'fashion']);
  });

  it('deduplicates listings across topic rails', () => {
    const topics = [
      { ...candidate('one', 'one', ['shared', 'one']), score: 0.8, direction: 'up' as const },
      { ...candidate('two', 'two', ['shared', 'two']), score: 0.7, direction: 'up' as const },
    ];
