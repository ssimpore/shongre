...
import type { TrendingAdminConfig, TrendingTopicOverride } from './trending.types';
...
export function getTrendingAdminConfig(marketCode = 'FR'): TrendingAdminConfig {
  const stored = storageService.get<Partial<TrendingAdminConfig> | null>(storageKey(marketCode), null);
  return cloneConfig({
    ...DEFAULT_TRENDING_ADMIN_CONFIG,
    ...stored,
    weights: { ...DEFAULT_TRENDING_ADMIN_CONFIG.weights, ...(stored?.weights || {}) },
    overrides: stored?.overrides || DEFAULT_TRENDING_ADMIN_CONFIG.overrides,
  });
}

export function updateTrendingAdminConfig(updates: Partial<TrendingAdminConfig>, marketCode = 'FR'): TrendingAdminConfig {
...
export function upsertTrendingOverride(input: TrendingTopicOverride): TrendingAdminConfig {
  const marketCode = input.marketCode || 'FR';
  const current = getTrendingAdminConfig(marketCode);
  const overrides = current.overrides.filter((override) => override.topicKey !== input.topicKey);
  return updateTrendingAdminConfig({ overrides: [...overrides, { ...input, marketCode }] }, marketCode);
}
