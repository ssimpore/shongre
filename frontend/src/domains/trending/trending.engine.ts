...
function activeOverride(
  ...
  key: string,
  now: Date,
): TrendingTopicOverride | undefined {
  return overrides.find((override) => {
    ...
    if (
      ...
      return false;
    if (override.endsAt && new Date(override.endsAt).getTime() < now.getTime())
      return false;
    return true;
  });
}
...
): Record<keyof TrendingSignalSnapshot, number> {
  const keys = Object.keys(candidates[0]?.signals || {}) as Array<
    keyof TrendingSignalSnapshot
  >;
  return keys.reduce(
    (maximums, key) => {
      maximums[key] = Math.max(
        1,
        ...candidates.map((candidate) => Number(candidate.signals[key]) || 0),
      );
      return maximums;
    },
...
export function rankTrendingCandidates(
  ...
  return eligible
    .map((candidate) => {
      ...
      const score =
        calculateTrendScore(
          derivedSignals(candidate, maximums, now),
          config.weights,
        ) +
        clamp(override?.boostScore || 0) * 0.25 +
        clamp(candidate.personalizationScore || 0) *
    ...
    .sort((a, b) => {
      ...
      const sortDelta =
        ...
        (b.override?.sortOrder ?? Number.MAX_SAFE_INTEGER);
      if (sortDelta !== 0 && (a.override?.isPinned || b.override?.isPinned))
        return sortDelta;
...
export function selectDiverseCandidates(
  ...
  for (const candidate of ranked) {
    const parent =
      candidate.parentKey || candidate.categorySlug || candidate.key;
    const count = parentCounts.get(parent) || 0;
    const canBreakDiversity =
      Boolean(candidate.override?.isPinned) && candidate.score >= 0.8;
    if (count >= config.maxTopicsPerParentCategory && !canBreakDiversity)
      continue;
...
export function deduplicateListings(
  topics: RankedTrendingCandidate[],
  listingsPerTopic = 5,
): Map<string, string[]> {
  ...
  for (const topic of topics) {
    const chosen = [...topic.listings]
      .sort((a, b) => {
        const qualityA =
          Number(a.sellerIsVerified) * 3 +
          Number(Boolean(a.coverImageUrl)) * 2 +
          a.viewsCount * 0.002 +
          a.favoritesCount * 0.01;
        const qualityB =
          Number(b.sellerIsVerified) * 3 +
...
export function toPublicTopics(
  ...
  return selected
    .map((candidate) => {
      ...
      const listings = candidate.listings.filter((listing) =>
