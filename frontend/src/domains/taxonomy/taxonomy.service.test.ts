...
describe('Taxonomy Service & Integrity', () => {
  ...
  it('resolves real estate publication schema without physical parcel shipping', () => {
    const schema = taxonomyService.resolvePublicationSchema('real_estate.sales');
    expect(schema).not.toBeNull();
  ...
  it('resolves card summary preview attributes', () => {
    ...
    expect(summary.some((s) => s.id === 'vehicle.year' || s.id === 'vehicle.mileage')).toBe(true);
  });
  ...
  describe('shortLabel Resolution & Fallbacks', () => {
    ...
    it('safely falls back to canonical label/name if shortLabel is undefined', () => {
      ...
      expect(taxonomyService.getLabel(nodeWithoutShort, 'compact')).toBe('Mon Super Produit');
      expect(taxonomyService.getLabel(nodeWithoutShort, 'full')).toBe('Mon Super Produit');
    });
    ...
    it('resolves localized shortLabels when locale is specified', () => {
      ...
      const enCompact = taxonomyService.getLabel(carsNode, { compact: true, locale: 'en-US' });
      const frCompact = taxonomyService.getLabel(carsNode, { compact: true, locale: 'fr-FR' });
    ...
    it('resolves breadcrumbs in full and compact modes', () => {
      const fullCrumbs = taxonomyService.getBreadcrumbs('vehicles.cars.citadines', 'full');
      expect(fullCrumbs.map((c) => c.label)).toEqual(['Véhicules', 'Voitures d\'occasion', 'Citadines']);

      const compactCrumbs = taxonomyService.getBreadcrumbs('vehicles.cars.citadines', 'compact');
      expect(compactCrumbs.map((c) => c.label)).toEqual(['Véhicules', 'Voitures', 'Citadines']);
    });
