...
import { TaxonomyNode, TaxonomyAttribute, ListingFamily } from '../taxonomy/taxonomy.types';
...
export class ListingDisplayResolver {
   ...
   */
  resolveSummaryAttributes(listing: Listing, node?: TaxonomyNode | null): string[] {
    const summary: string[] = [];
    const attrs = listing.attributes || {};
    const effectiveNode = node || taxonomyService.getNode(listing.subCategorySlug) || taxonomyService.getNode(listing.categorySlug);
    const family: ListingFamily = effectiveNode ? taxonomyService.getFamily(effectiveNode.id) : 'physical_product';

    // 1. Domain-specific prioritizations
    if (family === 'vehicle') {
      if (attrs.year) summary.push(`${attrs.year}`);
      if (attrs.mileage !== undefined) {
        const formattedKm = String(attrs.mileage).replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
        summary.push(`${formattedKm} km`);
      }
      if (attrs.fuel) summary.push(this.formatOptionLabel('fuel', attrs.fuel));
      if (attrs.gearbox) summary.push(this.formatOptionLabel('gearbox', attrs.gearbox));
      if (attrs.critair) summary.push(`Crit'Air ${attrs.critair}`);
    } else if (family === 'real_estate') {
      if (attrs.property_type) summary.push(this.formatOptionLabel('property_type', attrs.property_type));
      if (attrs.surface !== undefined) summary.push(`${attrs.surface} m²`);
      if (attrs.rooms !== undefined) summary.push(`${attrs.rooms} pièce${attrs.rooms > 1 ? 's' : ''}`);
      if (attrs.bedrooms !== undefined) summary.push(`${attrs.bedrooms} ch.`);
      if (attrs.energy_class) summary.push(`DPE ${String(attrs.energy_class).toUpperCase()}`);
    } else if (family === 'service') {
      if (attrs.service_type) summary.push(this.formatOptionLabel('service_type', attrs.service_type));
      if (attrs.location_type) summary.push(this.formatOptionLabel('location_type', attrs.location_type));
      if (attrs.experience_years) summary.push(`${attrs.experience_years} ans d'exp.`);
    } else if (family === 'job') {
      if (attrs.contract_type) summary.push(this.formatOptionLabel('contract_type', attrs.contract_type));
      if (attrs.work_rhythm) summary.push(this.formatOptionLabel('work_rhythm', attrs.work_rhythm));
      if (attrs.remote_policy) summary.push(this.formatOptionLabel('remote_policy', attrs.remote_policy));
    } else {
      // Physical Products (Smartphones, Furniture, Fashion, Appliances)
      if (attrs.storage_capacity) {
        const optLabel = this.formatOptionLabel('storage_capacity', attrs.storage_capacity);
        summary.push(optLabel.includes('Go') || optLabel.includes('To') ? optLabel : `${optLabel} Go`);
      }
      if (attrs.material) summary.push(this.formatOptionLabel('material', attrs.material));
      if (attrs.size) summary.push(`Taille ${attrs.size}`);
      if (attrs.dimensions) summary.push(String(attrs.dimensions));
      if (attrs.color) summary.push(this.formatOptionLabel('color', attrs.color));
    }

    // 2. Condition fallback if not yet full and condition exists
    if (listing.condition && family === 'physical_product' && summary.length < 3) {
   ...
   */
  resolveGroupedCharacteristics(listing: Listing, node?: TaxonomyNode | null): GroupedCharacteristics[] {
    const attrs = listing.attributes || {};
    const effectiveNode = node || taxonomyService.getNode(listing.subCategorySlug) || taxonomyService.getNode(listing.categorySlug);
    ...
    Object.entries(attrs).forEach(([key, rawValue]) => {
      ...
      const attrDef =
        Object.values(ATTRIBUTE_REGISTRY).find((a) => a.code === key || a.id === key || a.id.endsWith(`.${key}`)) ||
  ...
  private formatOptionLabel(attributeId: string, value: any): string {
    const cleanKey = attributeId.replace(/^(product|vehicle|real_estate|electronics|home|fashion|service|job)\./, '');
    const attr =
      ATTRIBUTE_REGISTRY[attributeId] ||
      Object.values(ATTRIBUTE_REGISTRY).find((a) => a.code === attributeId || a.code === cleanKey || a.id === attributeId || a.id.endsWith(`.${cleanKey}`));
  ...
  private inferGroupKey(key: string): string {
    if (key.startsWith('vehicle.') || key.includes('mileage') || key.includes('fuel') || key.includes('gearbox')) return 'performance';
    if (key.startsWith('real_estate.') || key.includes('surface') || key.includes('rooms') || key.includes('floor')) return 'property';
    if (key.includes('energy') || key.includes('ges') || key.includes('dpe')) return 'energy';
    if (key.includes('width') || key.includes('height') || key.includes('depth') || key.includes('dimensions') || key.includes('material')) return 'dimensions';
    if (key.includes('salary') || key.includes('contract') || key.includes('experience')) return 'employment';
    if (key.includes('power') || key.includes('storage') || key.includes('ram') || key.includes('battery')) return 'technical';
    return 'general';
  }

  private formatFallbackLabel(key: string): string {
    const clean = key.replace(/^(product|vehicle|real_estate|electronics|home|fashion|service|job)\./, '');
   ...
   */
  generateListingStructuredData(listing: Listing, seller?: UserProfile | null, effectiveMarket?: MarketConfiguration): Record<string, any> {
    const currency = listing.currency || effectiveMarket?.localization.defaultCurrency || 'EUR';
    const photos = listing.photos.map((p) => (typeof p === 'string' ? p : p.url)).filter(Boolean);
    const listingUrl = typeof window !== 'undefined' ? window.location.href : `https://shongre.fr/annonce/${listing.id}`;
    ...
    return {
      ...
      offers: {
        ...
        itemCondition: listing.condition === 'new_with_tag' ? 'https://schema.org/NewCondition' : 'https://schema.org/UsedCondition',
        seller: {
   ...
   */
  generateListingSeoMeta(listing: Listing, node?: TaxonomyNode | null, effectiveMarket?: MarketConfiguration): ListingSeoMetadata {
    const currency = listing.currency || effectiveMarket?.localization.defaultCurrency || 'EUR';
    const priceStr = listing.isFreeDonation ? 'Don gratuit' : `${listing.price} ${currency === 'EUR' ? '€' : currency}`;
    ...
    return {
      ...
      jsonLd: this.generateListingStructuredData(listing, null, effectiveMarket),
    };
  }
}
