...
describe('Listing Detail Display & Action Resolvers', () => {
  ...
  describe('Summary Attributes Resolver', () => {
    it('resolves vehicle summary attributes (Year, Mileage, Fuel, Transmission, CritAir)', () => {
      ...
      const summary = listingDisplayResolver.resolveSummaryAttributes(vehicleListing as Listing);
    ...
    it('resolves real estate summary attributes (Property type, Surface, Rooms, Bedrooms, DPE)', () => {
      ...
      const summary = listingDisplayResolver.resolveSummaryAttributes(realEstateListing as Listing);
    ...
    it('resolves smartphone summary attributes (Storage, Color, Condition)', () => {
      ...
      const summary = listingDisplayResolver.resolveSummaryAttributes(phoneListing as Listing);
    ...
    it('resolves furniture summary attributes (Material, Dimensions, Condition)', () => {
      ...
      const summary = listingDisplayResolver.resolveSummaryAttributes(furnitureListing as Listing);
  ...
  describe('Grouped Characteristics Resolver', () => {
    it('groups vehicle technical specifications and ignores absent attributes', () => {
      ...
      const groups = listingDisplayResolver.resolveGroupedCharacteristics(vehicleListing as Listing);
      expect(groups.length).toBeGreaterThanOrEqual(2);

      const allItems = groups.flatMap((g) => g.items);
      expect(allItems.some((i) => i.code === 'mileage' && i.value === '24 000 km')).toBe(true);
      expect(allItems.some((i) => i.code === 'fuel' && i.value === '100% Électrique')).toBe(true);
    ...
    it('does NOT include vehicle engine specs on real estate listings', () => {
      ...
      const groups = listingDisplayResolver.resolveGroupedCharacteristics(reListing as Listing);
  ...
  describe('SEO & Structured Data', () => {
    it('generates valid Schema.org Product structured data', () => {
      const sampleListing = {
        ...
        photos: [{ id: 'p1', url: 'https://images.unsplash.com/gravel.jpg', isCover: true }],
        deliveryOptions: [{ type: 'hand_delivery', available: true }],
        status: 'active',
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      ...
      const jsonLd = listingDisplayResolver.generateListingStructuredData(sampleListing);
