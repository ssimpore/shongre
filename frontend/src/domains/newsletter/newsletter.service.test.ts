...
describe('NewsletterService', () => {
  it('normalizes email addresses safely', () => {
    expect(newsletterService.normalizeEmail('  THOMAS.Laurent@Example.FR  ')).toBe('thomas.laurent@example.fr');
  });

  it('validates emails correctly', () => {
    expect(newsletterService.validateEmail('').isValid).toBe(false);
    expect(newsletterService.validateEmail('invalid-email').isValid).toBe(false);
    expect(newsletterService.validateEmail('thomas@example.fr').isValid).toBe(true);
  });
