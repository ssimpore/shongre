...
describe("CrmService & Capabilities", () => {
  it("formats money and stages correctly", () => {
    const formatted = crmService.formatCrmMoney({
      amountMinor: 118800,
      currency: "EUR",
    });
  ...
  it("normalizes domains accurately", () => {
    expect(
      crmService.normalizeDomain("https://www.atelier-nordique.fr/catalogue"),
    ).toBe("atelier-nordique.fr");
    expect(crmService.normalizeDomain("http://voltexpert-france.fr/")).toBe(
      "voltexpert-france.fr",
    );
  });
  ...
  it("detects duplicate companies based on domain or name", () => {
    ...
    const duplicateCand = {
      ...
      website: "https://www.atelier-nordique.fr/contact",
    };
    const dupCheck = crmService.detectDuplicate(
      duplicateCand,
      existingCompanies,
    );
    ...
    const freshCand = {
      ...
      website: "https://maison-nouvelle.com",
    };
