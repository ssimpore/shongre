...
describe('recording a decision', () => {
  ...
  it('forces the necessary category on however it was stored', () => {
    expect(normaliseCategories({ necessary: false } as never).necessary).toBe(true);
...
describe('a stored decision stops counting', () => {
  ...
  it('rejects one written against older categories', () => {
    expect(isDecisionCurrent(decision({ version: CONSENT_VERSION - 1 }) as never, NOW)).toBe(false);
  });

  // Consent is given for a period, not forever.
  it('rejects one past its lifetime', () => {
    expect(
      isDecisionCurrent(decision({ decidedAt: daysBefore(CONSENT_LIFETIME_DAYS + 1) }) as never, NOW),
    ).toBe(false);
    expect(
      isDecisionCurrent(decision({ decidedAt: daysBefore(CONSENT_LIFETIME_DAYS - 1) }) as never, NOW),
    ).toBe(true);
  ...
  it('rejects a future date and an unparseable one', () => {
    expect(isDecisionCurrent(decision({ decidedAt: daysBefore(-2) }) as never, NOW)).toBe(false);
    expect(isDecisionCurrent(decision({ decidedAt: 'not-a-date' }) as never, NOW)).toBe(false);
    expect(isDecisionCurrent(null, NOW)).toBe(false);
  });

  it('re-prompts rather than honouring an expired acceptance', () => {
    storageService.set(KEY, decision({ decidedAt: daysBefore(CONSENT_LIFETIME_DAYS + 5) }));
