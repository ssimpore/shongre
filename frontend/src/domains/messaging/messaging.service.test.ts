...
import { ConversationPreview, UserTimelineMessage, SystemTimelineEvent } from './messaging.types';
...
describe('MessagingService', () => {
  ...
  it('filters conversations by search query and unread status', () => {
    ...
    // Filter unread
    const unreadOnly = messagingService.filterConversations(convs, 'unread', '', 'u1');
    expect(unreadOnly.length).toBe(1);
    expect(unreadOnly[0].id).toBe('c1');

    // Search by title
    const searchedByTitle = messagingService.filterConversations(convs, 'all', 'canapé', 'u1');
    expect(searchedByTitle.length).toBe(1);
    expect(searchedByTitle[0].id).toBe('c2');

    // Search by counterpart name
    const searchedByName = messagingService.filterConversations(convs, 'all', 'thomas', 'u1');
