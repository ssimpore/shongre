...
const INITIAL_CAMPAIGNS: NewsletterCampaign[] = [
  {
    ...
    previewText:
      "Découvrez notre sélection exclusive de pièces vintage vérifiées.",
    content: {
...
export class MockNewsletterRepository implements INewsletterRepository {
  private getSubscriptions(): NewsletterSubscription[] {
    return storageService.get<NewsletterSubscription[]>(
      "shongre_newsletter_subs",
  ...
  private getCampaigns(): NewsletterCampaign[] {
    return storageService.get<NewsletterCampaign[]>(
      "shongre_newsletter_campaigns",
      INITIAL_CAMPAIGNS,
    );
  }
  ...
  async getSubscription(
    ...
    return (
      list.find((s) => s.email === normalized && s.marketCode === marketCode) ||
      null
    );
  }
  ...
  async subscribe(
    ...
    const existingIndex = list.findIndex(
      (s) => s.email === normalized && s.marketCode === market,
    );
    const now = new Date().toISOString();

    const topicsToAssign =
  ...
  async getAudienceEstimate(
    ...
    const subs = this.getSubscriptions().filter(
    ...
    const filtered = subs.filter((s) => {
      ...
      if (audience.accountTypes && audience.accountTypes.length > 0) {
        if (!audience.accountTypes.includes(s.accountType || "individual"))
          return false;
      }
