...
import type { NewsletterSubscription, NewsletterTopic, SubscribeNewsletterInput } from "../domains/newsletter/newsletter.types";
import { CURRENT_NEWSLETTER_CONSENT_VERSION } from "../domains/newsletter/newsletter.service";
import { DEFAULT_MARKET_CODE } from "../configuration/market-baseline";

interface AccountIdentity { userId: string; email: string; marketCode: string }

function toSubscription(view: MarketingSubscriptionView, subscriberId?: string): NewsletterSubscription {
  const status = view.status === "SUBSCRIBED" ? "subscribed" : view.status === "PENDING" ? "pending_confirmation" : view.status === "UNSUBSCRIBED" ? "unsubscribed" : "suppressed";
  ...
  return {
    ...
    consent: { consented: status === "subscribed" || status === "pending_confirmation", consentedAt: view.subscribedAt ?? now, version: CURRENT_NEWSLETTER_CONSENT_VERSION, source: "account" },
...
class MarketingNewsletterRepository implements INewsletterRepository {
  ...
  async subscribe(input: SubscribeNewsletterInput) {
    if (!input.subscriberId) throw new Error("Connectez-vous pour gérer cet abonnement depuis votre compte.");
    const identity = { userId: input.subscriberId, email: input.email, marketCode: input.marketCode ?? DEFAULT_MARKET_CODE };
    const view = await services.marketing.subscribeAccount({ ...identity, locale: input.locale ?? "fr-FR", topics: input.topics ?? [], consentGiven: true });
  ...
  async updatePreferences(subscriptionId: string, topics: NewsletterTopic[]) {
    ...
    if (!identity) throw new Error("Rechargez la page avant de modifier vos préférences.");
    return this.remember(await services.marketing.updateAccountPreferences({ ...identity, topics }), identity);
  }

  async unsubscribe(subscriptionId: string) {
    const identity = this.identities.get(subscriptionId);
    if (!identity) throw new Error("Rechargez la page avant de vous désabonner.");
    return this.remember(await services.marketing.unsubscribeAccount(identity), identity);
  }

  async resubscribe(subscriptionId: string, topics: NewsletterTopic[] = []) {
    const identity = this.identities.get(subscriptionId);
    if (!identity) throw new Error("Rechargez la page avant de vous réabonner.");
    return this.remember(await services.marketing.subscribeAccount({ ...identity, topics, consentGiven: true }), identity);
  }
}

export const newsletterRepository: INewsletterRepository = new MarketingNewsletterRepository();
