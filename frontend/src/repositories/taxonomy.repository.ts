...
class TaxonomyAdminRepository implements ITaxonomyAdminRepository {
  ...
  private loadInitialData() {
    // 1. Nodes
    const storedNodes = storageService.get<TaxonomyNode[] | null>(
      STORAGE_KEYS.TAXONOMY_NODES,
      null,
    );
    ...
    // 2. Attributes
    const storedAttributes = storageService.get<Record<
      string,
      TaxonomyAttribute
    ...
    // 3. Draft Changes
    this.draftChanges = storageService.get<TaxonomyDraftChange[]>(
      STORAGE_KEYS.DRAFT_CHANGES,
      [],
    );

    // 4. Versions
    this.versions = storageService.get<TaxonomyVersion[]>(
    ...
    // 5. Audit Logs
    this.auditLogs = storageService.get<TaxonomyAuditEvent[]>(
  ...
  async createNode(
    ...
    if (this.getNode(stableId)) {
      throw new Error(
        `Un nœud avec l'identifiant unique "${stableId}" existe déjà.`,
      );
    }
    ...
    const newNode: TaxonomyNode = {
      ...
      publishable:
        input.publishable ??
        (input.level === "type" || input.level === "subtype"),
      ...
      shortLabels: input.shortLabel?.trim()
        ? { "fr-FR": input.shortLabel.trim() }
        : undefined,
  ...
  async updateNode(
    ...
    if (updates.description !== undefined)
      node.description = updates.description.trim();
    if (updates.iconName !== undefined) node.iconName = updates.iconName;
    if (updates.accentColor !== undefined)
      node.accentColor = updates.accentColor;
    if (updates.publishable !== undefined)
      node.publishable = updates.publishable;
    ...
    if (updates.summaryAttributeIds !== undefined)
      node.summaryAttributeIds = updates.summaryAttributeIds;
    if (updates.filterFacetIds !== undefined)
      node.filterFacetIds = updates.filterFacetIds;
    if (updates.capabilities !== undefined)
      node.capabilities = updates.capabilities;
    if (updates.sellerEligibility !== undefined)
      node.sellerEligibility = updates.sellerEligibility;
  ...
  async moveNode(
    ...
    const updateDescendantsAncestors = (
    ...
    this.logAudit(
      node.id,
      node.name,
      `Déplacement de branche sous ${newParentId || "Racine"}`,
      actor,
    );
  }
  ...
  async duplicateNode(
    ...
    cloned.shortLabel = original.shortLabel
      ? `${original.shortLabel} Copie`
      : undefined;
    cloned.status = "draft";
    ...
    this.logAudit(
      cloned.id,
      cloned.name,
      `Nœud dupliqué depuis ${original.id}`,
      actor,
    );
  ...
  async setMarketOverride(
    ...
    this.logAudit(
      node.id,
      node.name,
      `Surcharge marché ${marketCode} enregistrée`,
      actor,
    );
  }
  ...
  async resetMarketOverride(
    ...
    if (node.marketOverrides && node.marketOverrides[marketCode]) {
      ...
      this.logAudit(
        node.id,
        node.name,
        `Surcharge marché ${marketCode} réinitialisée sur France`,
        actor,
      );
    }
  }
  ...
  async saveAttribute(
    ...
    this.logAudit(
      ...
      attr.label,
      isNew
        ? `Création de l'attribut "${attr.label}"`
        : `Mise à jour de l'attribut "${attr.label}"`,
      actor,
    );
  ...
  async deprecateAttribute(
    ...
    this.logAudit(
      attrId,
      attr.label,
      `Dépréciation de l'attribut "${attr.label}"`,
      actor,
    );
  }
  ...
  async publishDraft(
    ...
    const blockingErrors = validationIssues.filter(
      (i) => i.severity === "error",
    );
    ...
    const newVersion: TaxonomyVersion = {
      ...
      description:
        description ||
        `Publication de la version ${nextVersionNumber} de la taxonomie`,
    ...
    allNodes.forEach((node) => {
      ...
      if (!node.id || node.id.trim().length === 0) {
        issues.push({
          ...
          code: "MISSING_ID",
          message:
            "Ce nœud ne possède pas d'identifiant canonique stable (ID).",
      ...
      if (!node.slug || node.slug.trim().length === 0) {
        ...
        if (seenSlugs.has(siblingKey)) {
          issues.push({
            ...
            field: "slug",
            remediation:
              "Utilisez un slug distinct pour chaque sous-catégorie soeur.",
          });
        }
      ...
      if (node.parentId && !this.getNode(node.parentId)) {
        issues.push({
          ...
          remediation:
            "Rattachez cette catégorie à un parent valide ou placez-la à la racine.",
        });
      }
        ...
        node.attributeIds.forEach((attrId) => {
          if (!this.attributes[attrId]) {
            issues.push({
              ...
              field: "attributeIds",
              remediation:
                "Créez cet attribut dans le Registre ou retirez-le de la catégorie.",
            });
          }
        });
      }
      ...
      if (node.capabilities) {
        ...
        if (
          node.id.startsWith("real_estate") &&
          modes.includes("parcel_shipping")
        ) {
          issues.push({
            ...
            message:
              "La catégorie Immobilière autorise la livraison par colis.",
            field: "capabilities",
            remediation:
      ...
      if (node.shortLabel) {
        ...
        if (seenShortLabels.has(siblingShortKey)) {
          issues.push({
            ...
            field: "shortLabel",
            remediation:
              "Donnez un alias compact unique pour éviter toute confusion dans les menus mobiles.",
          });
        }
  ...
  analyzeNodeImpact(nodeId: string): TaxonomyImpactReport {
    ...
    const simulatedListings =
      isDraft && descendantsCount === 0
        ? 0
        : isRoot
          ? Math.floor(descendantsCount * 18 + 45)
          : node.children?.length
            ? node.children.length * 12 + 6
            : 8;
    const simulatedSavedSearches = isDraft
      ? 0
      : isRoot
        ? Math.floor(descendantsCount * 4 + 10)
        : 3;
    const marketOverridesCount = node.marketOverrides
      ? Object.keys(node.marketOverrides).length
      : 0;

    const blockingReasons: string[] = [];
    if (descendantsCount > 0) {
      blockingReasons.push(
        `Cette catégorie possède ${descendantsCount} sous-catégories / types enfants.`,
      );
    }
    if (simulatedListings > 0) {
      blockingReasons.push(
        `Elle est activement référencée par environ ${simulatedListings} annonce(s) en ligne.`,
      );
    }
    ...
    try {
      ...
      if (!parsed.nodes || !Array.isArray(parsed.nodes)) {
        return {
          ...
          updatedCount: 0,
          errors: [
            'Format de fichier invalide : propriété "nodes" absente ou non conforme.',
          ],
        };
      }
      ...
      return {
        ...
        updatedCount: 0,
        errors: [
          `Erreur de parsing JSON : ${e?.message || "Syntaxe JSON invalide"}`,
        ],
      };
    }
  }
