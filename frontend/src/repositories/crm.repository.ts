...
export interface UniversalSearchResult {
  ...
  subtitle: string;
  badgeText: string;
  badgeVariant?: 'neutral' | 'primary' | 'pro' | 'verified' | 'urgent' | 'deal' | 'warning' | 'success';
  linkTo: string;
}
...
export interface ICrmRepository {
  ...
  updateOpportunityStage(id: string, stage: OpportunityStage): Promise<CrmOpportunity>;
  updateOpportunity(id: string, updates: Partial<CrmOpportunity>): Promise<CrmOpportunity>;

  // Activities & Tasks
  listActivities(entityType: 'contact' | 'company' | 'opportunity', entityId: string): Promise<CrmActivity[]>;
  ...
  // AI Prospect Import
  importAiCandidate(candidate: ProspectResearchCandidate, ownerId?: string, ownerName?: string): Promise<CrmCompany>;
...
const INITIAL_COMPANIES: CrmCompany[] = [
  {
    ...
    aiFitScore: 98,
    aiSummary: 'Atelier de référence spécialisé dans le mobilier vintage certifié. 140 annonces actives sur Shongre.',
  ...
  },
  {
    ...
    aiFitScore: 95,
    aiSummary: 'Distributeur et installateur certifié IRVE. Fort potentiel pour catalogue Matériel Professionnel.',
  ...
  },
  {
    ...
    aiFitScore: 92,
    aiSummary: 'Boutique renommée du 11e arrondissement. Prise de contact commerciale planifiée.',
...
const INITIAL_OPPORTUNITIES: CrmOpportunity[] = [
  {
    ...
    ownerName: 'Antoine Fabre',
    marketCode: 'FR',
    notes: 'Discussion sur l\'importation automatique de flux CSV pour 200 nouvelles références.',
  ...
  },
  {
    ...
    ownerName: 'Antoine Fabre',
    marketCode: 'FR',
    notes: 'Offre Pro Business envoyée avec 1 mois offert pour tester la visibilité.',
...
const INITIAL_ACTIVITIES: CrmActivity[] = [
  {
    ...
    description: 'Marc est très satisfait des ventes réalisées via la boutique Shongre. Il souhaite étendre son catalogue aux luminaires.',
  ...
  },
  {
    ...
    description: 'Entreprise identifiée sur requête "Installateurs bornes de recharge France". Fit score Shongre estimé à 95/100.',
...
export class MockCrmRepository implements ICrmRepository {
  private getCompanies(): CrmCompany[] {
    return storageService.get<CrmCompany[]>('shongre_crm_companies', INITIAL_COMPANIES);
  }
  ...
  private getContacts(): CrmContact[] {
    return storageService.get<CrmContact[]>('shongre_crm_contacts', INITIAL_CONTACTS);
  }
  ...
  private getOpportunities(): CrmOpportunity[] {
    return storageService.get<CrmOpportunity[]>('shongre_crm_opportunities', INITIAL_OPPORTUNITIES);
  }
  ...
  private getActivities(): CrmActivity[] {
    return storageService.get<CrmActivity[]>('shongre_crm_activities', INITIAL_ACTIVITIES);
  }
  ...
  async listContacts(): Promise<CrmContact[]> {
    return this.getContacts().sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  }
  ...
  async createContact(contact: Partial<CrmContact>): Promise<CrmContact> {
    ...
    const newContact: CrmContact = {
      id: `crm-c-${Date.now()}`,
      identity: contact.identity || { firstName: 'Contact', lastName: '', email: 'contact@exemple.fr' },
  ...
  async listCompanies(): Promise<CrmCompany[]> {
    return this.getCompanies().sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  }
  ...
  async listOpportunities(): Promise<CrmOpportunity[]> {
    return this.getOpportunities().sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  }
  ...
  async createOpportunity(opp: Partial<CrmOpportunity>): Promise<CrmOpportunity> {
    ...
    const newOpp: CrmOpportunity = {
      ...
      stage: opp.stage || 'new',
      estimatedValue: opp.estimatedValue || { amountMinor: 50000, currency: 'EUR' },
  ...
  async updateOpportunityStage(id: string, stage: OpportunityStage): Promise<CrmOpportunity> {
    const list = this.getOpportunities();
  ...
  // ACTIVITIES
  async listActivities(entityType: 'contact' | 'company' | 'opportunity', entityId: string): Promise<CrmActivity[]> {
  ...
  async listTasks(): Promise<CrmTask[]> {
    return this.getTasks().sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  }
  ...
  async searchUniversal(query: string): Promise<UniversalSearchResult[]> {
    ...
    for (const comp of this.getCompanies()) {
      if (comp.name.toLowerCase().includes(q) || (comp.domain && comp.domain.toLowerCase().includes(q))) {
        results.push({
          type: 'company',
          id: comp.id,
          title: comp.name,
    ...
    for (const contact of this.getContacts()) {
      const fullName = `${contact.identity.firstName} ${contact.identity.lastName}`.toLowerCase();
      if (fullName.includes(q) || contact.identity.email.toLowerCase().includes(q)) {
        results.push({
          type: 'contact',
          id: contact.id,
    ...
    for (const opp of this.getOpportunities()) {
      if (opp.title.toLowerCase().includes(q) || (opp.companyName && opp.companyName.toLowerCase().includes(q))) {
        results.push({
          type: 'opportunity',
          id: opp.id,
          title: opp.title,
  ...
  async getOverviewStats(): Promise<CrmOverviewStats> {
    ...
    const pipelineValue = openOpps.reduce((sum, o) => sum + o.estimatedValue.amountMinor, 0);

    return {
      totalContacts: contacts.length,
      totalCompanies: companies.length,
      activeProspects: companies.filter((c) => c.lifecycle === 'prospect').length,
