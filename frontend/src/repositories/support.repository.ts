...
export interface ISupportRepository {
  ...
  getRequests(query?: SupportRequestQuery): Promise<{ requests: SupportRequest[]; total: number }>;
  ...
  simulateAgentReply(requestId: string, replyText?: string): Promise<SupportRequest>;
}
...
const INITIAL_SUPPORT_REQUESTS: SupportRequest[] = [
  {
    ...
    description: 'Bonjour, j\'ai effectué le paiement de 360 € pour le Fauteuil Scandinave. Le montant apparaît comme débité sur mon compte bancaire mais la commande indique « Séquestre en attente ». Pouvez-vous vérifier ?',
    ...
    messages: [
      {
        ...
        content: 'Bonjour, j\'ai effectué le paiement de 360 € pour le Fauteuil Scandinave. Le montant apparaît comme débité sur mon compte bancaire mais la commande indique « Séquestre en attente ». Pouvez-vous vérifier ?',
      ...
      },
      {
        ...
        content: 'Bonjour Thomas, nous avons bien identifié la pré-autorisation bancaire. La validation finale par votre banque a pris quelques minutes de plus que prévu. Les fonds sont désormais parfaitement sécurisés sous séquestre Shongre ! Pouvez-vous nous confirmer si votre statut s\'affiche à jour ?',
  ...
  },
  {
    ...
    description: 'Bonjour, je souhaite vendre un vélo Gravel et il n\'y a que « VTT » ou « Vélos de route ». Serait-il possible d\'ajouter la sous-catégorie Gravel ?',
    status: 'resolved',
    priority: 'low',
    messages: [
      {
        ...
        authorType: 'user',
        authorName: 'Thomas Laurent',
        content: 'Bonjour, je souhaite vendre un vélo Gravel et il n\'y a que « VTT » ou « Vélos de route ». Serait-il possible d\'ajouter la sous-catégorie Gravel ?',
      ...
      },
      {
        ...
        content: 'Bonjour Thomas, merci beaucoup pour votre suggestion ! La sous-catégorie « Vélos Gravel » a été ajoutée à notre taxonomie officielle dans la section Mobilité & Vélos.',
...
export class MockSupportRepository implements ISupportRepository {
  private getStorageRequests(): SupportRequest[] {
    return storageService.getSupportRequests<SupportRequest[]>(INITIAL_SUPPORT_REQUESTS);
  }
  ...
  async getRequests(query?: SupportRequestQuery): Promise<{ requests: SupportRequest[]; total: number }> {
