...
/** Small dependency-free class composer for the Design System. */
...
/**
 * Typed visual recipes without a runtime styling dependency.
 *
 * Variants are intentionally flat. When a component needs compound layout it
 * should compose smaller components instead of growing a conditional matrix.
 */
export function createVariants<const TSchema extends VariantSchema>({
  ...
  return (selection: VariantSelection<TSchema> & { className?: string } = {}) => {
