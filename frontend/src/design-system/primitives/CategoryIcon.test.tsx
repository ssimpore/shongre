...
import { CategoryIcon, ICON_NAME_MAP, CATEGORY_SLUG_ICON_MAP } from './CategoryIcon';
import { CANONICAL_TAXONOMY, TAXONOMY } from '../../domains/taxonomy/taxonomy.data';
...
describe('CategoryIcon Component & Taxonomy Icon Integrity', () => {
  ...
  it('ensures each category in TAXONOMY bridge has a non-empty iconName', () => {
    TAXONOMY.forEach((cat) => {
      ...
      expect(ICON_NAME_MAP[cat.iconName] || CATEGORY_SLUG_ICON_MAP[cat.slug]).toBeDefined();
    });
  });

  // 3. Verify subcategories have valid icon resolution
  it('resolves icons for subcategories across different domains', () => {
    const subCategories = ['vehicles.cars', 'vehicles.motos', 'real_estate.sales', 'electronics.smartphones', 'home_garden.furniture'];
    
    subCategories.forEach((subId) => {
      const node = taxonomyService.getNode(subId);
      expect(node).toBeDefined();
      
      const element = CategoryIcon({ category: node }) as React.ReactElement<any>;
  ...
  it('renders with background container when withBackground is true', () => {
    const element = CategoryIcon({ category: 'vehicules', size: 'lg', withBackground: true, className: 'custom-class' }) as React.ReactElement<any>;
    expect(element).toBeDefined();
