...
const UNSPLASH = 'https://images.unsplash.com/photo-1549399542?auto=format&fit=crop&w=800&q=80';
...
describe('isResizableSource', () => {
  ...
  it('rejects sources it cannot safely rewrite', () => {
    ...
    expect(isResizableSource('https://atelier-nordique.fr/logo.png')).toBe(false);
  });
});
...
describe('buildSrcSet', () => {
  ...
  it('emits a w-descriptor entry per ladder step', () => {
    ...
    expect(entries).toHaveLength(DEFAULT_WIDTH_LADDER.filter((w) => w <= 800).length);
