...
export const Container: React.FC<ContainerProps> = ({
  ...
  className,
  ...props
}) => <Component className={containerClasses({ width, gutter, className })} {...props} />;
  ...
  ({ columns = 1, gap = 'md', className, ...props }, ref) => {
    const columnsClass =
      columns === 'auto'
        ...
        : { 1: 'grid-cols-1', 2: 'grid-cols-1 sm:grid-cols-2', 3: 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3', 4: 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' }[
            columns
          ];
    return (
      <div ref={ref} className={cn('grid min-w-0', columnsClass, gapClasses[gap], className)} {...props} />
    );
  },
);
...
const surfaceClasses = createVariants({
  ...
  variants: {
    tone: {
      ...
      inverse: 'bg-stone-900 text-text-inverse',
    },
  ...
  defaultVariants: { tone: 'base', padding: 'md', radius: 'panel', elevation: 'none' },
});
