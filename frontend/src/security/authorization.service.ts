...
export class AuthorizationError extends Error {
  ...
  constructor(message: string, code: string = 'AUTH_FORBIDDEN', statusCode: number = 403) {
    super(message);
...
export class UnauthorizedError extends AuthorizationError {
  constructor(message: string = 'Connexion requise pour effectuer cette action.') {
    super(message, 'UNAUTHORIZED', 401);
  }
}

export class ForbiddenError extends AuthorizationError {
  constructor(message: string = 'Vous ne disposez pas des autorisations nécessaires pour réaliser cette action.') {
    super(message, 'FORBIDDEN', 403);
  }
}

export class AccountSuspendedError extends AuthorizationError {
  constructor(message: string = 'Votre compte est actuellement suspendu par nos équipes de modération.') {
...
export class MarketScopeForbiddenError extends AuthorizationError {
  constructor(country: string) {
    super(`Accès refusé : votre compte n'est pas habilité à administrer le marché ${country}.`, 'MARKET_SCOPE_FORBIDDEN', 403);
  }
}

export class ResourceOwnershipError extends AuthorizationError {
  constructor(message: string = 'Vous n\'êtes pas propriétaire de cette ressource.') {
    super(message, 'RESOURCE_OWNERSHIP_DENIED', 403);
  }
}

export class EntitlementLimitError extends AuthorizationError {
  constructor(message: string = 'La limite autorisée par votre forfait a été atteinte.') {
...
export class AuthorizationService {
  ...
  can(
    ...
    const isDeactivated = user.isDeactivated || user.status === 'disabled' || user.status === 'deleted';
    ...
    if (resource && !options?.skipOwnershipCheck) {
      const isSuperAdmin = user.primaryRole === 'super_admin' || user.role === 'super_admin';
      ...
      if (permission.endsWith('.own') && !isSuperAdmin && !isAdmin) {
        const resourceOwnerId =
          ...
          resource.userId ||
          resource.authorId ||
          (resource.id && (permission.startsWith('profile.') || permission.startsWith('seller.profile.')) ? resource.id : undefined);
  ...
  assertCan(
    ...
    if (!user) {
      ...
      if (!guestPerms.includes(permission)) {
        throw new UnauthorizedError('Vous devez vous connecter pour effectuer cette action.');
      }
      return;
    }

    if (user.isDeactivated || user.status === 'disabled' || user.status === 'deleted') {
    ...
    const effectivePermissions = this.getEffectivePermissions(user);
    if (!effectivePermissions.includes(permission)) {
      throw new ForbiddenError(`Action non autorisée : permission manquante [${permission}].`);
    }
    ...
    if (resource && !options?.skipOwnershipCheck) {
      ...
      if (permission.endsWith('.own') && !isSuperAdmin && !isAdmin) {
        ...
        if (resourceOwnerId && resourceOwnerId !== user.id) {
          throw new ResourceOwnershipError('Vous ne pouvez modifier ou administrer que vos propres ressources.');
        }
      }
    }
  ...
  canAccessMarket(user: UserProfile | null, countryCode?: string): boolean {
    ...
    if (!user.marketScope || !user.marketScope.countries || user.marketScope.countries.length === 0) {
      return true;
    }
    ...
    return (
      ...
      user.marketScope.countries.map((c) => c.toUpperCase()).includes(normalizedTarget)
    );
  }
  ...
  getUserPlan(user: UserProfile | null): ProPlan {
    const planId = user?.activePlanId || (isProSeller(user) ? 'pro_starter' : 'free');
