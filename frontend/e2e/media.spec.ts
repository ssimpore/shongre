...
for (const viewport of [
  ...
  test.describe(`responsive media at ${viewport.name}`, () => {
    for (const route of MEDIA_ROUTES) {
      test(`${route.name} declares a source ladder and respects the slot`, async ({
        page,
      }) => {
        ...
        await expect
          .poll(async () => (await paintedImages(page)).length, {
            message: `${route.name} rendered no CDN images to check`,
          })
          .toBeGreaterThan(0);
        ...
        expect(
          ...
          `sources overshooting their slot by more than ${OVERSHOOT_BUDGET}x on ${route.name}:\n` +
            oversized
              .map(
                (i) =>
                  `  ${i.slot}px slot needs ${i.painted}px, got ${i.chosen}w — ${i.src}`,
              )
              .join("\n"),
        ).toEqual([]);
      });
...
test("listing detail marks its lead photo as the priority fetch", async ({
  page,
}) => {
  await usePersona(page, "guest");
