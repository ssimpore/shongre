...
test.describe("accessibility", () => {
  for (const route of ALL_ROUTES) {
    test(`${route.name} has no critical or serious violations`, async ({
      ...
      expect(
        blocking,
        `${route.name} (${route.path}) accessibility violations:\n${report}`,
      ).toEqual([]);
    });
  }
});
...
test.describe("accessible names at phone width", () => {
  ...
  for (const route of ROUTES_WITH_COLLAPSING_LABELS) {
    test(`${route.path} names every control at ${PHONE.width}px`, async ({
      ...
      const unnamed = await page.evaluate(() => {
        ...
        document
          ...
          .forEach((el) => {
            ...
            if (!nameOf(el)) {
              offenders.push(
                `${el.tagName} .${(el.className || "").toString().slice(0, 70)}`,
              );
            }
          });
        return offenders;
      ...
      expect(
        unnamed,
        `controls with no accessible name at ${PHONE.width}px:\n  ${unnamed.join("\n  ")}`,
      ).toEqual([]);
    });
  }
});
...
test.describe("keyboard and focus", () => {
  test("every interactive control in the header is reachable and shows focus", async ({
    page,
  }) => {
